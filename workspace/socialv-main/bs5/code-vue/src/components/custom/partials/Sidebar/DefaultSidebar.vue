<template>
  <aside :class="`sidebar sidebar-default sidebar-base navs-rounded-all ${sidebarMenuStyle} ${sidebarType.join(
    ' '
    ...
    <div class="sidebar-header d-flex align-items-center justify-content-start position-relative">
      <router-link to="/" class="d-flex align-items-center gap-2 iq-header-logo">
        ...
        <h3 class="logo-title" data-setting="app_name">{{ appName }}</h3>
      </router-link>
...
export default {
  ...
  setup() {
    ...
    const sidebarMenuStyle = computed(
      () => store.sidebar_menu_style_value
    );
    const toggleSidebar = () => {
    ...
    return { sidebarType, sidebarMenuStyle, toggleSidebar, appName };
  },
};
</script>
