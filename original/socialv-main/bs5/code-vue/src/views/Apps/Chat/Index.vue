<template>
  <div class="chat-page p-0">
    <aside class="sidebar sidebar-chat sidebar-base border-end shadow-none">
      <div class="chat-search pt-4 px-4">
        <h5 class="fw-500">Chats</h5>

        <div
          class="sidebar-toggle d-block d-xl-none"
          data-toggle="sidebar"
          data-active="true"
          @click="toggleSidebar()"
        >
          <i class="icon">
            <svg
              class="icon-20 icon-rtl"
              width="20"
              height="20"
              viewBox="0 0 24 24"
              fill="none"
              xmlns="http://www.w3.org/2000/svg"
            >
              <path
                d="M4.25 12.2744L19.25 12.2744"
                stroke="currentColor"
                stroke-width="1.5"
                stroke-linecap="round"
                stroke-linejoin="round"
              ></path>
              <path
                d="M10.2998 18.2988L4.2498 12.2748L10.2998 6.24976"
                stroke="currentColor"
                stroke-width="1.5"
                stroke-linecap="round"
                stroke-linejoin="round"
              ></path>
            </svg>
          </i>
        </div>
        <div class="chat-searchbar mt-3 pt-1 mb-4">
          <div class="form-group chat-search-data m-0">
            <input
              type="text"
              class="form-control round"
              id="chat-search"
              placeholder="Search for messages or users..."
              v-model="search"
            />
            <i class="material-symbols-outlined"> search </i>
          </div>
        </div>
        <!-- chat swipper -->
        <chat></chat>
      </div>
      <div
        class="sidebar-body pt-0 data-scrollbar mb-5 pb-5 px-4"
        data-scrollbar="true"
        tabindex="-1"
        style="overflow: hidden; outline: none"
      >
        <div class="scroll-content">
          <!-- Sidebar Menu Start -->
          <ul
            class="nav navbar-nav iq-main-menu mb-5 pb-5"
            id="sidebar-menu"
            role="tablist"
          >
            <h6 class="mb-3 pb-1">Recent Chats</h6>
            <li
              class="nav-item iq-chat-list mb-3 ps-0 active"
              role="presentation"
            >
              <a
                href="#user-content-101"
                class="nav-link d-flex gap-3 rounded-2 zoom-in active"
                data-bs-toggle="tab"
                role="tab"
                aria-controls="user-content-101"
                aria-selected="true"
              >
                <div class="position-relative">
                  <img
                    src="../../../assets/images/chat/avatar/01.png"
                    alt="status-101"
                    class="avatar-48 object-cover rounded-circle"
                    loading="lazy"
                  />
                  <div class="iq-profile-badge bg-success"></div>
                </div>
                <div class="d-flex align-items-top w-100 iq-userlist-data">
                  <div class="d-flex flex-grow-1 flex-column">
                    <div class="d-flex align-items-center gap-1">
                      <h6
                        class="mb-0 iq-userlist-name font-size-14 fw-semibold mb-0 text-ellipsis short-1 flex-grow-1"
                      >
                        Paul Molive
                      </h6>
                      <span class="mb-0 font-size-12">03:20 PM</span>
                    </div>
                    <div class="d-flex align-items-center gap-2">
                      <p
                        class="text-ellipsis short-1 flex-grow-1 font-size-14 mb-0"
                      >
                        Lorem ipsum
                      </p>
                      <div class="btn-group dropdown-user">
                        <button
                          class="bg-transparent dropdown-toggle border-0 text-white"
                          data-bs-toggle="dropdown"
                          aria-expanded="false"
                        ></button>
                        <ul class="dropdown-menu dropdown-menu-end p-0">
                          <li>
                            <button
                              class="dropdown-item font-size-14 text-dark px-2"
                              type="button"
                            >
                              <span
                                class="material-symbols-outlined mx-1 font-size-20 align-middle text-body"
                                >share</span
                              >Share Contact
                            </button>
                          </li>
                          <li>
                            <button
                              class="dropdown-item font-size-14 text-dark px-2"
                              type="button"
                            >
                              <span
                                class="material-symbols-outlined mx-1 font-size-20 align-middle text-body"
                                >content_copy</span
                              >Copy Contact
                            </button>
                          </li>
                        </ul>
                      </div>
                    </div>
                  </div>
                </div>
              </a>
            </li>
            <li class="nav-item iq-chat-list mb-3 ps-0" role="presentation">
              <a
                href="#user-content-102"
                class="nav-link d-flex gap-3 rounded-2 zoom-in"
                data-bs-toggle="tab"
                role="tab"
                aria-controls="user-content-102"
                aria-selected="false"
                tabindex="-1"
              >
                <div class="position-relative">
                  <img
                    src="../../../assets/images/chat/avatar/02.png"
                    alt="status-102"
                    class="avatar-48 object-cover rounded-circle"
                    loading="lazy"
                  />
                  <div class="iq-profile-badge bg-success"></div>
                </div>
                <div class="d-flex align-items-top w-100 iq-userlist-data">
                  <div class="d-flex flex-grow-1 flex-column">
                    <div class="d-flex align-items-center gap-1">
                      <h6
                        class="mb-0 iq-userlist-name font-size-14 fw-semibold mb-0 text-ellipsis short-1 flex-grow-1"
                      >
                        John Travolta
                      </h6>
                      <span class="mb-0 font-size-12">01:10 PM</span>
                    </div>
                    <div class="d-flex align-items-center gap-2">
                      <p
                        class="text-ellipsis short-1 flex-grow-1 font-size-14 mb-0"
                      >
                        Lorem ipsum
                      </p>
                      <div class="btn-group dropdown-user">
                        <button
                          class="bg-transparent dropdown-toggle border-0 text-white"
                          data-bs-toggle="dropdown"
                          aria-expanded="false"
                        ></button>
                        <ul class="dropdown-menu dropdown-menu-end p-0">
                          <li>
                            <button
                              class="dropdown-item font-size-14 text-dark px-2"
                              type="button"
                            >
                              <span
                                class="material-symbols-outlined mx-1 font-size-20 align-middle text-body"
                                >share</span
                              >Share Contact
                            </button>
                          </li>
                          <li>
                            <button
                              class="dropdown-item font-size-14 text-dark px-2"
                              type="button"
                            >
                              <span
                                class="material-symbols-outlined mx-1 font-size-20 align-middle text-body"
                                >content_copy</span
                              >Copy Contact
                            </button>
                          </li>
                        </ul>
                      </div>
                    </div>
                  </div>
                </div>
              </a>
            </li>
            <li class="nav-item iq-chat-list mb-3 ps-0" role="presentation">
              <a
                href="#user-content-103"
                class="nav-link d-flex gap-3 rounded-2 zoom-in"
                data-bs-toggle="tab"
                role="tab"
                aria-controls="user-content-103"
                aria-selected="false"
                tabindex="-1"
              >
                <div class="position-relative">
                  <img
                    src="../../../assets/images/chat/avatar/03.png"
                    alt="status-103"
                    class="avatar-48 object-cover rounded-circle"
                    loading="lazy"
                  />
                  <div class="iq-profile-badge bg-success"></div>
                </div>
                <div class="d-flex align-items-top w-100 iq-userlist-data">
                  <div class="d-flex flex-grow-1 flex-column">
                    <div class="d-flex align-items-center gap-1">
                      <h6
                        class="mb-0 iq-userlist-name font-size-14 fw-semibold mb-0 text-ellipsis short-1 flex-grow-1"
                      >
                        Barb Ackue
                      </h6>
                      <span class="mb-0 font-size-12">05:09 AM</span>
                    </div>
                    <div class="d-flex align-items-center gap-2">
                      <p
                        class="text-ellipsis short-1 flex-grow-1 font-size-14 mb-0"
                      >
                        Lorem ipsum
                      </p>
                      <div class="btn-group dropdown-user">
                        <button
                          class="bg-transparent dropdown-toggle border-0 text-white"
                          data-bs-toggle="dropdown"
                          aria-expanded="false"
                        ></button>
                        <ul class="dropdown-menu dropdown-menu-end p-0">
                          <li>
                            <button
                              class="dropdown-item font-size-14 text-dark px-2"
                              type="button"
                            >
                              <span
                                class="material-symbols-outlined mx-1 font-size-20 align-middle text-body"
                                >share</span
                              >Share Contact
                            </button>
                          </li>
                          <li>
                            <button
                              class="dropdown-item font-size-14 text-dark px-2"
                              type="button"
                            >
                              <span
                                class="material-symbols-outlined mx-1 font-size-20 align-middle text-body"
                                >content_copy</span
                              >Copy Contact
                            </button>
                          </li>
                        </ul>
                      </div>
                    </div>
                  </div>
                </div>
              </a>
            </li>
            <li class="nav-item iq-chat-list mb-3 ps-0" role="presentation">
              <a
                href="#user-content-104"
                class="nav-link d-flex gap-3 rounded-2 zoom-in"
                data-bs-toggle="tab"
                role="tab"
                aria-controls="user-content-104"
                aria-selected="false"
                tabindex="-1"
              >
                <div class="position-relative">
                  <img
                    src="../../../assets/images/chat/avatar/04.png"
                    alt="status-104"
                    class="avatar-48 object-cover rounded-circle"
                    loading="lazy"
                  />
                  <div class="iq-profile-badge bg-success"></div>
                </div>
                <div class="d-flex align-items-top w-100 iq-userlist-data">
                  <div class="d-flex flex-grow-1 flex-column">
                    <div class="d-flex align-items-center gap-1">
                      <h6
                        class="mb-0 iq-userlist-name font-size-14 fw-semibold mb-0 text-ellipsis short-1 flex-grow-1"
                      >
                        Robert Fox
                      </h6>
                      <span class="mb-0 font-size-12">01:10 PM</span>
                    </div>
                    <div class="d-flex align-items-center gap-2">
                      <p
                        class="text-ellipsis short-1 flex-grow-1 font-size-14 mb-0"
                      >
                        Lorem ipsum
                      </p>
                      <div class="btn-group dropdown-user">
                        <button
                          class="bg-transparent dropdown-toggle border-0 text-white"
                          data-bs-toggle="dropdown"
                          aria-expanded="false"
                        ></button>
                        <ul class="dropdown-menu dropdown-menu-end p-0">
                          <li>
                            <button
                              class="dropdown-item font-size-14 text-dark px-2"
                              type="button"
                            >
                              <span
                                class="material-symbols-outlined mx-1 font-size-20 align-middle text-body"
                                >share</span
                              >Share Contact
                            </button>
                          </li>
                          <li>
                            <button
                              class="dropdown-item font-size-14 text-dark px-2"
                              type="button"
                            >
                              <span
                                class="material-symbols-outlined mx-1 font-size-20 align-middle text-body"
                                >content_copy</span
                              >Copy Contact
                            </button>
                          </li>
                        </ul>
                      </div>
                    </div>
                  </div>
                </div>
              </a>
            </li>
            <li class="nav-item iq-chat-list mb-3 ps-0" role="presentation">
              <a
                href="#user-content-105"
                class="nav-link d-flex gap-3 rounded-2 zoom-in"
                data-bs-toggle="tab"
                role="tab"
                aria-controls="user-content-105"
                aria-selected="false"
                tabindex="-1"
              >
                <div class="position-relative">
                  <img
                    src="../../../assets/images/chat/avatar/05.png"
                    alt="status-105"
                    class="avatar-48 object-cover rounded-circle"
                    loading="lazy"
                  />
                  <div class="iq-profile-badge bg-danger"></div>
                </div>
                <div class="d-flex align-items-top w-100 iq-userlist-data">
                  <div class="d-flex flex-grow-1 flex-column">
                    <div class="d-flex align-items-center gap-1">
                      <h6
                        class="mb-0 iq-userlist-name font-size-14 fw-semibold mb-0 text-ellipsis short-1 flex-grow-1"
                      >
                        Maya Didas
                      </h6>
                      <span class="mb-0 font-size-12">01:10 PM</span>
                    </div>
                    <div class="d-flex align-items-center gap-2">
                      <p
                        class="text-ellipsis short-1 flex-grow-1 font-size-14 mb-0"
                      >
                        Lorem ipsum
                      </p>
                      <div class="btn-group dropdown-user">
                        <button
                          class="bg-transparent dropdown-toggle border-0 text-white"
                          data-bs-toggle="dropdown"
                          aria-expanded="false"
                        ></button>
                        <ul class="dropdown-menu dropdown-menu-end p-0">
                          <li>
                            <button
                              class="dropdown-item font-size-14 text-dark px-2"
                              type="button"
                            >
                              <span
                                class="material-symbols-outlined mx-1 font-size-20 align-middle text-body"
                                >share</span
                              >Share Contact
                            </button>
                          </li>
                          <li>
                            <button
                              class="dropdown-item font-size-14 text-dark px-2"
                              type="button"
                            >
                              <span
                                class="material-symbols-outlined mx-1 font-size-20 align-middle text-body"
                                >content_copy</span
                              >Copy Contact
                            </button>
                          </li>
                        </ul>
                      </div>
                    </div>
                  </div>
                </div>
              </a>
            </li>
            <li class="nav-item iq-chat-list mb-3 ps-0" role="presentation">
              <a
                href="#user-content-106"
                class="nav-link d-flex gap-3 rounded-2 zoom-in"
                data-bs-toggle="tab"
                role="tab"
                aria-controls="user-content-106"
                aria-selected="false"
                tabindex="-1"
              >
                <div class="position-relative">
                  <img
                    src="../../../assets/images/chat/avatar/06.png"
                    alt="status-106"
                    class="avatar-48 object-cover rounded-circle"
                    loading="lazy"
                  />
                  <div class="iq-profile-badge bg-success"></div>
                </div>
                <div class="d-flex align-items-top w-100 iq-userlist-data">
                  <div class="d-flex flex-grow-1 flex-column">
                    <div class="d-flex align-items-center gap-1">
                      <h6
                        class="mb-0 iq-userlist-name font-size-14 fw-semibold mb-0 text-ellipsis short-1 flex-grow-1"
                      >
                        Monty Carlo
                      </h6>
                      <span class="mb-0 font-size-12">01:10 PM</span>
                    </div>
                    <div class="d-flex align-items-center gap-2">
                      <p
                        class="text-ellipsis short-1 flex-grow-1 font-size-14 mb-0"
                      >
                        Lorem ipsum
                      </p>
                      <div class="btn-group dropdown-user">
                        <button
                          class="bg-transparent dropdown-toggle border-0 text-white"
                          data-bs-toggle="dropdown"
                          aria-expanded="false"
                        ></button>
                        <ul class="dropdown-menu dropdown-menu-end p-0">
                          <li>
                            <button
                              class="dropdown-item font-size-14 text-dark px-2"
                              type="button"
                            >
                              <span
                                class="material-symbols-outlined mx-1 font-size-20 align-middle text-body"
                                >share</span
                              >Share Contact
                            </button>
                          </li>
                          <li>
                            <button
                              class="dropdown-item font-size-14 text-dark px-2"
                              type="button"
                            >
                              <span
                                class="material-symbols-outlined mx-1 font-size-20 align-middle text-body"
                                >content_copy</span
                              >Copy Contact
                            </button>
                          </li>
                        </ul>
                      </div>
                    </div>
                  </div>
                </div>
              </a>
            </li>
            <li class="nav-item iq-chat-list mb-3 ps-0" role="presentation">
              <a
                href="#user-content-107"
                class="nav-link d-flex gap-3 rounded-2 zoom-in"
                data-bs-toggle="tab"
                role="tab"
                aria-controls="user-content-107"
                aria-selected="false"
                tabindex="-1"
              >
                <div class="position-relative">
                  <img
                    src="../../../assets/images/chat/avatar/07.png"
                    alt="status-107"
                    class="avatar-48 object-cover rounded-circle"
                    loading="lazy"
                  />
                  <div class="iq-profile-badge bg-success"></div>
                </div>
                <div class="d-flex align-items-top w-100 iq-userlist-data">
                  <div class="d-flex flex-grow-1 flex-column">
                    <div class="d-flex align-items-center gap-1">
                      <h6
                        class="mb-0 iq-userlist-name font-size-14 fw-semibold mb-0 text-ellipsis short-1 flex-grow-1"
                      >
                        Paige Turner
                      </h6>
                      <span class="mb-0 font-size-12">01:10 PM</span>
                    </div>
                    <div class="d-flex align-items-center gap-2">
                      <p
                        class="text-ellipsis short-1 flex-grow-1 font-size-14 mb-0"
                      >
                        Lorem ipsum
                      </p>
                      <div class="btn-group dropdown-user">
                        <button
                          class="bg-transparent dropdown-toggle border-0 text-white"
                          data-bs-toggle="dropdown"
                          aria-expanded="false"
                        ></button>
                        <ul class="dropdown-menu dropdown-menu-end p-0">
                          <li>
                            <button
                              class="dropdown-item font-size-14 text-dark px-2"
                              type="button"
                            >
                              <span
                                class="material-symbols-outlined mx-1 font-size-20 align-middle text-body"
                                >share</span
                              >Share Contact
                            </button>
                          </li>
                          <li>
                            <button
                              class="dropdown-item font-size-14 text-dark px-2"
                              type="button"
                            >
                              <span
                                class="material-symbols-outlined mx-1 font-size-20 align-middle text-body"
                                >content_copy</span
                              >Copy Contact
                            </button>
                          </li>
                        </ul>
                      </div>
                    </div>
                  </div>
                </div>
              </a>
            </li>
            <li class="nav-item iq-chat-list mb-3 ps-0" role="presentation">
              <a
                href="#user-content-108"
                class="nav-link d-flex gap-3 rounded-2 zoom-in"
                data-bs-toggle="tab"
                role="tab"
                aria-controls="user-content-108"
                aria-selected="false"
                tabindex="-1"
              >
                <div class="position-relative">
                  <img
                    src="../../../assets/images/chat/avatar/08.png"
                    alt="status-108"
                    class="avatar-48 object-cover rounded-circle"
                    loading="lazy"
                  />
                  <div class="iq-profile-badge bg-danger"></div>
                </div>
                <div class="d-flex align-items-top w-100 iq-userlist-data">
                  <div class="d-flex flex-grow-1 flex-column">
                    <div class="d-flex align-items-center gap-1">
                      <h6
                        class="mb-0 iq-userlist-name font-size-14 fw-semibold mb-0 text-ellipsis short-1 flex-grow-1"
                      >
                        Arnold Schwarzenegger
                      </h6>
                      <span class="mb-0 font-size-12">01:10 PM</span>
                    </div>
                    <div class="d-flex align-items-center gap-2">
                      <p
                        class="text-ellipsis short-1 flex-grow-1 font-size-14 mb-0"
                      >
                        Lorem ipsum
                      </p>
                      <div class="btn-group dropdown-user">
                        <button
                          class="bg-transparent dropdown-toggle border-0 text-white"
                          data-bs-toggle="dropdown"
                          aria-expanded="false"
                        ></button>
                        <ul class="dropdown-menu dropdown-menu-end p-0">
                          <li>
                            <button
                              class="dropdown-item font-size-14 text-dark px-2"
                              type="button"
                            >
                              <span
                                class="material-symbols-outlined mx-1 font-size-20 align-middle text-body"
                                >share</span
                              >Share Contact
                            </button>
                          </li>
                          <li>
                            <button
                              class="dropdown-item font-size-14 text-dark px-2"
                              type="button"
                            >
                              <span
                                class="material-symbols-outlined mx-1 font-size-20 align-middle text-body"
                                >content_copy</span
                              >Copy Contact
                            </button>
                          </li>
                        </ul>
                      </div>
                    </div>
                  </div>
                </div>
              </a>
            </li>
            <li class="nav-item iq-chat-list mb-3 ps-0" role="presentation">
              <a
                href="#user-content-109"
                class="nav-link d-flex gap-3 rounded-2 zoom-in"
                data-bs-toggle="tab"
                role="tab"
                aria-controls="user-content-109"
                aria-selected="false"
                tabindex="-1"
              >
                <div class="position-relative">
                  <img
                    src="../../../assets/images/chat/avatar/09.png"
                    alt="status-109"
                    class="avatar-48 object-cover rounded-circle"
                    loading="lazy"
                  />
                  <div class="iq-profile-badge bg-success"></div>
                </div>
                <div class="d-flex align-items-top w-100 iq-userlist-data">
                  <div class="d-flex flex-grow-1 flex-column">
                    <div class="d-flex align-items-center gap-1">
                      <h6
                        class="mb-0 iq-userlist-name font-size-14 fw-semibold mb-0 text-ellipsis short-1 flex-grow-1"
                      >
                        Leonardo DiCaprio
                      </h6>
                      <span class="mb-0 font-size-12">01:10 PM</span>
                    </div>
                    <div class="d-flex align-items-center gap-2">
                      <p
                        class="text-ellipsis short-1 flex-grow-1 font-size-14 mb-0"
                      >
                        Lorem ipsum
                      </p>
                      <div class="btn-group dropdown-user">
                        <button
                          class="bg-transparent dropdown-toggle border-0 text-white"
                          data-bs-toggle="dropdown"
                          aria-expanded="false"
                        ></button>
                        <ul class="dropdown-menu dropdown-menu-end p-0">
                          <li>
                            <button
                              class="dropdown-item font-size-14 text-dark px-2"
                              type="button"
                            >
                              <span
                                class="material-symbols-outlined mx-1 font-size-20 align-middle text-body"
                                >share</span
                              >Share Contact
                            </button>
                          </li>
                          <li>
                            <button
                              class="dropdown-item font-size-14 text-dark px-2"
                              type="button"
                            >
                              <span
                                class="material-symbols-outlined mx-1 font-size-20 align-middle text-body"
                                >content_copy</span
                              >Copy Contact
                            </button>
                          </li>
                        </ul>
                      </div>
                    </div>
                  </div>
                </div>
              </a>
            </li>
          </ul>
          <!-- Sidebar Menu End -->
        </div>
        <div class="scrollbar-track scrollbar-track-x" style="display: none">
          <div
            class="scrollbar-thumb scrollbar-thumb-x"
            style="width: 469px; transform: translate3d(0px, 0px, 0px)"
          ></div>
        </div>
        <div class="scrollbar-track scrollbar-track-y" style="display: block">
          <div
            class="scrollbar-thumb scrollbar-thumb-y"
            style="height: 266.998px; transform: translate3d(0px, 0px, 0px)"
          ></div>
        </div>
      </div>
    </aside>
    <!-- <div class="col-lg-9 chat-data p-0 chat-data-right" :style="`background: url(${require('../../../assets/images/page-img/100.jpg')}) no-repeat scroll center center;background-size: cover;`">
                </div> -->
    <main class="main-content">
      <div class="container-fluid content-inner p-0" id="page_layout">
        <div class="tab-content" id="myTabContent">
          <div
            class="card tab-pane mb-0 fade active show"
            id="user-content-101"
            role="tabpanel"
          >
            <div class="chat-head">
              <header
                class="d-flex justify-content-between align-items-center pt-3 ps-3 pe-3 pb-3"
              >
                <div class="d-flex align-items-center gap-3">
                  <div class="d-block d-xl-none">
                    <button
                      class="btn btn-sm btn-primary rounded btn-icon"
                      data-toggle="sidebar"
                      data-active="true"
                      @click="toggleSidebar()"
                    >
                      <span class="btn-inner">
                        <svg class="icon-rtl" width="20px" viewBox="0 0 24 24">
                          <path
                            fill="currentColor"
                            d="M4,11V13H16L10.5,18.5L11.92,19.92L19.84,12L11.92,4.08L10.5,5.5L16,11H4Z"
                          ></path>
                        </svg>
                      </span>
                    </button>
                  </div>
                  <div class="avatar chat-user-profile m-0">
                    <img
                      src="../../../assets/images/chat/avatar/01.png"
                      alt="avatar"
                      class="avatar-50 rounded-pill"
                      loading="lazy"
                    />
                    <div class="iq-profile-badge bg-success"></div>
                  </div>
                  <div>
                    <h5 class="mb-0">Paul Molive</h5>
                    <small class="text-capitalize fw-500">Online</small>
                  </div>
                </div>

                <div class="chat-header-icons d-inline-flex ms-auto">
                  <a
                    href="#"
                    class="chat-icon-phone bg-primary-subtle d-flex align-items-center justify-content-center"
                  >
                    <i class="material-symbols-outlined md-18">phone</i>
                  </a>
                  <a
                    href="#"
                    class="chat-icon-video bg-primary-subtle d-flex align-items-center justify-content-center"
                  >
                    <i class="material-symbols-outlined md-18">videocam</i>
                  </a>
                  <a
                    href="#"
                    class="chat-icon-delete bg-primary-subtle d-flex align-items-center justify-content-center"
                  >
                    <i class="material-symbols-outlined md-18">delete</i>
                  </a>
                  <span
                    class="dropdown bg-primary-subtle d-flex align-items-center justify-content-center"
                  >
                    <svg
                      class="icon-20 nav-hide-arrow cursor-pointer pe-0"
                      id="dropdownMenuButton-09"
                      data-bs-toggle="dropdown"
                      aria-haspopup="true"
                      aria-expanded="false"
                      role="menu"
                      width="24"
                      xmlns="http://www.w3.org/2000/svg"
                      viewBox="0 0 24 24"
                      fill="none"
                    >
                      <path
                        fill-rule="evenodd"
                        clip-rule="evenodd"
                        d="M10 20.6788C10 21.9595 11.0378 23 12.3113 23C13.5868 23 14.6265 21.9595 14.6265 20.6788C14.6265 19.3981 13.5868 18.3576 12.3113 18.3576C11.0378 18.3576 10 19.3981 10 20.6788ZM10 12.0005C10 13.2812 11.0378 14.3217 12.3113 14.3217C13.5868 14.3217 14.6265 13.2812 14.6265 12.0005C14.6265 10.7198 13.5868 9.67929 12.3113 9.67929C11.0378 9.67929 10 10.7198 10 12.0005ZM12.3113 5.64239C11.0378 5.64239 10 4.60192 10 3.3212C10 2.04047 11.0378 1 12.3113 1C13.5868 1 14.6265 2.04047 14.6265 3.3212C14.6265 4.60192 13.5868 5.64239 12.3113 5.64239Z"
                        fill="currentColor"
                      ></path>
                    </svg>
                    <span
                      class="dropdown-menu dropdown-menu-right"
                      aria-labelledby="dropdownMenuButton-09"
                    >
                      <a
                        class="dropdown-item d-flex align-items-center"
                        href="#"
                        ><i class="material-symbols-outlined md-18 me-1"
                          >push_pin</i
                        >Pin to top</a
                      >
                      <a
                        class="dropdown-item d-flex align-items-center"
                        href="#"
                        ><i class="material-symbols-outlined md-18 me-1"
                          >delete_outline</i
                        >Delete chat</a
                      >
                      <a
                        class="dropdown-item d-flex align-items-center"
                        href="#"
                        ><i class="material-symbols-outlined md-18 me-1"
                          >watch_later</i
                        >Block</a
                      >
                    </span>
                  </span>
                </div>
              </header>
            </div>
            <div class="card-body chat-body bg-body">
              <div class="chat-day-title">
                <span class="main-title">Feb 1,2021</span>
              </div>
              <div class="iq-message-body iq-current-user">
                <div class="chat-profile text-center">
                  <img
                    src="../../../assets/images/chat/avatar/10.png"
                    alt="chat-user"
                    class="avatar-40 rounded-pill"
                    loading="lazy"
                  />
                  <small class="iq-chating p-0 mb-0 d-block">16:34</small>
                </div>
                <div class="iq-chat-text">
                  <div
                    class="d-flex align-items-center justify-content-end gap-1 gap-md-2"
                  >
                    <div
                      class="dropdown cursor-pointer more"
                      v-b-tooltip.hover
                      title="More"
                    >
                      <div
                        class="lh-1"
                        id="post-option"
                        data-bs-toggle="dropdown"
                      >
                        <span class="material-symbols-outlined text-dark">
                          more_vert
                        </span>
                      </div>
                      <div
                        class="dropdown-menu dropdown-menu-right"
                        aria-labelledby="post-option"
                        style=""
                      >
                        <a class="dropdown-item" href="#"
                          ><span
                            class="material-symbols-outlined align-middle font-size-20 me-1"
                            >content_copy</span
                          >Copy message text</a
                        >
                        <a class="dropdown-item" href="#">
                          <span
                            class="material-symbols-outlined align-middle font-size-20 me-1"
                            >edit</span
                          >Edit</a
                        >
                      </div>
                    </div>
                    <div class="iq-chating-content d-flex align-items-center">
                      <p class="mr-2 mb-0">
                        How can we help? We're here for you! 😄
                      </p>
                    </div>
                  </div>
                </div>
              </div>
              <div class="iq-message-body iq-other-user">
                <div class="chat-profile text-center">
                  <img
                    src="../../../assets/images/chat/avatar/01.png"
                    alt="chat-user"
                    class="avatar-40 rounded-pill"
                    loading="lazy"
                  />
                  <small class="iq-chating p-0 mb-0 d-block">16:40</small>
                </div>
                <div class="iq-chat-text">
                  <div
                    class="d-flex align-items-center justify-content-start gap-md-2"
                  >
                    <div class="iq-chating-content d-flex align-items-center">
                      <p class="mr-2 mb-0">
                        Hey John, I am looking for the best admin template.Could
                        you please help me to find it out? 🤔
                      </p>
                    </div>
                    <a
                      href="#"
                      class="material-symbols-outlined font-size-20 text-dark reply"
                      v-b-tooltip.hover
                      title="Reply"
                      >reply</a
                    >
                    <div
                      class="dropdown cursor-pointer more"
                      v-b-tooltip.hover
                      title="More"
                    >
                      <div
                        class="lh-1"
                        id="post-option"
                        data-bs-toggle="dropdown"
                      >
                        <span class="material-symbols-outlined text-dark">
                          more_vert
                        </span>
                      </div>
                      <div
                        class="dropdown-menu dropdown-menu-right"
                        aria-labelledby="post-option"
                        style=""
                      >
                        <a class="dropdown-item" href="#"
                          ><span
                            class="material-symbols-outlined align-middle font-size-20 me-1"
                            >content_copy</span
                          >Copy message text</a
                        >
                        <a class="dropdown-item" href="#">
                          <span
                            class="material-symbols-outlined align-middle font-size-20 me-1"
                            >edit</span
                          >Edit</a
                        >
                      </div>
                    </div>
                  </div>
                </div>
              </div>
              <div class="iq-message-body iq-current-user">
                <div class="chat-profile text-center">
                  <img
                    src="../../../assets/images/chat/avatar/10.png"
                    alt="chat-user"
                    class="avatar-40 rounded-pill"
                    loading="lazy"
                  />
                  <small class="iq-chating p-0 mb-0 d-block">6:49</small>
                </div>
                <div class="iq-chat-text">
                  <div
                    class="d-flex align-items-center justify-content-end gap-1 gap-md-2"
                  >
                    <div
                      class="dropdown cursor-pointer more"
                      v-b-tooltip.hover
                      title="More"
                    >
                      <div
                        class="lh-1"
                        id="post-option"
                        data-bs-toggle="dropdown"
                      >
                        <span class="material-symbols-outlined text-dark">
                          more_vert
                        </span>
                      </div>
                      <div
                        class="dropdown-menu dropdown-menu-right"
                        aria-labelledby="post-option"
                        style=""
                      >
                        <a class="dropdown-item" href="#"
                          ><span
                            class="material-symbols-outlined align-middle font-size-20 me-1"
                            >content_copy</span
                          >Copy message text</a
                        >
                        <a class="dropdown-item" href="#">
                          <span
                            class="material-symbols-outlined align-middle font-size-20 me-1"
                            >edit</span
                          >Edit</a
                        >
                      </div>
                    </div>
                    <div class="iq-chating-content d-flex align-items-center">
                      <p class="mr-2 mb-0">
                        Absolutely Yes! SocialV is the Responsive Bootstrap 5
                        Admin Dashboard Template.
                      </p>
                    </div>
                  </div>
                </div>
              </div>
              <div class="iq-message-body iq-other-user">
                <div class="chat-profile text-center">
                  <img
                    src="../../../assets/images/chat/avatar/01.png"
                    alt="chat-user"
                    class="avatar-40 rounded-pill"
                    loading="lazy"
                  />
                  <small class="iq-chating p-0 mb-0 d-block">6:52</small>
                </div>
                <div class="iq-chat-text">
                  <div
                    class="d-flex align-items-center justify-content-start gap-md-2"
                  >
                    <div class="iq-chating-content d-flex align-items-center">
                      <p class="mr-2 mb-0">Looks clean and fresh UI.</p>
                    </div>
                    <a
                      href="#"
                      class="material-symbols-outlined font-size-20 text-dark reply"
                      v-b-tooltip.hover
                      title="Reply"
                      >reply</a
                    >
                    <div
                      class="dropdown cursor-pointer more"
                      v-b-tooltip.hover
                      title="More"
                    >
                      <div
                        class="lh-1"
                        id="post-option"
                        data-bs-toggle="dropdown"
                      >
                        <span class="material-symbols-outlined text-dark">
                          more_vert
                        </span>
                      </div>
                      <div
                        class="dropdown-menu dropdown-menu-right"
                        aria-labelledby="post-option"
                        style=""
                      >
                        <a class="dropdown-item" href="#"
                          ><span
                            class="material-symbols-outlined align-middle font-size-20 me-1"
                            >content_copy</span
                          >Copy message text</a
                        >
                        <a class="dropdown-item" href="#">
                          <span
                            class="material-symbols-outlined align-middle font-size-20 me-1"
                            >edit</span
                          >Edit</a
                        >
                      </div>
                    </div>
                  </div>
                </div>
              </div>
              <div class="iq-message-body iq-current-user">
                <div class="chat-profile text-center">
                  <img
                    src="../../../assets/images/chat/avatar/10.png"
                    alt="chat-user"
                    class="avatar-40 rounded-pill"
                    loading="lazy"
                  />
                  <small class="iq-chating p-0 mb-0 d-block">6:53</small>
                </div>
                <div class="iq-chat-text">
                  <div
                    class="d-flex align-items-center justify-content-end gap-1 gap-md-2"
                  >
                    <div
                      class="dropdown cursor-pointer more"
                      v-b-tooltip.hover
                      title="More"
                    >
                      <div
                        class="lh-1"
                        id="post-option"
                        data-bs-toggle="dropdown"
                      >
                        <span class="material-symbols-outlined text-dark">
                          more_vert
                        </span>
                      </div>
                      <div
                        class="dropdown-menu dropdown-menu-right"
                        aria-labelledby="post-option"
                        style=""
                      >
                        <a class="dropdown-item" href="#"
                          ><span
                            class="material-symbols-outlined align-middle font-size-20 me-1"
                            >content_copy</span
                          >Copy message text</a
                        >
                        <a class="dropdown-item" href="#">
                          <span
                            class="material-symbols-outlined align-middle font-size-20 me-1"
                            >edit</span
                          >Edit</a
                        >
                      </div>
                    </div>
                    <div class="iq-chating-content d-flex align-items-center">
                      <p class="mr-2 mb-0">Thanks, from ThemeForest.</p>
                    </div>
                  </div>
                </div>
              </div>
              <div class="iq-message-body iq-other-user">
                <div class="chat-profile text-center">
                  <img
                    src="../../../assets/images/chat/avatar/01.png"
                    alt="chat-user"
                    class="avatar-40 rounded-pill"
                    loading="lazy"
                  />
                  <small class="iq-chating p-0 mb-0 d-block">6:54</small>
                </div>
                <div class="iq-chat-text">
                  <div
                    class="d-flex align-items-center justify-content-start gap-md-2"
                  >
                    <div class="iq-chating-content d-flex align-items-center">
                      <p class="mr-2 mb-0">I will purchase it for sure. 👍</p>
                    </div>
                    <a
                      href="#"
                      class="material-symbols-outlined font-size-20 text-dark reply"
                      v-b-tooltip.hover
                      title="Reply"
                      >reply</a
                    >
                    <div
                      class="dropdown cursor-pointer more"
                      v-b-tooltip.hover
                      title="More"
                    >
                      <div
                        class="lh-1"
                        id="post-option"
                        data-bs-toggle="dropdown"
                      >
                        <span class="material-symbols-outlined text-dark">
                          more_vert
                        </span>
                      </div>
                      <div
                        class="dropdown-menu dropdown-menu-right"
                        aria-labelledby="post-option"
                        style=""
                      >
                        <a class="dropdown-item" href="#"
                          ><span
                            class="material-symbols-outlined align-middle font-size-20 me-1"
                            >content_copy</span
                          >Copy message text</a
                        >
                        <a class="dropdown-item" href="#">
                          <span
                            class="material-symbols-outlined align-middle font-size-20 me-1"
                            >edit</span
                          >Edit</a
                        >
                      </div>
                    </div>
                  </div>
                </div>
              </div>
              <div class="iq-message-body iq-current-user">
                <div class="chat-profile text-center">
                  <img
                    src="../../../assets/images/chat/avatar/10.png"
                    alt="chat-user"
                    class="avatar-40 rounded-pill"
                    loading="lazy"
                  />
                  <small class="iq-chating p-0 mb-0 d-block">6:55</small>
                </div>
                <div class="iq-chat-text">
                  <div
                    class="d-flex align-items-center justify-content-end gap-1 gap-md-2"
                  >
                    <div
                      class="dropdown cursor-pointer more"
                      v-b-tooltip.hover
                      title="More"
                    >
                      <div
                        class="lh-1"
                        id="post-option"
                        data-bs-toggle="dropdown"
                      >
                        <span class="material-symbols-outlined text-dark">
                          more_vert
                        </span>
                      </div>
                      <div
                        class="dropdown-menu dropdown-menu-right"
                        aria-labelledby="post-option"
                        style=""
                      >
                        <a class="dropdown-item" href="#"
                          ><span
                            class="material-symbols-outlined align-middle font-size-20 me-1"
                            >content_copy</span
                          >Copy message text</a
                        >
                        <a class="dropdown-item" href="#">
                          <span
                            class="material-symbols-outlined align-middle font-size-20 me-1"
                            >edit</span
                          >Edit</a
                        >
                      </div>
                    </div>
                    <div class="iq-chating-content d-flex align-items-center">
                      <p class="mr-2 mb-0">Okay Thanks..</p>
                    </div>
                  </div>
                </div>
              </div>
              <div class="iq-message-body iq-other-user">
                <div class="chat-profile text-center">
                  <img
                    src="../../../assets/images/chat/avatar/01.png"
                    alt="chat-user"
                    class="avatar-40 rounded-pill"
                    loading="lazy"
                  />
                  <small class="iq-chating p-0 mb-0 d-block">7:54</small>
                </div>
                <div class="iq-chat-text">
                  <div
                    class="d-flex align-items-center justify-content-start gap-md-2"
                  >
                    <div class="iq-chating-content d-flex align-items-center">
                      <p class="mr-2 mb-0">Looks clean and fresh UI.</p>
                    </div>
                    <a
                      href="#"
                      class="material-symbols-outlined font-size-20 text-dark reply"
                      v-b-tooltip.hover
                      title="Reply"
                      >reply</a
                    >
                    <div
                      class="dropdown cursor-pointer more"
                      v-b-tooltip.hover
                      title="More"
                    >
                      <div
                        class="lh-1"
                        id="post-option"
                        data-bs-toggle="dropdown"
                      >
                        <span class="material-symbols-outlined text-dark">
                          more_vert
                        </span>
                      </div>
                      <div
                        class="dropdown-menu dropdown-menu-right"
                        aria-labelledby="post-option"
                        style=""
                      >
                        <a class="dropdown-item" href="#"
                          ><span
                            class="material-symbols-outlined align-middle font-size-20 me-1"
                            >content_copy</span
                          >Copy message text</a
                        >
                        <a class="dropdown-item" href="#">
                          <span
                            class="material-symbols-outlined align-middle font-size-20 me-1"
                            >edit</span
                          >Edit</a
                        >
                      </div>
                    </div>
                  </div>
                </div>
              </div>
              <div class="iq-message-body iq-current-user">
                <div class="chat-profile text-center">
                  <img
                    src="../../../assets/images/chat/avatar/10.png"
                    alt="chat-user"
                    class="avatar-40 rounded-pill"
                    loading="lazy"
                  />
                  <small class="iq-chating p-0 mb-0 d-block">7:55</small>
                </div>
                <div class="iq-chat-text">
                  <div
                    class="d-flex align-items-center justify-content-end gap-1 gap-md-2"
                  >
                    <div
                      class="dropdown cursor-pointer more"
                      v-b-tooltip.hover
                      title="More"
                    >
                      <div
                        class="lh-1"
                        id="post-option"
                        data-bs-toggle="dropdown"
                      >
                        <span class="material-symbols-outlined text-dark">
                          more_vert
                        </span>
                      </div>
                      <div
                        class="dropdown-menu dropdown-menu-right"
                        aria-labelledby="post-option"
                        style=""
                      >
                        <a class="dropdown-item" href="#"
                          ><span
                            class="material-symbols-outlined align-middle font-size-20 me-1"
                            >content_copy</span
                          >Copy message text</a
                        >
                        <a class="dropdown-item" href="#">
                          <span
                            class="material-symbols-outlined align-middle font-size-20 me-1"
                            >edit</span
                          >Edit</a
                        >
                      </div>
                    </div>
                    <div class="iq-chating-content d-flex align-items-center">
                      <p class="mr-2 mb-0">Thanks, from ThemeForest.</p>
                    </div>
                  </div>
                </div>
              </div>
            </div>
            <div class="card-footer px-3 py-3 border-top rounded-0">
              <form class="d-flex align-items-center" action="#">
                <div class="chat-attagement d-flex">
                  <a href="#" class="d-flex align-items-center pe-3">
                    <svg
                      class="icon-24"
                      width="24"
                      viewBox="0 0 24 25"
                      fill="none"
                      xmlns="http://www.w3.org/2000/svg"
                    >
                      <g clip-path="url(#clip0_156_599)">
                        <path
                          d="M20.4853 4.01473C18.2188 1.74823 15.2053 0.5 12 0.5C8.79469 0.5 5.78119 1.74823 3.51473 4.01473C1.24819 6.28119 0 9.29469 0 12.5C0 15.7053 1.24819 18.7188 3.51473 20.9853C5.78119 23.2518 8.79469 24.5 12 24.5C15.2053 24.5 18.2188 23.2518 20.4853 20.9853C22.7518 18.7188 24 15.7053 24 12.5C24 9.29469 22.7518 6.28119 20.4853 4.01473ZM12 23.0714C6.17091 23.0714 1.42856 18.3291 1.42856 12.5C1.42856 6.67091 6.17091 1.92856 12 1.92856C17.8291 1.92856 22.5714 6.67091 22.5714 12.5C22.5714 18.3291 17.8291 23.0714 12 23.0714Z"
                          fill="currentcolor"
                        ></path>
                        <path
                          d="M9.40398 9.3309C8.23431 8.16114 6.33104 8.16123 5.16136 9.3309C4.88241 9.60981 4.88241 10.0621 5.16136 10.3411C5.44036 10.62 5.89266 10.62 6.17157 10.3411C6.78432 9.72836 7.78126 9.7284 8.39392 10.3411C8.53342 10.4806 8.71618 10.5503 8.89895 10.5503C9.08171 10.5503 9.26457 10.4806 9.40398 10.3411C9.68293 10.0621 9.68293 9.60986 9.40398 9.3309Z"
                          fill="currentcolor"
                        ></path>
                        <path
                          d="M18.8384 9.3309C17.6688 8.16123 15.7655 8.16114 14.5958 9.3309C14.3169 9.60981 14.3169 10.0621 14.5958 10.3411C14.8748 10.62 15.3271 10.62 15.606 10.3411C16.2187 9.72836 17.2156 9.72831 17.8284 10.3411C17.9679 10.4806 18.1506 10.5503 18.3334 10.5503C18.5162 10.5503 18.699 10.4806 18.8384 10.3411C19.1174 10.0621 19.1174 9.60986 18.8384 9.3309Z"
                          fill="currentcolor"
                        ></path>
                        <path
                          d="M18.3335 13.024H5.6668C5.2723 13.024 4.95251 13.3438 4.95251 13.7383C4.95251 17.6243 8.11409 20.7859 12.0001 20.7859C15.8862 20.7859 19.0477 17.6243 19.0477 13.7383C19.0477 13.3438 18.728 13.024 18.3335 13.024ZM12.0001 19.3573C9.14366 19.3573 6.77816 17.215 6.42626 14.4525H17.574C17.2221 17.215 14.8566 19.3573 12.0001 19.3573Z"
                          fill="currentcolor"
                        ></path>
                      </g>
                      <defs>
                        <clipPath>
                          <rect
                            width="24"
                            height="24"
                            fill="white"
                            transform="translate(0 0.5)"
                          ></rect>
                        </clipPath>
                      </defs>
                    </svg>
                  </a>
                  <a href="#" class="d-flex align-items-center pe-3">
                    <svg
                      class="icon-24"
                      width="18"
                      height="23"
                      viewBox="0 0 18 23"
                      fill="none"
                      xmlns="http://www.w3.org/2000/svg"
                    >
                      <path
                        d="M9.00021 21.5V18.3391"
                        stroke="currentcolor"
                        stroke-width="2"
                        stroke-linecap="round"
                        stroke-linejoin="round"
                      ></path>
                      <path
                        fill-rule="evenodd"
                        clip-rule="evenodd"
                        d="M9.00021 14.3481V14.3481C6.75611 14.3481 4.9384 12.5218 4.9384 10.2682V5.58095C4.9384 3.32732 6.75611 1.5 9.00021 1.5C11.2433 1.5 13.061 3.32732 13.061 5.58095V10.2682C13.061 12.5218 11.2433 14.3481 9.00021 14.3481Z"
                        stroke="currentcolor"
                        stroke-width="2"
                        stroke-linecap="round"
                        stroke-linejoin="round"
                      ></path>
                      <path
                        d="M17 10.3006C17 14.7394 13.418 18.3383 9 18.3383C4.58093 18.3383 1 14.7394 1 10.3006"
                        stroke="currentcolor"
                        stroke-width="2"
                        stroke-linecap="round"
                        stroke-linejoin="round"
                      ></path>
                      <path
                        d="M11.0689 6.25579H13.0585"
                        stroke="currentcolor"
                        stroke-width="2"
                        stroke-linecap="round"
                        stroke-linejoin="round"
                      ></path>
                      <path
                        d="M10.0704 9.59344H13.0605"
                        stroke="currentcolor"
                        stroke-width="2"
                        stroke-linecap="round"
                        stroke-linejoin="round"
                      ></path>
                    </svg>
                  </a>
                </div>
                <input
                  type="text"
                  class="form-control me-3"
                  placeholder="Type your message"
                />
                <button
                  type="submit"
                  class="btn btn-primary d-flex align-items-center"
                >
                  <svg
                    class="icon-20"
                    width="18"
                    viewBox="0 0 20 21"
                    fill="none"
                    xmlns="http://www.w3.org/2000/svg"
                  >
                    <path
                      d="M13.8325 6.67463L8.10904 12.4592L1.59944 8.38767C0.66675 7.80414 0.860765 6.38744 1.91572 6.07893L17.3712 1.55277C18.3373 1.26963 19.2326 2.17283 18.9456 3.142L14.3731 18.5868C14.0598 19.6432 12.6512 19.832 12.0732 18.8953L8.10601 12.4602"
                      stroke="currentcolor"
                      stroke-width="2"
                      stroke-linecap="round"
                      stroke-linejoin="round"
                    ></path></svg
                  ><span class="d-none d-lg-block ms-1">Send</span>
                </button>
              </form>
            </div>
          </div>
          <div
            class="card tab-pane mb-0 fade"
            id="user-content-102"
            role="tabpanel"
          >
            <div class="chat-head">
              <header
                class="d-flex justify-content-between align-items-center pt-3 ps-3 pe-3 pb-3"
              >
                <div class="d-flex align-items-center gap-3">
                  <div class="d-block d-xl-none">
                    <button
                      class="btn btn-sm btn-primary rounded btn-icon"
                      data-toggle="sidebar"
                      data-active="true"
                    >
                      <span class="btn-inner">
                        <svg class="icon-rtl" width="20px" viewBox="0 0 24 24">
                          <path
                            fill="currentColor"
                            d="M4,11V13H16L10.5,18.5L11.92,19.92L19.84,12L11.92,4.08L10.5,5.5L16,11H4Z"
                          ></path>
                        </svg>
                      </span>
                    </button>
                  </div>
                  <div class="avatar chat-user-profile m-0">
                    <img
                      src="../../../assets/images/chat/avatar/02.png"
                      alt="avatar"
                      class="avatar-50 rounded-pill"
                      loading="lazy"
                    />
                    <div class="iq-profile-badge bg-success"></div>
                  </div>
                  <div>
                    <h5 class="mb-0">John Travolta</h5>
                    <small class="text-capitalize fw-500">Online</small>
                  </div>
                </div>

                <div class="chat-header-icons d-inline-flex ms-auto">
                  <a
                    href="#"
                    class="chat-icon-phone bg-primary-subtle d-flex align-items-center justify-content-center"
                  >
                    <i class="material-symbols-outlined md-18">phone</i>
                  </a>
                  <a
                    href="#"
                    class="chat-icon-video bg-primary-subtle d-flex align-items-center justify-content-center"
                  >
                    <i class="material-symbols-outlined md-18">videocam</i>
                  </a>
                  <a
                    href="#"
                    class="chat-icon-delete bg-primary-subtle d-flex align-items-center justify-content-center"
                  >
                    <i class="material-symbols-outlined md-18">delete</i>
                  </a>
                  <span
                    class="dropdown bg-primary-subtle d-flex align-items-center justify-content-center"
                  >
                    <svg
                      class="icon-20 nav-hide-arrow cursor-pointer pe-0"
                      id="dropdownMenuButton-08"
                      data-bs-toggle="dropdown"
                      aria-haspopup="true"
                      aria-expanded="false"
                      role="menu"
                      width="24"
                      xmlns="http://www.w3.org/2000/svg"
                      viewBox="0 0 24 24"
                      fill="none"
                    >
                      <path
                        fill-rule="evenodd"
                        clip-rule="evenodd"
                        d="M10 20.6788C10 21.9595 11.0378 23 12.3113 23C13.5868 23 14.6265 21.9595 14.6265 20.6788C14.6265 19.3981 13.5868 18.3576 12.3113 18.3576C11.0378 18.3576 10 19.3981 10 20.6788ZM10 12.0005C10 13.2812 11.0378 14.3217 12.3113 14.3217C13.5868 14.3217 14.6265 13.2812 14.6265 12.0005C14.6265 10.7198 13.5868 9.67929 12.3113 9.67929C11.0378 9.67929 10 10.7198 10 12.0005ZM12.3113 5.64239C11.0378 5.64239 10 4.60192 10 3.3212C10 2.04047 11.0378 1 12.3113 1C13.5868 1 14.6265 2.04047 14.6265 3.3212C14.6265 4.60192 13.5868 5.64239 12.3113 5.64239Z"
                        fill="currentColor"
                      ></path>
                    </svg>
                    <span
                      class="dropdown-menu dropdown-menu-right"
                      aria-labelledby="dropdownMenuButton-08"
                    >
                      <a
                        class="dropdown-item d-flex align-items-center"
                        href="#"
                        ><i class="material-symbols-outlined md-18 me-1"
                          >push_pin</i
                        >Pin to top</a
                      >
                      <a
                        class="dropdown-item d-flex align-items-center"
                        href="#"
                        ><i class="material-symbols-outlined md-18 me-1"
                          >delete_outline</i
                        >Delete chat</a
                      >
                      <a
                        class="dropdown-item d-flex align-items-center"
                        href="#"
                        ><i class="material-symbols-outlined md-18 me-1"
                          >watch_later</i
                        >Block</a
                      >
                    </span>
                  </span>
                </div>
              </header>
            </div>
            <div class="card-body chat-body bg-body">
              <div class="chat-day-title">
                <span class="main-title">Feb 2,2021</span>
              </div>
              <div class="iq-message-body iq-current-user">
                <div class="chat-profile text-center">
                  <img
                    src="../../../assets/images/chat/avatar/10.png"
                    alt="chat-user"
                    class="avatar-40 rounded-pill"
                    loading="lazy"
                  />
                  <small class="iq-chating p-0 mb-0 d-block">16:34</small>
                </div>
                <div class="iq-chat-text">
                  <div
                    class="d-flex align-items-center justify-content-end gap-1 gap-md-2"
                  >
                    <div
                      class="dropdown cursor-pointer more"
                      v-b-tooltip.hover
                      title="More"
                    >
                      <div
                        class="lh-1"
                        id="post-option"
                        data-bs-toggle="dropdown"
                      >
                        <span class="material-symbols-outlined text-dark">
                          more_vert
                        </span>
                      </div>
                      <div
                        class="dropdown-menu dropdown-menu-right"
                        aria-labelledby="post-option"
                        style=""
                      >
                        <a class="dropdown-item" href="#"
                          ><span
                            class="material-symbols-outlined align-middle font-size-20 me-1"
                            >content_copy</span
                          >Copy message text</a
                        >
                        <a class="dropdown-item" href="#">
                          <span
                            class="material-symbols-outlined align-middle font-size-20 me-1"
                            >edit</span
                          >Edit</a
                        >
                      </div>
                    </div>
                    <div class="iq-chating-content d-flex align-items-center">
                      <p class="mr-2 mb-0">
                        How can we help? We're here for you! 😄
                      </p>
                    </div>
                  </div>
                </div>
              </div>
              <div class="iq-message-body iq-other-user">
                <div class="chat-profile text-center">
                  <img
                    src="../../../assets/images/chat/avatar/02.png"
                    alt="chat-user"
                    class="avatar-40 rounded-pill"
                    loading="lazy"
                  />
                  <small class="iq-chating p-0 mb-0 d-block">16:40</small>
                </div>
                <div class="iq-chat-text">
                  <div
                    class="d-flex align-items-center justify-content-start gap-md-2"
                  >
                    <div class="iq-chating-content d-flex align-items-center">
                      <p class="mr-2 mb-0">
                        Hey John, I am looking for the best admin template.
                        Could you please help me to find it out? 🤔
                      </p>
                    </div>
                    <a
                      href="#"
                      class="material-symbols-outlined font-size-20 text-dark reply"
                      v-b-tooltip.hover
                      title="Reply"
                      >reply</a
                    >
                    <div
                      class="dropdown cursor-pointer more"
                      v-b-tooltip.hover
                      title="More"
                    >
                      <div
                        class="lh-1"
                        id="post-option"
                        data-bs-toggle="dropdown"
                      >
                        <span class="material-symbols-outlined text-dark">
                          more_vert
                        </span>
                      </div>
                      <div
                        class="dropdown-menu dropdown-menu-right"
                        aria-labelledby="post-option"
                        style=""
                      >
                        <a class="dropdown-item" href="#"
                          ><span
                            class="material-symbols-outlined align-middle font-size-20 me-1"
                            >content_copy</span
                          >Copy message text</a
                        >
                        <a class="dropdown-item" href="#">
                          <span
                            class="material-symbols-outlined align-middle font-size-20 me-1"
                            >edit</span
                          >Edit</a
                        >
                      </div>
                    </div>
                  </div>
                </div>
              </div>
              <div class="iq-message-body iq-current-user">
                <div class="chat-profile text-center">
                  <img
                    src="../../../assets/images/chat/avatar/10.png"
                    alt="chat-user"
                    class="avatar-40 rounded-pill"
                    loading="lazy"
                  />
                  <small class="iq-chating p-0 mb-0 d-block">6:49</small>
                </div>
                <div class="iq-chat-text">
                  <div
                    class="d-flex align-items-center justify-content-end gap-1 gap-md-2"
                  >
                    <div
                      class="dropdown cursor-pointer more"
                      v-b-tooltip.hover
                      title="More"
                    >
                      <div
                        class="lh-1"
                        id="post-option"
                        data-bs-toggle="dropdown"
                      >
                        <span class="material-symbols-outlined text-dark">
                          more_vert
                        </span>
                      </div>
                      <div
                        class="dropdown-menu dropdown-menu-right"
                        aria-labelledby="post-option"
                        style=""
                      >
                        <a class="dropdown-item" href="#"
                          ><span
                            class="material-symbols-outlined align-middle font-size-20 me-1"
                            >content_copy</span
                          >Copy message text</a
                        >
                        <a class="dropdown-item" href="#">
                          <span
                            class="material-symbols-outlined align-middle font-size-20 me-1"
                            >edit</span
                          >Edit</a
                        >
                      </div>
                    </div>
                    <div class="iq-chating-content d-flex align-items-center">
                      <p class="mr-2 mb-0">
                        Absolutely Yes! SocialV is the Responsive Bootstrap 5
                        Admin Dashboard Template.
                      </p>
                    </div>
                  </div>
                </div>
              </div>
              <div class="iq-message-body iq-other-user">
                <div class="chat-profile text-center">
                  <img
                    src="../../../assets/images/chat/avatar/02.png"
                    alt="chat-user"
                    class="avatar-40 rounded-pill"
                    loading="lazy"
                  />
                  <small class="iq-chating p-0 mb-0 d-block">6:52</small>
                </div>
                <div class="iq-chat-text">
                  <div
                    class="d-flex align-items-center justify-content-start gap-md-2"
                  >
                    <div class="iq-chating-content d-flex align-items-center">
                      <p class="mr-2 mb-0">Looks clean and fresh UI.</p>
                    </div>
                    <a
                      href="#"
                      class="material-symbols-outlined font-size-20 text-dark reply"
                      v-b-tooltip.hover
                      title="Reply"
                      >reply</a
                    >
                    <div
                      class="dropdown cursor-pointer more"
                      v-b-tooltip.hover
                      title="More"
                    >
                      <div
                        class="lh-1"
                        id="post-option"
                        data-bs-toggle="dropdown"
                      >
                        <span class="material-symbols-outlined text-dark">
                          more_vert
                        </span>
                      </div>
                      <div
                        class="dropdown-menu dropdown-menu-right"
                        aria-labelledby="post-option"
                        style=""
                      >
                        <a class="dropdown-item" href="#"
                          ><span
                            class="material-symbols-outlined align-middle font-size-20 me-1"
                            >content_copy</span
                          >Copy message text</a
                        >
                        <a class="dropdown-item" href="#">
                          <span
                            class="material-symbols-outlined align-middle font-size-20 me-1"
                            >edit</span
                          >Edit</a
                        >
                      </div>
                    </div>
                  </div>
                </div>
              </div>
              <div class="iq-message-body iq-current-user">
                <div class="chat-profile text-center">
                  <img
                    src="../../../assets/images/chat/avatar/10.png"
                    alt="chat-user"
                    class="avatar-40 rounded-pill"
                    loading="lazy"
                  />
                  <small class="iq-chating p-0 mb-0 d-block">6:53</small>
                </div>
                <div class="iq-chat-text">
                  <div
                    class="d-flex align-items-center justify-content-end gap-1 gap-md-2"
                  >
                    <div
                      class="dropdown cursor-pointer more"
                      v-b-tooltip.hover
                      title="More"
                    >
                      <div
                        class="lh-1"
                        id="post-option"
                        data-bs-toggle="dropdown"
                      >
                        <span class="material-symbols-outlined text-dark">
                          more_vert
                        </span>
                      </div>
                      <div
                        class="dropdown-menu dropdown-menu-right"
                        aria-labelledby="post-option"
                        style=""
                      >
                        <a class="dropdown-item" href="#"
                          ><span
                            class="material-symbols-outlined align-middle font-size-20 me-1"
                            >content_copy</span
                          >Copy message text</a
                        >
                        <a class="dropdown-item" href="#">
                          <span
                            class="material-symbols-outlined align-middle font-size-20 me-1"
                            >edit</span
                          >Edit</a
                        >
                      </div>
                    </div>
                    <div class="iq-chating-content d-flex align-items-center">
                      <p class="mr-2 mb-0">Thanks, from ThemeForest.</p>
                    </div>
                  </div>
                </div>
              </div>
              <div class="iq-message-body iq-other-user">
                <div class="chat-profile text-center">
                  <img
                    src="../../../assets/images/chat/avatar/02.png"
                    alt="chat-user"
                    class="avatar-40 rounded-pill"
                    loading="lazy"
                  />
                  <small class="iq-chating p-0 mb-0 d-block">6:54</small>
                </div>
                <div class="iq-chat-text">
                  <div
                    class="d-flex align-items-center justify-content-start gap-md-2"
                  >
                    <div class="iq-chating-content d-flex align-items-center">
                      <p class="mr-2 mb-0">I will purchase it for sure. 👍</p>
                    </div>
                    <a
                      href="#"
                      class="material-symbols-outlined font-size-20 text-dark reply"
                      v-b-tooltip.hover
                      title="Reply"
                      >reply</a
                    >
                    <div
                      class="dropdown cursor-pointer more"
                      v-b-tooltip.hover
                      title="More"
                    >
                      <div
                        class="lh-1"
                        id="post-option"
                        data-bs-toggle="dropdown"
                      >
                        <span class="material-symbols-outlined text-dark">
                          more_vert
                        </span>
                      </div>
                      <div
                        class="dropdown-menu dropdown-menu-right"
                        aria-labelledby="post-option"
                        style=""
                      >
                        <a class="dropdown-item" href="#"
                          ><span
                            class="material-symbols-outlined align-middle font-size-20 me-1"
                            >content_copy</span
                          >Copy message text</a
                        >
                        <a class="dropdown-item" href="#">
                          <span
                            class="material-symbols-outlined align-middle font-size-20 me-1"
                            >edit</span
                          >Edit</a
                        >
                      </div>
                    </div>
                  </div>
                </div>
              </div>
              <div class="iq-message-body iq-current-user">
                <div class="chat-profile text-center">
                  <img
                    src="../../../assets/images/chat/avatar/10.png"
                    alt="chat-user"
                    class="avatar-40 rounded-pill"
                    loading="lazy"
                  />
                  <small class="iq-chating p-0 mb-0 d-block">6:55</small>
                </div>
                <div class="iq-chat-text">
                  <div
                    class="d-flex align-items-center justify-content-end gap-1 gap-md-2"
                  >
                    <div
                      class="dropdown cursor-pointer more"
                      v-b-tooltip.hover
                      title="More"
                    >
                      <div
                        class="lh-1"
                        id="post-option"
                        data-bs-toggle="dropdown"
                      >
                        <span class="material-symbols-outlined text-dark">
                          more_vert
                        </span>
                      </div>
                      <div
                        class="dropdown-menu dropdown-menu-right"
                        aria-labelledby="post-option"
                        style=""
                      >
                        <a class="dropdown-item" href="#"
                          ><span
                            class="material-symbols-outlined align-middle font-size-20 me-1"
                            >content_copy</span
                          >Copy message text</a
                        >
                        <a class="dropdown-item" href="#">
                          <span
                            class="material-symbols-outlined align-middle font-size-20 me-1"
                            >edit</span
                          >Edit</a
                        >
                      </div>
                    </div>
                    <div class="iq-chating-content d-flex align-items-center">
                      <p class="mr-2 mb-0">Okay Thanks..</p>
                    </div>
                  </div>
                </div>
              </div>
              <div class="iq-message-body iq-other-user">
                <div class="chat-profile text-center">
                  <img
                    src="../../../assets/images/chat/avatar/02.png"
                    alt="chat-user"
                    class="avatar-40 rounded-pill"
                    loading="lazy"
                  />
                  <small class="iq-chating p-0 mb-0 d-block">7:54</small>
                </div>
                <div class="iq-chat-text">
                  <div
                    class="d-flex align-items-center justify-content-start gap-md-2"
                  >
                    <div class="iq-chating-content d-flex align-items-center">
                      <p class="mr-2 mb-0">Looks clean and fresh UI.</p>
                    </div>
                    <a
                      href="#"
                      class="material-symbols-outlined font-size-20 text-dark reply"
                      v-b-tooltip.hover
                      title="Reply"
                      >reply</a
                    >
                    <div
                      class="dropdown cursor-pointer more"
                      v-b-tooltip.hover
                      title="More"
                    >
                      <div
                        class="lh-1"
                        id="post-option"
                        data-bs-toggle="dropdown"
                      >
                        <span class="material-symbols-outlined text-dark">
                          more_vert
                        </span>
                      </div>
                      <div
                        class="dropdown-menu dropdown-menu-right"
                        aria-labelledby="post-option"
                        style=""
                      >
                        <a class="dropdown-item" href="#"
                          ><span
                            class="material-symbols-outlined align-middle font-size-20 me-1"
                            >content_copy</span
                          >Copy message text</a
                        >
                        <a class="dropdown-item" href="#">
                          <span
                            class="material-symbols-outlined align-middle font-size-20 me-1"
                            >edit</span
                          >Edit</a
                        >
                      </div>
                    </div>
                  </div>
                </div>
              </div>
              <div class="iq-message-body iq-current-user">
                <div class="chat-profile text-center">
                  <img
                    src="../../../assets/images/chat/avatar/10.png"
                    alt="chat-user"
                    class="avatar-40 rounded-pill"
                    loading="lazy"
                  />
                  <small class="iq-chating p-0 mb-0 d-block">7:55</small>
                </div>
                <div class="iq-chat-text">
                  <div
                    class="d-flex align-items-center justify-content-end gap-1 gap-md-2"
                  >
                    <div
                      class="dropdown cursor-pointer more"
                      v-b-tooltip.hover
                      title="More"
                    >
                      <div
                        class="lh-1"
                        id="post-option"
                        data-bs-toggle="dropdown"
                      >
                        <span class="material-symbols-outlined text-dark">
                          more_vert
                        </span>
                      </div>
                      <div
                        class="dropdown-menu dropdown-menu-right"
                        aria-labelledby="post-option"
                        style=""
                      >
                        <a class="dropdown-item" href="#"
                          ><span
                            class="material-symbols-outlined align-middle font-size-20 me-1"
                            >content_copy</span
                          >Copy message text</a
                        >
                        <a class="dropdown-item" href="#">
                          <span
                            class="material-symbols-outlined align-middle font-size-20 me-1"
                            >edit</span
                          >Edit</a
                        >
                      </div>
                    </div>
                    <div class="iq-chating-content d-flex align-items-center">
                      <p class="mr-2 mb-0">Thanks, from ThemeForest.</p>
                    </div>
                  </div>
                </div>
              </div>
            </div>
            <div class="card-footer px-3 py-3 border-top rounded-0">
              <form class="d-flex align-items-center" action="#">
                <div class="chat-attagement d-flex">
                  <a href="#" class="d-flex align-items-center pe-3">
                    <svg
                      class="icon-24"
                      width="24"
                      viewBox="0 0 24 25"
                      fill="none"
                      xmlns="http://www.w3.org/2000/svg"
                    >
                      <g clip-path="url(#clip0_156_599)">
                        <path
                          d="M20.4853 4.01473C18.2188 1.74823 15.2053 0.5 12 0.5C8.79469 0.5 5.78119 1.74823 3.51473 4.01473C1.24819 6.28119 0 9.29469 0 12.5C0 15.7053 1.24819 18.7188 3.51473 20.9853C5.78119 23.2518 8.79469 24.5 12 24.5C15.2053 24.5 18.2188 23.2518 20.4853 20.9853C22.7518 18.7188 24 15.7053 24 12.5C24 9.29469 22.7518 6.28119 20.4853 4.01473ZM12 23.0714C6.17091 23.0714 1.42856 18.3291 1.42856 12.5C1.42856 6.67091 6.17091 1.92856 12 1.92856C17.8291 1.92856 22.5714 6.67091 22.5714 12.5C22.5714 18.3291 17.8291 23.0714 12 23.0714Z"
                          fill="currentcolor"
                        ></path>
                        <path
                          d="M9.40398 9.3309C8.23431 8.16114 6.33104 8.16123 5.16136 9.3309C4.88241 9.60981 4.88241 10.0621 5.16136 10.3411C5.44036 10.62 5.89266 10.62 6.17157 10.3411C6.78432 9.72836 7.78126 9.7284 8.39392 10.3411C8.53342 10.4806 8.71618 10.5503 8.89895 10.5503C9.08171 10.5503 9.26457 10.4806 9.40398 10.3411C9.68293 10.0621 9.68293 9.60986 9.40398 9.3309Z"
                          fill="currentcolor"
                        ></path>
                        <path
                          d="M18.8384 9.3309C17.6688 8.16123 15.7655 8.16114 14.5958 9.3309C14.3169 9.60981 14.3169 10.0621 14.5958 10.3411C14.8748 10.62 15.3271 10.62 15.606 10.3411C16.2187 9.72836 17.2156 9.72831 17.8284 10.3411C17.9679 10.4806 18.1506 10.5503 18.3334 10.5503C18.5162 10.5503 18.699 10.4806 18.8384 10.3411C19.1174 10.0621 19.1174 9.60986 18.8384 9.3309Z"
                          fill="currentcolor"
                        ></path>
                        <path
                          d="M18.3335 13.024H5.6668C5.2723 13.024 4.95251 13.3438 4.95251 13.7383C4.95251 17.6243 8.11409 20.7859 12.0001 20.7859C15.8862 20.7859 19.0477 17.6243 19.0477 13.7383C19.0477 13.3438 18.728 13.024 18.3335 13.024ZM12.0001 19.3573C9.14366 19.3573 6.77816 17.215 6.42626 14.4525H17.574C17.2221 17.215 14.8566 19.3573 12.0001 19.3573Z"
                          fill="currentcolor"
                        ></path>
                      </g>
                      <defs>
                        <clipPath>
                          <rect
                            width="24"
                            height="24"
                            fill="white"
                            transform="translate(0 0.5)"
                          ></rect>
                        </clipPath>
                      </defs>
                    </svg>
                  </a>
                  <a href="#" class="d-flex align-items-center pe-3">
                    <svg
                      class="icon-24"
                      width="18"
                      height="23"
                      viewBox="0 0 18 23"
                      fill="none"
                      xmlns="http://www.w3.org/2000/svg"
                    >
                      <path
                        d="M9.00021 21.5V18.3391"
                        stroke="currentcolor"
                        stroke-width="2"
                        stroke-linecap="round"
                        stroke-linejoin="round"
                      ></path>
                      <path
                        fill-rule="evenodd"
                        clip-rule="evenodd"
                        d="M9.00021 14.3481V14.3481C6.75611 14.3481 4.9384 12.5218 4.9384 10.2682V5.58095C4.9384 3.32732 6.75611 1.5 9.00021 1.5C11.2433 1.5 13.061 3.32732 13.061 5.58095V10.2682C13.061 12.5218 11.2433 14.3481 9.00021 14.3481Z"
                        stroke="currentcolor"
                        stroke-width="2"
                        stroke-linecap="round"
                        stroke-linejoin="round"
                      ></path>
                      <path
                        d="M17 10.3006C17 14.7394 13.418 18.3383 9 18.3383C4.58093 18.3383 1 14.7394 1 10.3006"
                        stroke="currentcolor"
                        stroke-width="2"
                        stroke-linecap="round"
                        stroke-linejoin="round"
                      ></path>
                      <path
                        d="M11.0689 6.25579H13.0585"
                        stroke="currentcolor"
                        stroke-width="2"
                        stroke-linecap="round"
                        stroke-linejoin="round"
                      ></path>
                      <path
                        d="M10.0704 9.59344H13.0605"
                        stroke="currentcolor"
                        stroke-width="2"
                        stroke-linecap="round"
                        stroke-linejoin="round"
                      ></path>
                    </svg>
                  </a>
                </div>
                <input
                  type="text"
                  class="form-control me-3"
                  placeholder="Type your message"
                />
                <button
                  type="submit"
                  class="btn btn-primary d-flex align-items-center"
                >
                  <svg
                    class="icon-20"
                    width="18"
                    viewBox="0 0 20 21"
                    fill="none"
                    xmlns="http://www.w3.org/2000/svg"
                  >
                    <path
                      d="M13.8325 6.67463L8.10904 12.4592L1.59944 8.38767C0.66675 7.80414 0.860765 6.38744 1.91572 6.07893L17.3712 1.55277C18.3373 1.26963 19.2326 2.17283 18.9456 3.142L14.3731 18.5868C14.0598 19.6432 12.6512 19.832 12.0732 18.8953L8.10601 12.4602"
                      stroke="currentcolor"
                      stroke-width="2"
                      stroke-linecap="round"
                      stroke-linejoin="round"
                    ></path></svg
                  ><span class="d-none d-lg-block ms-1">Send</span>
                </button>
              </form>
            </div>
          </div>
          <div
            class="card tab-pane mb-0 fade"
            id="user-content-103"
            role="tabpanel"
          >
            <div class="chat-head">
              <header
                class="d-flex justify-content-between align-items-center pt-3 ps-3 pe-3 pb-3"
              >
                <div class="d-flex align-items-center gap-3">
                  <div class="d-block d-xl-none">
                    <button
                      class="btn btn-sm btn-primary rounded btn-icon"
                      data-toggle="sidebar"
                      data-active="true"
                    >
                      <span class="btn-inner">
                        <svg class="icon-rtl" width="20px" viewBox="0 0 24 24">
                          <path
                            fill="currentColor"
                            d="M4,11V13H16L10.5,18.5L11.92,19.92L19.84,12L11.92,4.08L10.5,5.5L16,11H4Z"
                          ></path>
                        </svg>
                      </span>
                    </button>
                  </div>
                  <div class="avatar chat-user-profile m-0">
                    <img
                      src="../../../assets/images/chat/avatar/03.png"
                      alt="avatar"
                      class="avatar-50 rounded-pill"
                      loading="lazy"
                    />
                    <div class="iq-profile-badge bg-success"></div>
                  </div>
                  <div>
                    <h5 class="mb-0">Barb Ackue</h5>
                    <small class="text-capitalize fw-500">Online</small>
                  </div>
                </div>

                <div class="chat-header-icons d-inline-flex ms-auto">
                  <a
                    href="#"
                    class="chat-icon-phone bg-primary-subtle d-flex align-items-center justify-content-center"
                  >
                    <i class="material-symbols-outlined md-18">phone</i>
                  </a>
                  <a
                    href="#"
                    class="chat-icon-video bg-primary-subtle d-flex align-items-center justify-content-center"
                  >
                    <i class="material-symbols-outlined md-18">videocam</i>
                  </a>
                  <a
                    href="#"
                    class="chat-icon-delete bg-primary-subtle d-flex align-items-center justify-content-center"
                  >
                    <i class="material-symbols-outlined md-18">delete</i>
                  </a>
                  <span
                    class="dropdown bg-primary-subtle d-flex align-items-center justify-content-center"
                  >
                    <svg
                      class="icon-20 nav-hide-arrow cursor-pointer pe-0"
                      id="dropdownMenuButton-10"
                      data-bs-toggle="dropdown"
                      aria-haspopup="true"
                      aria-expanded="false"
                      role="menu"
                      width="24"
                      xmlns="http://www.w3.org/2000/svg"
                      viewBox="0 0 24 24"
                      fill="none"
                    >
                      <path
                        fill-rule="evenodd"
                        clip-rule="evenodd"
                        d="M10 20.6788C10 21.9595 11.0378 23 12.3113 23C13.5868 23 14.6265 21.9595 14.6265 20.6788C14.6265 19.3981 13.5868 18.3576 12.3113 18.3576C11.0378 18.3576 10 19.3981 10 20.6788ZM10 12.0005C10 13.2812 11.0378 14.3217 12.3113 14.3217C13.5868 14.3217 14.6265 13.2812 14.6265 12.0005C14.6265 10.7198 13.5868 9.67929 12.3113 9.67929C11.0378 9.67929 10 10.7198 10 12.0005ZM12.3113 5.64239C11.0378 5.64239 10 4.60192 10 3.3212C10 2.04047 11.0378 1 12.3113 1C13.5868 1 14.6265 2.04047 14.6265 3.3212C14.6265 4.60192 13.5868 5.64239 12.3113 5.64239Z"
                        fill="currentColor"
                      ></path>
                    </svg>
                    <span
                      class="dropdown-menu dropdown-menu-right"
                      aria-labelledby="dropdownMenuButton-10"
                    >
                      <a
                        class="dropdown-item d-flex align-items-center"
                        href="#"
                        ><i class="material-symbols-outlined md-18 me-1"
                          >push_pin</i
                        >Pin to top</a
                      >
                      <a
                        class="dropdown-item d-flex align-items-center"
                        href="#"
                        ><i class="material-symbols-outlined md-18 me-1"
                          >delete_outline</i
                        >Delete chat</a
                      >
                      <a
                        class="dropdown-item d-flex align-items-center"
                        href="#"
                        ><i class="material-symbols-outlined md-18 me-1"
                          >watch_later</i
                        >Block</a
                      >
                    </span>
                  </span>
                </div>
              </header>
            </div>
            <div class="card-body chat-body bg-body">
              <div class="chat-day-title">
                <span class="main-title">Feb 1,2021</span>
              </div>
              <div class="iq-message-body iq-current-user">
                <div class="chat-profile text-center">
                  <img
                    src="../../../assets/images/chat/avatar/10.png"
                    alt="chat-user"
                    class="avatar-40 rounded-pill"
                    loading="lazy"
                  />
                  <small class="iq-chating p-0 mb-0 d-block">16:34</small>
                </div>
                <div class="iq-chat-text">
                  <div
                    class="d-flex align-items-center justify-content-end gap-1 gap-md-2"
                  >
                    <div
                      class="dropdown cursor-pointer more"
                      v-b-tooltip.hover
                      title="More"
                    >
                      <div
                        class="lh-1"
                        id="post-option"
                        data-bs-toggle="dropdown"
                      >
                        <span class="material-symbols-outlined text-dark">
                          more_vert
                        </span>
                      </div>
                      <div
                        class="dropdown-menu dropdown-menu-right"
                        aria-labelledby="post-option"
                        style=""
                      >
                        <a class="dropdown-item" href="#"
                          ><span
                            class="material-symbols-outlined align-middle font-size-20 me-1"
                            >content_copy</span
                          >Copy message text</a
                        >
                        <a class="dropdown-item" href="#">
                          <span
                            class="material-symbols-outlined align-middle font-size-20 me-1"
                            >edit</span
                          >Edit</a
                        >
                      </div>
                    </div>
                    <div class="iq-chating-content d-flex align-items-center">
                      <p class="mr-2 mb-0">
                        How can we help? We're here for you! 😄
                      </p>
                    </div>
                  </div>
                </div>
              </div>
              <div class="iq-message-body iq-other-user">
                <div class="chat-profile text-center">
                  <img
                    src="../../../assets/images/chat/avatar/03.png"
                    alt="chat-user"
                    class="avatar-40 rounded-pill"
                    loading="lazy"
                  />
                  <small class="iq-chating p-0 mb-0 d-block">16:40</small>
                </div>
                <div class="iq-chat-text">
                  <div
                    class="d-flex align-items-center justify-content-start gap-md-2"
                  >
                    <div class="iq-chating-content d-flex align-items-center">
                      <p class="mr-2 mb-0">
                        Hey John, I am looking for the best admin template.
                        Could you please help me to find it out? 🤔
                      </p>
                    </div>
                    <a
                      href="#"
                      class="material-symbols-outlined font-size-20 text-dark reply"
                      v-b-tooltip.hover
                      title="Reply"
                      >reply</a
                    >
                    <div
                      class="dropdown cursor-pointer more"
                      v-b-tooltip.hover
                      title="More"
                    >
                      <div
                        class="lh-1"
                        id="post-option"
                        data-bs-toggle="dropdown"
                      >
                        <span class="material-symbols-outlined text-dark">
                          more_vert
                        </span>
                      </div>
                      <div
                        class="dropdown-menu dropdown-menu-right"
                        aria-labelledby="post-option"
                        style=""
                      >
                        <a class="dropdown-item" href="#"
                          ><span
                            class="material-symbols-outlined align-middle font-size-20 me-1"
                            >content_copy</span
                          >Copy message text</a
                        >
                        <a class="dropdown-item" href="#">
                          <span
                            class="material-symbols-outlined align-middle font-size-20 me-1"
                            >edit</span
                          >Edit</a
                        >
                      </div>
                    </div>
                  </div>
                </div>
              </div>
              <div class="iq-message-body iq-current-user">
                <div class="chat-profile text-center">
                  <img
                    src="../../../assets/images/chat/avatar/10.png"
                    alt="chat-user"
                    class="avatar-40 rounded-pill"
                    loading="lazy"
                  />
                  <small class="iq-chating p-0 mb-0 d-block">6:49</small>
                </div>
                <div class="iq-chat-text">
                  <div
                    class="d-flex align-items-center justify-content-end gap-1 gap-md-2"
                  >
                    <div
                      class="dropdown cursor-pointer more"
                      v-b-tooltip.hover
                      title="More"
                    >
                      <div
                        class="lh-1"
                        id="post-option"
                        data-bs-toggle="dropdown"
                      >
                        <span class="material-symbols-outlined text-dark">
                          more_vert
                        </span>
                      </div>
                      <div
                        class="dropdown-menu dropdown-menu-right"
                        aria-labelledby="post-option"
                        style=""
                      >
                        <a class="dropdown-item" href="#"
                          ><span
                            class="material-symbols-outlined align-middle font-size-20 me-1"
                            >content_copy</span
                          >Copy message text</a
                        >
                        <a class="dropdown-item" href="#">
                          <span
                            class="material-symbols-outlined align-middle font-size-20 me-1"
                            >edit</span
                          >Edit</a
                        >
                      </div>
                    </div>
                    <div class="iq-chating-content d-flex align-items-center">
                      <p class="mr-2 mb-0">
                        Absolutely Yes! SocialV is the Responsive Bootstrap 5
                        Admin Dashboard Template.
                      </p>
                    </div>
                  </div>
                </div>
              </div>
              <div class="iq-message-body iq-other-user">
                <div class="chat-profile text-center">
                  <img
                    src="../../../assets/images/chat/avatar/03.png"
                    alt="chat-user"
                    class="avatar-40 rounded-pill"
                    loading="lazy"
                  />
                  <small class="iq-chating p-0 mb-0 d-block">6:52</small>
                </div>
                <div class="iq-chat-text">
                  <div
                    class="d-flex align-items-center justify-content-start gap-md-2"
                  >
                    <div class="iq-chating-content d-flex align-items-center">
                      <p class="mr-2 mb-0">Looks clean and fresh UI.</p>
                    </div>
                    <a
                      href="#"
                      class="material-symbols-outlined font-size-20 text-dark reply"
                      v-b-tooltip.hover
                      title="Reply"
                      >reply</a
                    >
                    <div
                      class="dropdown cursor-pointer more"
                      v-b-tooltip.hover
                      title="More"
                    >
                      <div
                        class="lh-1"
                        id="post-option"
                        data-bs-toggle="dropdown"
                      >
                        <span class="material-symbols-outlined text-dark">
                          more_vert
                        </span>
                      </div>
                      <div
                        class="dropdown-menu dropdown-menu-right"
                        aria-labelledby="post-option"
                        style=""
                      >
                        <a class="dropdown-item" href="#"
                          ><span
                            class="material-symbols-outlined align-middle font-size-20 me-1"
                            >content_copy</span
                          >Copy message text</a
                        >
                        <a class="dropdown-item" href="#">
                          <span
                            class="material-symbols-outlined align-middle font-size-20 me-1"
                            >edit</span
                          >Edit</a
                        >
                      </div>
                    </div>
                  </div>
                </div>
              </div>
              <div class="iq-message-body iq-current-user">
                <div class="chat-profile text-center">
                  <img
                    src="../../../assets/images/chat/avatar/10.png"
                    alt="chat-user"
                    class="avatar-40 rounded-pill"
                    loading="lazy"
                  />
                  <small class="iq-chating p-0 mb-0 d-block">6:53</small>
                </div>
                <div class="iq-chat-text">
                  <div
                    class="d-flex align-items-center justify-content-end gap-1 gap-md-2"
                  >
                    <div
                      class="dropdown cursor-pointer more"
                      v-b-tooltip.hover
                      title="More"
                    >
                      <div
                        class="lh-1"
                        id="post-option"
                        data-bs-toggle="dropdown"
                      >
                        <span class="material-symbols-outlined text-dark">
                          more_vert
                        </span>
                      </div>
                      <div
                        class="dropdown-menu dropdown-menu-right"
                        aria-labelledby="post-option"
                        style=""
                      >
                        <a class="dropdown-item" href="#"
                          ><span
                            class="material-symbols-outlined align-middle font-size-20 me-1"
                            >content_copy</span
                          >Copy message text</a
                        >
                        <a class="dropdown-item" href="#">
                          <span
                            class="material-symbols-outlined align-middle font-size-20 me-1"
                            >edit</span
                          >Edit</a
                        >
                      </div>
                    </div>
                    <div class="iq-chating-content d-flex align-items-center">
                      <p class="mr-2 mb-0">Thanks, from ThemeForest.</p>
                    </div>
                  </div>
                </div>
              </div>
              <div class="iq-message-body iq-other-user">
                <div class="chat-profile text-center">
                  <img
                    src="../../../assets/images/chat/avatar/03.png"
                    alt="chat-user"
                    class="avatar-40 rounded-pill"
                    loading="lazy"
                  />
                  <small class="iq-chating p-0 mb-0 d-block">6:54</small>
                </div>
                <div class="iq-chat-text">
                  <div
                    class="d-flex align-items-center justify-content-start gap-md-2"
                  >
                    <div class="iq-chating-content d-flex align-items-center">
                      <p class="mr-2 mb-0">I will purchase it for sure. 👍</p>
                    </div>
                    <a
                      href="#"
                      class="material-symbols-outlined font-size-20 text-dark reply"
                      v-b-tooltip.hover
                      title="Reply"
                      >reply</a
                    >
                    <div
                      class="dropdown cursor-pointer more"
                      v-b-tooltip.hover
                      title="More"
                    >
                      <div
                        class="lh-1"
                        id="post-option"
                        data-bs-toggle="dropdown"
                      >
                        <span class="material-symbols-outlined text-dark">
                          more_vert
                        </span>
                      </div>
                      <div
                        class="dropdown-menu dropdown-menu-right"
                        aria-labelledby="post-option"
                        style=""
                      >
                        <a class="dropdown-item" href="#"
                          ><span
                            class="material-symbols-outlined align-middle font-size-20 me-1"
                            >content_copy</span
                          >Copy message text</a
                        >
                        <a class="dropdown-item" href="#">
                          <span
                            class="material-symbols-outlined align-middle font-size-20 me-1"
                            >edit</span
                          >Edit</a
                        >
                      </div>
                    </div>
                  </div>
                </div>
              </div>
              <div class="iq-message-body iq-current-user">
                <div class="chat-profile text-center">
                  <img
                    src="../../../assets/images/chat/avatar/10.png"
                    alt="chat-user"
                    class="avatar-40 rounded-pill"
                    loading="lazy"
                  />
                  <small class="iq-chating p-0 mb-0 d-block">6:55</small>
                </div>
                <div class="iq-chat-text">
                  <div
                    class="d-flex align-items-center justify-content-end gap-1 gap-md-2"
                  >
                    <div
                      class="dropdown cursor-pointer more"
                      v-b-tooltip.hover
                      title="More"
                    >
                      <div
                        class="lh-1"
                        id="post-option"
                        data-bs-toggle="dropdown"
                      >
                        <span class="material-symbols-outlined text-dark">
                          more_vert
                        </span>
                      </div>
                      <div
                        class="dropdown-menu dropdown-menu-right"
                        aria-labelledby="post-option"
                        style=""
                      >
                        <a class="dropdown-item" href="#"
                          ><span
                            class="material-symbols-outlined align-middle font-size-20 me-1"
                            >content_copy</span
                          >Copy message text</a
                        >
                        <a class="dropdown-item" href="#">
                          <span
                            class="material-symbols-outlined align-middle font-size-20 me-1"
                            >edit</span
                          >Edit</a
                        >
                      </div>
                    </div>
                    <div class="iq-chating-content d-flex align-items-center">
                      <p class="mr-2 mb-0">Okay Thanks..</p>
                    </div>
                  </div>
                </div>
              </div>
              <div class="iq-message-body iq-other-user">
                <div class="chat-profile text-center">
                  <img
                    src="../../../assets/images/chat/avatar/03.png"
                    alt="chat-user"
                    class="avatar-40 rounded-pill"
                    loading="lazy"
                  />
                  <small class="iq-chating p-0 mb-0 d-block">7:54</small>
                </div>
                <div class="iq-chat-text">
                  <div
                    class="d-flex align-items-center justify-content-start gap-md-2"
                  >
                    <div class="iq-chating-content d-flex align-items-center">
                      <p class="mr-2 mb-0">Looks clean and fresh UI.</p>
                    </div>
                    <a
                      href="#"
                      class="material-symbols-outlined font-size-20 text-dark reply"
                      v-b-tooltip.hover
                      title="Reply"
                      >reply</a
                    >
                    <div
                      class="dropdown cursor-pointer more"
                      v-b-tooltip.hover
                      title="More"
                    >
                      <div
                        class="lh-1"
                        id="post-option"
                        data-bs-toggle="dropdown"
                      >
                        <span class="material-symbols-outlined text-dark">
                          more_vert
                        </span>
                      </div>
                      <div
                        class="dropdown-menu dropdown-menu-right"
                        aria-labelledby="post-option"
                        style=""
                      >
                        <a class="dropdown-item" href="#"
                          ><span
                            class="material-symbols-outlined align-middle font-size-20 me-1"
                            >content_copy</span
                          >Copy message text</a
                        >
                        <a class="dropdown-item" href="#">
                          <span
                            class="material-symbols-outlined align-middle font-size-20 me-1"
                            >edit</span
                          >Edit</a
                        >
                      </div>
                    </div>
                  </div>
                </div>
              </div>
              <div class="iq-message-body iq-current-user">
                <div class="chat-profile text-center">
                  <img
                    src="../../../assets/images/chat/avatar/10.png"
                    alt="chat-user"
                    class="avatar-40 rounded-pill"
                    loading="lazy"
                  />
                  <small class="iq-chating p-0 mb-0 d-block">7:55</small>
                </div>
                <div class="iq-chat-text">
                  <div
                    class="d-flex align-items-center justify-content-end gap-1 gap-md-2"
                  >
                    <div
                      class="dropdown cursor-pointer more"
                      v-b-tooltip.hover
                      title="More"
                    >
                      <div
                        class="lh-1"
                        id="post-option"
                        data-bs-toggle="dropdown"
                      >
                        <span class="material-symbols-outlined text-dark">
                          more_vert
                        </span>
                      </div>
                      <div
                        class="dropdown-menu dropdown-menu-right"
                        aria-labelledby="post-option"
                        style=""
                      >
                        <a class="dropdown-item" href="#"
                          ><span
                            class="material-symbols-outlined align-middle font-size-20 me-1"
                            >content_copy</span
                          >Copy message text</a
                        >
                        <a class="dropdown-item" href="#">
                          <span
                            class="material-symbols-outlined align-middle font-size-20 me-1"
                            >edit</span
                          >Edit</a
                        >
                      </div>
                    </div>
                    <div class="iq-chating-content d-flex align-items-center">
                      <p class="mr-2 mb-0">Thanks, from ThemeForest.</p>
                    </div>
                  </div>
                </div>
              </div>
            </div>
            <div class="card-footer px-3 py-3 border-top rounded-0">
              <form class="d-flex align-items-center" action="#">
                <div class="chat-attagement d-flex">
                  <a href="#" class="d-flex align-items-center pe-3">
                    <svg
                      class="icon-24"
                      width="24"
                      viewBox="0 0 24 25"
                      fill="none"
                      xmlns="http://www.w3.org/2000/svg"
                    >
                      <g clip-path="url(#clip0_156_599)">
                        <path
                          d="M20.4853 4.01473C18.2188 1.74823 15.2053 0.5 12 0.5C8.79469 0.5 5.78119 1.74823 3.51473 4.01473C1.24819 6.28119 0 9.29469 0 12.5C0 15.7053 1.24819 18.7188 3.51473 20.9853C5.78119 23.2518 8.79469 24.5 12 24.5C15.2053 24.5 18.2188 23.2518 20.4853 20.9853C22.7518 18.7188 24 15.7053 24 12.5C24 9.29469 22.7518 6.28119 20.4853 4.01473ZM12 23.0714C6.17091 23.0714 1.42856 18.3291 1.42856 12.5C1.42856 6.67091 6.17091 1.92856 12 1.92856C17.8291 1.92856 22.5714 6.67091 22.5714 12.5C22.5714 18.3291 17.8291 23.0714 12 23.0714Z"
                          fill="currentcolor"
                        ></path>
                        <path
                          d="M9.40398 9.3309C8.23431 8.16114 6.33104 8.16123 5.16136 9.3309C4.88241 9.60981 4.88241 10.0621 5.16136 10.3411C5.44036 10.62 5.89266 10.62 6.17157 10.3411C6.78432 9.72836 7.78126 9.7284 8.39392 10.3411C8.53342 10.4806 8.71618 10.5503 8.89895 10.5503C9.08171 10.5503 9.26457 10.4806 9.40398 10.3411C9.68293 10.0621 9.68293 9.60986 9.40398 9.3309Z"
                          fill="currentcolor"
                        ></path>
                        <path
                          d="M18.8384 9.3309C17.6688 8.16123 15.7655 8.16114 14.5958 9.3309C14.3169 9.60981 14.3169 10.0621 14.5958 10.3411C14.8748 10.62 15.3271 10.62 15.606 10.3411C16.2187 9.72836 17.2156 9.72831 17.8284 10.3411C17.9679 10.4806 18.1506 10.5503 18.3334 10.5503C18.5162 10.5503 18.699 10.4806 18.8384 10.3411C19.1174 10.0621 19.1174 9.60986 18.8384 9.3309Z"
                          fill="currentcolor"
                        ></path>
                        <path
                          d="M18.3335 13.024H5.6668C5.2723 13.024 4.95251 13.3438 4.95251 13.7383C4.95251 17.6243 8.11409 20.7859 12.0001 20.7859C15.8862 20.7859 19.0477 17.6243 19.0477 13.7383C19.0477 13.3438 18.728 13.024 18.3335 13.024ZM12.0001 19.3573C9.14366 19.3573 6.77816 17.215 6.42626 14.4525H17.574C17.2221 17.215 14.8566 19.3573 12.0001 19.3573Z"
                          fill="currentcolor"
                        ></path>
                      </g>
                      <defs>
                        <clipPath>
                          <rect
                            width="24"
                            height="24"
                            fill="white"
                            transform="translate(0 0.5)"
                          ></rect>
                        </clipPath>
                      </defs>
                    </svg>
                  </a>
                  <a href="#" class="d-flex align-items-center pe-3">
                    <svg
                      class="icon-24"
                      width="18"
                      height="23"
                      viewBox="0 0 18 23"
                      fill="none"
                      xmlns="http://www.w3.org/2000/svg"
                    >
                      <path
                        d="M9.00021 21.5V18.3391"
                        stroke="currentcolor"
                        stroke-width="2"
                        stroke-linecap="round"
                        stroke-linejoin="round"
                      ></path>
                      <path
                        fill-rule="evenodd"
                        clip-rule="evenodd"
                        d="M9.00021 14.3481V14.3481C6.75611 14.3481 4.9384 12.5218 4.9384 10.2682V5.58095C4.9384 3.32732 6.75611 1.5 9.00021 1.5C11.2433 1.5 13.061 3.32732 13.061 5.58095V10.2682C13.061 12.5218 11.2433 14.3481 9.00021 14.3481Z"
                        stroke="currentcolor"
                        stroke-width="2"
                        stroke-linecap="round"
                        stroke-linejoin="round"
                      ></path>
                      <path
                        d="M17 10.3006C17 14.7394 13.418 18.3383 9 18.3383C4.58093 18.3383 1 14.7394 1 10.3006"
                        stroke="currentcolor"
                        stroke-width="2"
                        stroke-linecap="round"
                        stroke-linejoin="round"
                      ></path>
                      <path
                        d="M11.0689 6.25579H13.0585"
                        stroke="currentcolor"
                        stroke-width="2"
                        stroke-linecap="round"
                        stroke-linejoin="round"
                      ></path>
                      <path
                        d="M10.0704 9.59344H13.0605"
                        stroke="currentcolor"
                        stroke-width="2"
                        stroke-linecap="round"
                        stroke-linejoin="round"
                      ></path>
                    </svg>
                  </a>
                </div>
                <input
                  type="text"
                  class="form-control me-3"
                  placeholder="Type your message"
                />
                <button
                  type="submit"
                  class="btn btn-primary d-flex align-items-center"
                >
                  <svg
                    class="icon-20"
                    width="18"
                    viewBox="0 0 20 21"
                    fill="none"
                    xmlns="http://www.w3.org/2000/svg"
                  >
                    <path
                      d="M13.8325 6.67463L8.10904 12.4592L1.59944 8.38767C0.66675 7.80414 0.860765 6.38744 1.91572 6.07893L17.3712 1.55277C18.3373 1.26963 19.2326 2.17283 18.9456 3.142L14.3731 18.5868C14.0598 19.6432 12.6512 19.832 12.0732 18.8953L8.10601 12.4602"
                      stroke="currentcolor"
                      stroke-width="2"
                      stroke-linecap="round"
                      stroke-linejoin="round"
                    ></path></svg
                  ><span class="d-none d-lg-block ms-1">Send</span>
                </button>
              </form>
            </div>
          </div>
          <div
            class="card tab-pane mb-0 fade"
            id="user-content-104"
            role="tabpanel"
          >
            <div class="chat-head">
              <header
                class="d-flex justify-content-between align-items-center pt-3 ps-3 pe-3 pb-3"
              >
                <div class="d-flex align-items-center gap-3">
                  <div class="d-block d-xl-none">
                    <button
                      class="btn btn-sm btn-primary rounded btn-icon"
                      data-toggle="sidebar"
                      data-active="true"
                    >
                      <span class="btn-inner">
                        <svg class="icon-rtl" width="20px" viewBox="0 0 24 24">
                          <path
                            fill="currentColor"
                            d="M4,11V13H16L10.5,18.5L11.92,19.92L19.84,12L11.92,4.08L10.5,5.5L16,11H4Z"
                          ></path>
                        </svg>
                      </span>
                    </button>
                  </div>
                  <div class="avatar chat-user-profile m-0">
                    <img
                      src="../../../assets/images/chat/avatar/04.png"
                      alt="avatar"
                      class="avatar-50 rounded-pill"
                      loading="lazy"
                    />
                    <div class="iq-profile-badge bg-success"></div>
                  </div>
                  <div>
                    <h5 class="mb-0">Robert Fox</h5>
                    <small class="text-capitalize fw-500">Online</small>
                  </div>
                </div>

                <div class="chat-header-icons d-inline-flex ms-auto">
                  <a
                    href="#"
                    class="chat-icon-phone bg-primary-subtle d-flex align-items-center justify-content-center"
                  >
                    <i class="material-symbols-outlined md-18">phone</i>
                  </a>
                  <a
                    href="#"
                    class="chat-icon-video bg-primary-subtle d-flex align-items-center justify-content-center"
                  >
                    <i class="material-symbols-outlined md-18">videocam</i>
                  </a>
                  <a
                    href="#"
                    class="chat-icon-delete bg-primary-subtle d-flex align-items-center justify-content-center"
                  >
                    <i class="material-symbols-outlined md-18">delete</i>
                  </a>
                  <span
                    class="dropdown bg-primary-subtle d-flex align-items-center justify-content-center"
                  >
                    <svg
                      class="icon-20 nav-hide-arrow cursor-pointer pe-0"
                      id="dropdownMenuButton-11"
                      data-bs-toggle="dropdown"
                      aria-haspopup="true"
                      aria-expanded="false"
                      role="menu"
                      width="24"
                      xmlns="http://www.w3.org/2000/svg"
                      viewBox="0 0 24 24"
                      fill="none"
                    >
                      <path
                        fill-rule="evenodd"
                        clip-rule="evenodd"
                        d="M10 20.6788C10 21.9595 11.0378 23 12.3113 23C13.5868 23 14.6265 21.9595 14.6265 20.6788C14.6265 19.3981 13.5868 18.3576 12.3113 18.3576C11.0378 18.3576 10 19.3981 10 20.6788ZM10 12.0005C10 13.2812 11.0378 14.3217 12.3113 14.3217C13.5868 14.3217 14.6265 13.2812 14.6265 12.0005C14.6265 10.7198 13.5868 9.67929 12.3113 9.67929C11.0378 9.67929 10 10.7198 10 12.0005ZM12.3113 5.64239C11.0378 5.64239 10 4.60192 10 3.3212C10 2.04047 11.0378 1 12.3113 1C13.5868 1 14.6265 2.04047 14.6265 3.3212C14.6265 4.60192 13.5868 5.64239 12.3113 5.64239Z"
                        fill="currentColor"
                      ></path>
                    </svg>
                    <span
                      class="dropdown-menu dropdown-menu-right"
                      aria-labelledby="dropdownMenuButton-11"
                    >
                      <a
                        class="dropdown-item d-flex align-items-center"
                        href="#"
                        ><i class="material-symbols-outlined md-18 me-1"
                          >push_pin</i
                        >Pin to top</a
                      >
                      <a
                        class="dropdown-item d-flex align-items-center"
                        href="#"
                        ><i class="material-symbols-outlined md-18 me-1"
                          >delete_outline</i
                        >Delete chat</a
                      >
                      <a
                        class="dropdown-item d-flex align-items-center"
                        href="#"
                        ><i class="material-symbols-outlined md-18 me-1"
                          >watch_later</i
                        >Block</a
                      >
                    </span>
                  </span>
                </div>
              </header>
            </div>
            <div class="card-body chat-body bg-body">
              <div class="chat-day-title">
                <span class="main-title">Feb 1,2021</span>
              </div>
              <div class="iq-message-body iq-current-user">
                <div class="chat-profile text-center">
                  <img
                    src="../../../assets/images/chat/avatar/10.png"
                    alt="chat-user"
                    class="avatar-40 rounded-pill"
                    loading="lazy"
                  />
                  <small class="iq-chating p-0 mb-0 d-block">16:34</small>
                </div>
                <div class="iq-chat-text">
                  <div
                    class="d-flex align-items-center justify-content-end gap-1 gap-md-2"
                  >
                    <div
                      class="dropdown cursor-pointer more"
                      v-b-tooltip.hover
                      title="More"
                    >
                      <div
                        class="lh-1"
                        id="post-option"
                        data-bs-toggle="dropdown"
                      >
                        <span class="material-symbols-outlined text-dark">
                          more_vert
                        </span>
                      </div>
                      <div
                        class="dropdown-menu dropdown-menu-right"
                        aria-labelledby="post-option"
                        style=""
                      >
                        <a class="dropdown-item" href="#"
                          ><span
                            class="material-symbols-outlined align-middle font-size-20 me-1"
                            >content_copy</span
                          >Copy message text</a
                        >
                        <a class="dropdown-item" href="#">
                          <span
                            class="material-symbols-outlined align-middle font-size-20 me-1"
                            >edit</span
                          >Edit</a
                        >
                      </div>
                    </div>
                    <div class="iq-chating-content d-flex align-items-center">
                      <p class="mr-2 mb-0">
                        How can we help? We're here for you! 😄
                      </p>
                    </div>
                  </div>
                </div>
              </div>
              <div class="iq-message-body iq-other-user">
                <div class="chat-profile text-center">
                  <img
                    src="../../../assets/images/chat/avatar/04.png"
                    alt="chat-user"
                    class="avatar-40 rounded-pill"
                    loading="lazy"
                  />
                  <small class="iq-chating p-0 mb-0 d-block">16:40</small>
                </div>
                <div class="iq-chat-text">
                  <div
                    class="d-flex align-items-center justify-content-start gap-md-2"
                  >
                    <div class="iq-chating-content d-flex align-items-center">
                      <p class="mr-2 mb-0">
                        Hey John, I am looking for the best admin template.Could
                        you please help me to find it out? 🤔
                      </p>
                    </div>
                    <a
                      href="#"
                      class="material-symbols-outlined font-size-20 text-dark reply"
                      v-b-tooltip.hover
                      title="Reply"
                      >reply</a
                    >
                    <div
                      class="dropdown cursor-pointer more"
                      v-b-tooltip.hover
                      title="More"
                    >
                      <div
                        class="lh-1"
                        id="post-option"
                        data-bs-toggle="dropdown"
                      >
                        <span class="material-symbols-outlined text-dark">
                          more_vert
                        </span>
                      </div>
                      <div
                        class="dropdown-menu dropdown-menu-right"
                        aria-labelledby="post-option"
                        style=""
                      >
                        <a class="dropdown-item" href="#"
                          ><span
                            class="material-symbols-outlined align-middle font-size-20 me-1"
                            >content_copy</span
                          >Copy message text</a
                        >
                        <a class="dropdown-item" href="#">
                          <span
                            class="material-symbols-outlined align-middle font-size-20 me-1"
                            >edit</span
                          >Edit</a
                        >
                      </div>
                    </div>
                  </div>
                </div>
              </div>
              <div class="iq-message-body iq-current-user">
                <div class="chat-profile text-center">
                  <img
                    src="../../../assets/images/chat/avatar/10.png"
                    alt="chat-user"
                    class="avatar-40 rounded-pill"
                    loading="lazy"
                  />
                  <small class="iq-chating p-0 mb-0 d-block">6:49</small>
                </div>
                <div class="iq-chat-text">
                  <div
                    class="d-flex align-items-center justify-content-end gap-1 gap-md-2"
                  >
                    <div
                      class="dropdown cursor-pointer more"
                      v-b-tooltip.hover
                      title="More"
                    >
                      <div
                        class="lh-1"
                        id="post-option"
                        data-bs-toggle="dropdown"
                      >
                        <span class="material-symbols-outlined text-dark">
                          more_vert
                        </span>
                      </div>
                      <div
                        class="dropdown-menu dropdown-menu-right"
                        aria-labelledby="post-option"
                        style=""
                      >
                        <a class="dropdown-item" href="#"
                          ><span
                            class="material-symbols-outlined align-middle font-size-20 me-1"
                            >content_copy</span
                          >Copy message text</a
                        >
                        <a class="dropdown-item" href="#">
                          <span
                            class="material-symbols-outlined align-middle font-size-20 me-1"
                            >edit</span
                          >Edit</a
                        >
                      </div>
                    </div>
                    <div class="iq-chating-content d-flex align-items-center">
                      <p class="mr-2 mb-0">
                        Absolutely Yes! SocialV is the Responsive Bootstrap 5
                        Admin Dashboard Template.
                      </p>
                    </div>
                  </div>
                </div>
              </div>
              <div class="iq-message-body iq-other-user">
                <div class="chat-profile text-center">
                  <img
                    src="../../../assets/images/chat/avatar/04.png"
                    alt="chat-user"
                    class="avatar-40 rounded-pill"
                    loading="lazy"
                  />
                  <small class="iq-chating p-0 mb-0 d-block">6:52</small>
                </div>
                <div class="iq-chat-text">
                  <div
                    class="d-flex align-items-center justify-content-start gap-md-2"
                  >
                    <div class="iq-chating-content d-flex align-items-center">
                      <p class="mr-2 mb-0">Looks clean and fresh UI.</p>
                    </div>
                    <a
                      href="#"
                      class="material-symbols-outlined font-size-20 text-dark reply"
                      v-b-tooltip.hover
                      title="Reply"
                      >reply</a
                    >
                    <div
                      class="dropdown cursor-pointer more"
                      v-b-tooltip.hover
                      title="More"
                    >
                      <div
                        class="lh-1"
                        id="post-option"
                        data-bs-toggle="dropdown"
                      >
                        <span class="material-symbols-outlined text-dark">
                          more_vert
                        </span>
                      </div>
                      <div
                        class="dropdown-menu dropdown-menu-right"
                        aria-labelledby="post-option"
                        style=""
                      >
                        <a class="dropdown-item" href="#"
                          ><span
                            class="material-symbols-outlined align-middle font-size-20 me-1"
                            >content_copy</span
                          >Copy message text</a
                        >
                        <a class="dropdown-item" href="#">
                          <span
                            class="material-symbols-outlined align-middle font-size-20 me-1"
                            >edit</span
                          >Edit</a
                        >
                      </div>
                    </div>
                  </div>
                </div>
              </div>
              <div class="iq-message-body iq-current-user">
                <div class="chat-profile text-center">
                  <img
                    src="../../../assets/images/chat/avatar/10.png"
                    alt="chat-user"
                    class="avatar-40 rounded-pill"
                    loading="lazy"
                  />
                  <small class="iq-chating p-0 mb-0 d-block">6:53</small>
                </div>
                <div class="iq-chat-text">
                  <div
                    class="d-flex align-items-center justify-content-end gap-1 gap-md-2"
                  >
                    <div
                      class="dropdown cursor-pointer more"
                      v-b-tooltip.hover
                      title="More"
                    >
                      <div
                        class="lh-1"
                        id="post-option"
                        data-bs-toggle="dropdown"
                      >
                        <span class="material-symbols-outlined text-dark">
                          more_vert
                        </span>
                      </div>
                      <div
                        class="dropdown-menu dropdown-menu-right"
                        aria-labelledby="post-option"
                        style=""
                      >
                        <a class="dropdown-item" href="#"
                          ><span
                            class="material-symbols-outlined align-middle font-size-20 me-1"
                            >content_copy</span
                          >Copy message text</a
                        >
                        <a class="dropdown-item" href="#">
                          <span
                            class="material-symbols-outlined align-middle font-size-20 me-1"
                            >edit</span
                          >Edit</a
                        >
                      </div>
                    </div>
                    <div class="iq-chating-content d-flex align-items-center">
                      <p class="mr-2 mb-0">Thanks, from ThemeForest.</p>
                    </div>
                  </div>
                </div>
              </div>
              <div class="iq-message-body iq-other-user">
                <div class="chat-profile text-center">
                  <img
                    src="../../../assets/images/chat/avatar/04.png"
                    alt="chat-user"
                    class="avatar-40 rounded-pill"
                    loading="lazy"
                  />
                  <small class="iq-chating p-0 mb-0 d-block">6:54</small>
                </div>
                <div class="iq-chat-text">
                  <div
                    class="d-flex align-items-center justify-content-start gap-md-2"
                  >
                    <div class="iq-chating-content d-flex align-items-center">
                      <p class="mr-2 mb-0">I will purchase it for sure. 👍</p>
                    </div>
                    <a
                      href="#"
                      class="material-symbols-outlined font-size-20 text-dark reply"
                      v-b-tooltip.hover
                      title="Reply"
                      >reply</a
                    >
                    <div
                      class="dropdown cursor-pointer more"
                      v-b-tooltip.hover
                      title="More"
                    >
                      <div
                        class="lh-1"
                        id="post-option"
                        data-bs-toggle="dropdown"
                      >
                        <span class="material-symbols-outlined text-dark">
                          more_vert
                        </span>
                      </div>
                      <div
                        class="dropdown-menu dropdown-menu-right"
                        aria-labelledby="post-option"
                        style=""
                      >
                        <a class="dropdown-item" href="#"
                          ><span
                            class="material-symbols-outlined align-middle font-size-20 me-1"
                            >content_copy</span
                          >Copy message text</a
                        >
                        <a class="dropdown-item" href="#">
                          <span
                            class="material-symbols-outlined align-middle font-size-20 me-1"
                            >edit</span
                          >Edit</a
                        >
                      </div>
                    </div>
                  </div>
                </div>
              </div>
              <div class="iq-message-body iq-current-user">
                <div class="chat-profile text-center">
                  <img
                    src="../../../assets/images/chat/avatar/10.png"
                    alt="chat-user"
                    class="avatar-40 rounded-pill"
                    loading="lazy"
                  />
                  <small class="iq-chating p-0 mb-0 d-block">6:55</small>
                </div>
                <div class="iq-chat-text">
                  <div
                    class="d-flex align-items-center justify-content-end gap-1 gap-md-2"
                  >
                    <div
                      class="dropdown cursor-pointer more"
                      v-b-tooltip.hover
                      title="More"
                    >
                      <div
                        class="lh-1"
                        id="post-option"
                        data-bs-toggle="dropdown"
                      >
                        <span class="material-symbols-outlined text-dark">
                          more_vert
                        </span>
                      </div>
                      <div
                        class="dropdown-menu dropdown-menu-right"
                        aria-labelledby="post-option"
                        style=""
                      >
                        <a class="dropdown-item" href="#"
                          ><span
                            class="material-symbols-outlined align-middle font-size-20 me-1"
                            >content_copy</span
                          >Copy message text</a
                        >
                        <a class="dropdown-item" href="#">
                          <span
                            class="material-symbols-outlined align-middle font-size-20 me-1"
                            >edit</span
                          >Edit</a
                        >
                      </div>
                    </div>
                    <div class="iq-chating-content d-flex align-items-center">
                      <p class="mr-2 mb-0">Okay Thanks..</p>
                    </div>
                  </div>
                </div>
              </div>
              <div class="iq-message-body iq-other-user">
                <div class="chat-profile text-center">
                  <img
                    src="../../../assets/images/chat/avatar/04.png"
                    alt="chat-user"
                    class="avatar-40 rounded-pill"
                    loading="lazy"
                  />
                  <small class="iq-chating p-0 mb-0 d-block">7:54</small>
                </div>
                <div class="iq-chat-text">
                  <div
                    class="d-flex align-items-center justify-content-start gap-md-2"
                  >
                    <div class="iq-chating-content d-flex align-items-center">
                      <p class="mr-2 mb-0">Looks clean and fresh UI.</p>
                    </div>
                    <a
                      href="#"
                      class="material-symbols-outlined font-size-20 text-dark reply"
                      v-b-tooltip.hover
                      title="Reply"
                      >reply</a
                    >
                    <div
                      class="dropdown cursor-pointer more"
                      v-b-tooltip.hover
                      title="More"
                    >
                      <div
                        class="lh-1"
                        id="post-option"
                        data-bs-toggle="dropdown"
                      >
                        <span class="material-symbols-outlined text-dark">
                          more_vert
                        </span>
                      </div>
                      <div
                        class="dropdown-menu dropdown-menu-right"
                        aria-labelledby="post-option"
                        style=""
                      >
                        <a class="dropdown-item" href="#"
                          ><span
                            class="material-symbols-outlined align-middle font-size-20 me-1"
                            >content_copy</span
                          >Copy message text</a
                        >
                        <a class="dropdown-item" href="#">
                          <span
                            class="material-symbols-outlined align-middle font-size-20 me-1"
                            >edit</span
                          >Edit</a
                        >
                      </div>
                    </div>
                  </div>
                </div>
              </div>
              <div class="iq-message-body iq-current-user">
                <div class="chat-profile text-center">
                  <img
                    src="../../../assets/images/chat/avatar/10.png"
                    alt="chat-user"
                    class="avatar-40 rounded-pill"
                    loading="lazy"
                  />
                  <small class="iq-chating p-0 mb-0 d-block">7:55</small>
                </div>
                <div class="iq-chat-text">
                  <div
                    class="d-flex align-items-center justify-content-end gap-1 gap-md-2"
                  >
                    <div
                      class="dropdown cursor-pointer more"
                      v-b-tooltip.hover
                      title="More"
                    >
                      <div
                        class="lh-1"
                        id="post-option"
                        data-bs-toggle="dropdown"
                      >
                        <span class="material-symbols-outlined text-dark">
                          more_vert
                        </span>
                      </div>
                      <div
                        class="dropdown-menu dropdown-menu-right"
                        aria-labelledby="post-option"
                        style=""
                      >
                        <a class="dropdown-item" href="#"
                          ><span
                            class="material-symbols-outlined align-middle font-size-20 me-1"
                            >content_copy</span
                          >Copy message text</a
                        >
                        <a class="dropdown-item" href="#">
                          <span
                            class="material-symbols-outlined align-middle font-size-20 me-1"
                            >edit</span
                          >Edit</a
                        >
                      </div>
                    </div>
                    <div class="iq-chating-content d-flex align-items-center">
                      <p class="mr-2 mb-0">Thanks, from ThemeForest.</p>
                    </div>
                  </div>
                </div>
              </div>
              <div class="iq-message-body iq-other-user">
                <div class="chat-profile text-center">
                  <img
                    src="../../../assets/images/chat/avatar/04.png"
                    alt="chat-user"
                    class="avatar-40 rounded-pill"
                    loading="lazy"
                  />
                  <small class="iq-chating p-0 mb-0 d-block">7:54</small>
                </div>
                <div class="iq-chat-text">
                  <div
                    class="d-flex align-items-center justify-content-start gap-md-2"
                  >
                    <div class="iq-chating-content d-flex align-items-center">
                      <p class="mr-2 mb-0">Looks clean and fresh UI.</p>
                    </div>
                    <a
                      href="#"
                      class="material-symbols-outlined font-size-20 text-dark reply"
                      v-b-tooltip.hover
                      title="Reply"
                      >reply</a
                    >
                    <div
                      class="dropdown cursor-pointer more"
                      v-b-tooltip.hover
                      title="More"
                    >
                      <div
                        class="lh-1"
                        id="post-option"
                        data-bs-toggle="dropdown"
                      >
                        <span class="material-symbols-outlined text-dark">
                          more_vert
                        </span>
                      </div>
                      <div
                        class="dropdown-menu dropdown-menu-right"
                        aria-labelledby="post-option"
                        style=""
                      >
                        <a class="dropdown-item" href="#"
                          ><span
                            class="material-symbols-outlined align-middle font-size-20 me-1"
                            >content_copy</span
                          >Copy message text</a
                        >
                        <a class="dropdown-item" href="#">
                          <span
                            class="material-symbols-outlined align-middle font-size-20 me-1"
                            >edit</span
                          >Edit</a
                        >
                      </div>
                    </div>
                  </div>
                </div>
              </div>
              <div class="iq-message-body iq-current-user">
                <div class="chat-profile text-center">
                  <img
                    src="../../../assets/images/chat/avatar/10.png"
                    alt="chat-user"
                    class="avatar-40 rounded-pill"
                    loading="lazy"
                  />
                  <small class="iq-chating p-0 mb-0 d-block">7:55</small>
                </div>
                <div class="iq-chat-text">
                  <div
                    class="d-flex align-items-center justify-content-end gap-1 gap-md-2"
                  >
                    <div
                      class="dropdown cursor-pointer more"
                      v-b-tooltip.hover
                      title="More"
                    >
                      <div
                        class="lh-1"
                        id="post-option"
                        data-bs-toggle="dropdown"
                      >
                        <span class="material-symbols-outlined text-dark">
                          more_vert
                        </span>
                      </div>
                      <div
                        class="dropdown-menu dropdown-menu-right"
                        aria-labelledby="post-option"
                        style=""
                      >
                        <a class="dropdown-item" href="#"
                          ><span
                            class="material-symbols-outlined align-middle font-size-20 me-1"
                            >content_copy</span
                          >Copy message text</a
                        >
                        <a class="dropdown-item" href="#">
                          <span
                            class="material-symbols-outlined align-middle font-size-20 me-1"
                            >edit</span
                          >Edit</a
                        >
                      </div>
                    </div>
                    <div class="iq-chating-content d-flex align-items-center">
                      <p class="mr-2 mb-0">Thanks, from ThemeForest.</p>
                    </div>
                  </div>
                </div>
              </div>
              <div class="iq-message-body iq-other-user">
                <div class="chat-profile text-center">
                  <img
                    src="../../../assets/images/chat/avatar/04.png"
                    alt="chat-user"
                    class="avatar-40 rounded-pill"
                    loading="lazy"
                  />
                  <small class="iq-chating p-0 mb-0 d-block">7:54</small>
                </div>
                <div class="iq-chat-text">
                  <div
                    class="d-flex align-items-center justify-content-start gap-md-2"
                  >
                    <div class="iq-chating-content d-flex align-items-center">
                      <p class="mr-2 mb-0">Looks clean and fresh UI.</p>
                    </div>
                    <a
                      href="#"
                      class="material-symbols-outlined font-size-20 text-dark reply"
                      v-b-tooltip.hover
                      title="Reply"
                      >reply</a
                    >
                    <div
                      class="dropdown cursor-pointer more"
                      v-b-tooltip.hover
                      title="More"
                    >
                      <div
                        class="lh-1"
                        id="post-option"
                        data-bs-toggle="dropdown"
                      >
                        <span class="material-symbols-outlined text-dark">
                          more_vert
                        </span>
                      </div>
                      <div
                        class="dropdown-menu dropdown-menu-right"
                        aria-labelledby="post-option"
                        style=""
                      >
                        <a class="dropdown-item" href="#"
                          ><span
                            class="material-symbols-outlined align-middle font-size-20 me-1"
                            >content_copy</span
                          >Copy message text</a
                        >
                        <a class="dropdown-item" href="#">
                          <span
                            class="material-symbols-outlined align-middle font-size-20 me-1"
                            >edit</span
                          >Edit</a
                        >
                      </div>
                    </div>
                  </div>
                </div>
              </div>
              <div class="iq-message-body iq-current-user">
                <div class="chat-profile text-center">
                  <img
                    src="../../../assets/images/chat/avatar/10.png"
                    alt="chat-user"
                    class="avatar-40 rounded-pill"
                    loading="lazy"
                  />
                  <small class="iq-chating p-0 mb-0 d-block">7:55</small>
                </div>
                <div class="iq-chat-text">
                  <div
                    class="d-flex align-items-center justify-content-end gap-1 gap-md-2"
                  >
                    <div
                      class="dropdown cursor-pointer more"
                      v-b-tooltip.hover
                      title="More"
                    >
                      <div
                        class="lh-1"
                        id="post-option"
                        data-bs-toggle="dropdown"
                      >
                        <span class="material-symbols-outlined text-dark">
                          more_vert
                        </span>
                      </div>
                      <div
                        class="dropdown-menu dropdown-menu-right"
                        aria-labelledby="post-option"
                        style=""
                      >
                        <a class="dropdown-item" href="#"
                          ><span
                            class="material-symbols-outlined align-middle font-size-20 me-1"
                            >content_copy</span
                          >Copy message text</a
                        >
                        <a class="dropdown-item" href="#">
                          <span
                            class="material-symbols-outlined align-middle font-size-20 me-1"
                            >edit</span
                          >Edit</a
                        >
                      </div>
                    </div>
                    <div class="iq-chating-content d-flex align-items-center">
                      <p class="mr-2 mb-0">Thanks, from ThemeForest.</p>
                    </div>
                  </div>
                </div>
              </div>
            </div>
            <div class="card-footer px-3 py-3 border-top rounded-0">
              <form class="d-flex align-items-center" action="#">
                <div class="chat-attagement d-flex">
                  <a href="#" class="d-flex align-items-center pe-3">
                    <svg
                      class="icon-24"
                      width="24"
                      viewBox="0 0 24 25"
                      fill="none"
                      xmlns="http://www.w3.org/2000/svg"
                    >
                      <g clip-path="url(#clip0_156_599)">
                        <path
                          d="M20.4853 4.01473C18.2188 1.74823 15.2053 0.5 12 0.5C8.79469 0.5 5.78119 1.74823 3.51473 4.01473C1.24819 6.28119 0 9.29469 0 12.5C0 15.7053 1.24819 18.7188 3.51473 20.9853C5.78119 23.2518 8.79469 24.5 12 24.5C15.2053 24.5 18.2188 23.2518 20.4853 20.9853C22.7518 18.7188 24 15.7053 24 12.5C24 9.29469 22.7518 6.28119 20.4853 4.01473ZM12 23.0714C6.17091 23.0714 1.42856 18.3291 1.42856 12.5C1.42856 6.67091 6.17091 1.92856 12 1.92856C17.8291 1.92856 22.5714 6.67091 22.5714 12.5C22.5714 18.3291 17.8291 23.0714 12 23.0714Z"
                          fill="currentcolor"
                        ></path>
                        <path
                          d="M9.40398 9.3309C8.23431 8.16114 6.33104 8.16123 5.16136 9.3309C4.88241 9.60981 4.88241 10.0621 5.16136 10.3411C5.44036 10.62 5.89266 10.62 6.17157 10.3411C6.78432 9.72836 7.78126 9.7284 8.39392 10.3411C8.53342 10.4806 8.71618 10.5503 8.89895 10.5503C9.08171 10.5503 9.26457 10.4806 9.40398 10.3411C9.68293 10.0621 9.68293 9.60986 9.40398 9.3309Z"
                          fill="currentcolor"
                        ></path>
                        <path
                          d="M18.8384 9.3309C17.6688 8.16123 15.7655 8.16114 14.5958 9.3309C14.3169 9.60981 14.3169 10.0621 14.5958 10.3411C14.8748 10.62 15.3271 10.62 15.606 10.3411C16.2187 9.72836 17.2156 9.72831 17.8284 10.3411C17.9679 10.4806 18.1506 10.5503 18.3334 10.5503C18.5162 10.5503 18.699 10.4806 18.8384 10.3411C19.1174 10.0621 19.1174 9.60986 18.8384 9.3309Z"
                          fill="currentcolor"
                        ></path>
                        <path
                          d="M18.3335 13.024H5.6668C5.2723 13.024 4.95251 13.3438 4.95251 13.7383C4.95251 17.6243 8.11409 20.7859 12.0001 20.7859C15.8862 20.7859 19.0477 17.6243 19.0477 13.7383C19.0477 13.3438 18.728 13.024 18.3335 13.024ZM12.0001 19.3573C9.14366 19.3573 6.77816 17.215 6.42626 14.4525H17.574C17.2221 17.215 14.8566 19.3573 12.0001 19.3573Z"
                          fill="currentcolor"
                        ></path>
                      </g>
                      <defs>
                        <clipPath>
                          <rect
                            width="24"
                            height="24"
                            fill="white"
                            transform="translate(0 0.5)"
                          ></rect>
                        </clipPath>
                      </defs>
                    </svg>
                  </a>
                  <a href="#" class="d-flex align-items-center pe-3">
                    <svg
                      class="icon-24"
                      width="18"
                      height="23"
                      viewBox="0 0 18 23"
                      fill="none"
                      xmlns="http://www.w3.org/2000/svg"
                    >
                      <path
                        d="M9.00021 21.5V18.3391"
                        stroke="currentcolor"
                        stroke-width="2"
                        stroke-linecap="round"
                        stroke-linejoin="round"
                      ></path>
                      <path
                        fill-rule="evenodd"
                        clip-rule="evenodd"
                        d="M9.00021 14.3481V14.3481C6.75611 14.3481 4.9384 12.5218 4.9384 10.2682V5.58095C4.9384 3.32732 6.75611 1.5 9.00021 1.5C11.2433 1.5 13.061 3.32732 13.061 5.58095V10.2682C13.061 12.5218 11.2433 14.3481 9.00021 14.3481Z"
                        stroke="currentcolor"
                        stroke-width="2"
                        stroke-linecap="round"
                        stroke-linejoin="round"
                      ></path>
                      <path
                        d="M17 10.3006C17 14.7394 13.418 18.3383 9 18.3383C4.58093 18.3383 1 14.7394 1 10.3006"
                        stroke="currentcolor"
                        stroke-width="2"
                        stroke-linecap="round"
                        stroke-linejoin="round"
                      ></path>
                      <path
                        d="M11.0689 6.25579H13.0585"
                        stroke="currentcolor"
                        stroke-width="2"
                        stroke-linecap="round"
                        stroke-linejoin="round"
                      ></path>
                      <path
                        d="M10.0704 9.59344H13.0605"
                        stroke="currentcolor"
                        stroke-width="2"
                        stroke-linecap="round"
                        stroke-linejoin="round"
                      ></path>
                    </svg>
                  </a>
                </div>
                <input
                  type="text"
                  class="form-control me-3"
                  placeholder="Type your message"
                />
                <button
                  type="submit"
                  class="btn btn-primary d-flex align-items-center"
                >
                  <svg
                    class="icon-20"
                    width="18"
                    viewBox="0 0 20 21"
                    fill="none"
                    xmlns="http://www.w3.org/2000/svg"
                  >
                    <path
                      d="M13.8325 6.67463L8.10904 12.4592L1.59944 8.38767C0.66675 7.80414 0.860765 6.38744 1.91572 6.07893L17.3712 1.55277C18.3373 1.26963 19.2326 2.17283 18.9456 3.142L14.3731 18.5868C14.0598 19.6432 12.6512 19.832 12.0732 18.8953L8.10601 12.4602"
                      stroke="currentcolor"
                      stroke-width="2"
                      stroke-linecap="round"
                      stroke-linejoin="round"
                    ></path></svg
                  ><span class="d-none d-lg-block ms-1">Send</span>
                </button>
              </form>
            </div>
          </div>
          <div
            class="card tab-pane mb-0 fade"
            id="user-content-105"
            role="tabpanel"
          >
            <div class="chat-head">
              <header
                class="d-flex justify-content-between align-items-center pt-3 ps-3 pe-3 pb-3"
              >
                <div class="d-flex align-items-center gap-3">
                  <div class="d-block d-xl-none">
                    <button
                      class="btn btn-sm btn-primary rounded btn-icon"
                      data-toggle="sidebar"
                      data-active="true"
                    >
                      <span class="btn-inner">
                        <svg class="icon-rtl" width="20px" viewBox="0 0 24 24">
                          <path
                            fill="currentColor"
                            d="M4,11V13H16L10.5,18.5L11.92,19.92L19.84,12L11.92,4.08L10.5,5.5L16,11H4Z"
                          ></path>
                        </svg>
                      </span>
                    </button>
                  </div>
                  <div class="avatar chat-user-profile m-0">
                    <img
                      src="../../../assets/images/chat/avatar/05.png"
                      alt="avatar"
                      class="avatar-50 rounded-pill"
                      loading="lazy"
                    />
                    <div class="iq-profile-badge bg-danger"></div>
                  </div>
                  <div>
                    <h5 class="mb-0">Maya Didas</h5>
                    <small class="text-capitalize fw-500">Offline</small>
                  </div>
                </div>

                <div class="chat-header-icons d-inline-flex ms-auto">
                  <a
                    href="#"
                    class="chat-icon-phone bg-primary-subtle d-flex align-items-center justify-content-center"
                  >
                    <i class="material-symbols-outlined md-18">phone</i>
                  </a>
                  <a
                    href="#"
                    class="chat-icon-video bg-primary-subtle d-flex align-items-center justify-content-center"
                  >
                    <i class="material-symbols-outlined md-18">videocam</i>
                  </a>
                  <a
                    href="#"
                    class="chat-icon-delete bg-primary-subtle d-flex align-items-center justify-content-center"
                  >
                    <i class="material-symbols-outlined md-18">delete</i>
                  </a>
                  <span
                    class="dropdown bg-primary-subtle d-flex align-items-center justify-content-center"
                  >
                    <svg
                      class="icon-20 nav-hide-arrow cursor-pointer pe-0"
                      id="dropdownMenuButton-12"
                      data-bs-toggle="dropdown"
                      aria-haspopup="true"
                      aria-expanded="false"
                      role="menu"
                      width="24"
                      xmlns="http://www.w3.org/2000/svg"
                      viewBox="0 0 24 24"
                      fill="none"
                    >
                      <path
                        fill-rule="evenodd"
                        clip-rule="evenodd"
                        d="M10 20.6788C10 21.9595 11.0378 23 12.3113 23C13.5868 23 14.6265 21.9595 14.6265 20.6788C14.6265 19.3981 13.5868 18.3576 12.3113 18.3576C11.0378 18.3576 10 19.3981 10 20.6788ZM10 12.0005C10 13.2812 11.0378 14.3217 12.3113 14.3217C13.5868 14.3217 14.6265 13.2812 14.6265 12.0005C14.6265 10.7198 13.5868 9.67929 12.3113 9.67929C11.0378 9.67929 10 10.7198 10 12.0005ZM12.3113 5.64239C11.0378 5.64239 10 4.60192 10 3.3212C10 2.04047 11.0378 1 12.3113 1C13.5868 1 14.6265 2.04047 14.6265 3.3212C14.6265 4.60192 13.5868 5.64239 12.3113 5.64239Z"
                        fill="currentColor"
                      ></path>
                    </svg>
                    <span
                      class="dropdown-menu dropdown-menu-right"
                      aria-labelledby="dropdownMenuButton-12"
                    >
                      <a
                        class="dropdown-item d-flex align-items-center"
                        href="#"
                        ><i class="material-symbols-outlined md-18 me-1"
                          >push_pin</i
                        >Pin to top</a
                      >
                      <a
                        class="dropdown-item d-flex align-items-center"
                        href="#"
                        ><i class="material-symbols-outlined md-18 me-1"
                          >delete_outline</i
                        >Delete chat</a
                      >
                      <a
                        class="dropdown-item d-flex align-items-center"
                        href="#"
                        ><i class="material-symbols-outlined md-18 me-1"
                          >watch_later</i
                        >Block</a
                      >
                    </span>
                  </span>
                </div>
              </header>
            </div>
            <div class="card-body chat-body bg-body">
              <div class="chat-day-title">
                <span class="main-title">Feb 1,2021</span>
              </div>
              <div class="iq-message-body iq-current-user">
                <div class="chat-profile text-center">
                  <img
                    src="../../../assets/images/chat/avatar/10.png"
                    alt="chat-user"
                    class="avatar-40 rounded-pill"
                    loading="lazy"
                  />
                  <small class="iq-chating p-0 mb-0 d-block">16:34</small>
                </div>
                <div class="iq-chat-text">
                  <div
                    class="d-flex align-items-center justify-content-end gap-1 gap-md-2"
                  >
                    <div
                      class="dropdown cursor-pointer more"
                      v-b-tooltip.hover
                      title="More"
                    >
                      <div
                        class="lh-1"
                        id="post-option"
                        data-bs-toggle="dropdown"
                      >
                        <span class="material-symbols-outlined text-dark">
                          more_vert
                        </span>
                      </div>
                      <div
                        class="dropdown-menu dropdown-menu-right"
                        aria-labelledby="post-option"
                        style=""
                      >
                        <a class="dropdown-item" href="#"
                          ><span
                            class="material-symbols-outlined align-middle font-size-20 me-1"
                            >content_copy</span
                          >Copy message text</a
                        >
                        <a class="dropdown-item" href="#">
                          <span
                            class="material-symbols-outlined align-middle font-size-20 me-1"
                            >edit</span
                          >Edit</a
                        >
                      </div>
                    </div>
                    <div class="iq-chating-content d-flex align-items-center">
                      <p class="mr-2 mb-0">
                        How can we help? We're here for you! 😄
                      </p>
                    </div>
                  </div>
                </div>
              </div>
              <div class="iq-message-body iq-other-user">
                <div class="chat-profile text-center">
                  <img
                    src="../../../assets/images/chat/avatar/05.png"
                    alt="chat-user"
                    class="avatar-40 rounded-pill"
                    loading="lazy"
                  />
                  <small class="iq-chating p-0 mb-0 d-block">16:40</small>
                </div>
                <div class="iq-chat-text">
                  <div
                    class="d-flex align-items-center justify-content-start gap-md-2"
                  >
                    <div class="iq-chating-content d-flex align-items-center">
                      <p class="mr-2 mb-0">
                        Hey John, I am looking for the best admin template.
                        Could you please help me to find it out? 🤔
                      </p>
                    </div>
                    <a
                      href="#"
                      class="material-symbols-outlined font-size-20 text-dark reply"
                      v-b-tooltip.hover
                      title="Reply"
                      >reply</a
                    >
                    <div
                      class="dropdown cursor-pointer more"
                      v-b-tooltip.hover
                      title="More"
                    >
                      <div
                        class="lh-1"
                        id="post-option"
                        data-bs-toggle="dropdown"
                      >
                        <span class="material-symbols-outlined text-dark">
                          more_vert
                        </span>
                      </div>
                      <div
                        class="dropdown-menu dropdown-menu-right"
                        aria-labelledby="post-option"
                        style=""
                      >
                        <a class="dropdown-item" href="#"
                          ><span
                            class="material-symbols-outlined align-middle font-size-20 me-1"
                            >content_copy</span
                          >Copy message text</a
                        >
                        <a class="dropdown-item" href="#">
                          <span
                            class="material-symbols-outlined align-middle font-size-20 me-1"
                            >edit</span
                          >Edit</a
                        >
                      </div>
                    </div>
                  </div>
                </div>
              </div>
              <div class="iq-message-body iq-current-user">
                <div class="chat-profile text-center">
                  <img
                    src="../../../assets/images/chat/avatar/10.png"
                    alt="chat-user"
                    class="avatar-40 rounded-pill"
                    loading="lazy"
                  />
                  <small class="iq-chating p-0 mb-0 d-block">6:49</small>
                </div>
                <div class="iq-chat-text">
                  <div
                    class="d-flex align-items-center justify-content-end gap-1 gap-md-2"
                  >
                    <div
                      class="dropdown cursor-pointer more"
                      v-b-tooltip.hover
                      title="More"
                    >
                      <div
                        class="lh-1"
                        id="post-option"
                        data-bs-toggle="dropdown"
                      >
                        <span class="material-symbols-outlined text-dark">
                          more_vert
                        </span>
                      </div>
                      <div
                        class="dropdown-menu dropdown-menu-right"
                        aria-labelledby="post-option"
                        style=""
                      >
                        <a class="dropdown-item" href="#"
                          ><span
                            class="material-symbols-outlined align-middle font-size-20 me-1"
                            >content_copy</span
                          >Copy message text</a
                        >
                        <a class="dropdown-item" href="#">
                          <span
                            class="material-symbols-outlined align-middle font-size-20 me-1"
                            >edit</span
                          >Edit</a
                        >
                      </div>
                    </div>
                    <div class="iq-chating-content d-flex align-items-center">
                      <p class="mr-2 mb-0">
                        Absolutely Yes! SocialV is the Responsive Bootstrap 5
                        Admin Dashboard Template.
                      </p>
                    </div>
                  </div>
                </div>
              </div>
              <div class="iq-message-body iq-other-user">
                <div class="chat-profile text-center">
                  <img
                    src="../../../assets/images/chat/avatar/05.png"
                    alt="chat-user"
                    class="avatar-40 rounded-pill"
                    loading="lazy"
                  />
                  <small class="iq-chating p-0 mb-0 d-block">6:52</small>
                </div>
                <div class="iq-chat-text">
                  <div
                    class="d-flex align-items-center justify-content-start gap-md-2"
                  >
                    <div class="iq-chating-content d-flex align-items-center">
                      <p class="mr-2 mb-0">Looks clean and fresh UI.</p>
                    </div>
                    <a
                      href="#"
                      class="material-symbols-outlined font-size-20 text-dark reply"
                      v-b-tooltip.hover
                      title="Reply"
                      >reply</a
                    >
                    <div
                      class="dropdown cursor-pointer more"
                      v-b-tooltip.hover
                      title="More"
                    >
                      <div
                        class="lh-1"
                        id="post-option"
                        data-bs-toggle="dropdown"
                      >
                        <span class="material-symbols-outlined text-dark">
                          more_vert
                        </span>
                      </div>
                      <div
                        class="dropdown-menu dropdown-menu-right"
                        aria-labelledby="post-option"
                        style=""
                      >
                        <a class="dropdown-item" href="#"
                          ><span
                            class="material-symbols-outlined align-middle font-size-20 me-1"
                            >content_copy</span
                          >Copy message text</a
                        >
                        <a class="dropdown-item" href="#">
                          <span
                            class="material-symbols-outlined align-middle font-size-20 me-1"
                            >edit</span
                          >Edit</a
                        >
                      </div>
                    </div>
                  </div>
                </div>
              </div>
              <div class="iq-message-body iq-current-user">
                <div class="chat-profile text-center">
                  <img
                    src="../../../assets/images/chat/avatar/10.png"
                    alt="chat-user"
                    class="avatar-40 rounded-pill"
                    loading="lazy"
                  />
                  <small class="iq-chating p-0 mb-0 d-block">6:53</small>
                </div>
                <div class="iq-chat-text">
                  <div
                    class="d-flex align-items-center justify-content-end gap-1 gap-md-2"
                  >
                    <div
                      class="dropdown cursor-pointer more"
                      v-b-tooltip.hover
                      title="More"
                    >
                      <div
                        class="lh-1"
                        id="post-option"
                        data-bs-toggle="dropdown"
                      >
                        <span class="material-symbols-outlined text-dark">
                          more_vert
                        </span>
                      </div>
                      <div
                        class="dropdown-menu dropdown-menu-right"
                        aria-labelledby="post-option"
                        style=""
                      >
                        <a class="dropdown-item" href="#"
                          ><span
                            class="material-symbols-outlined align-middle font-size-20 me-1"
                            >content_copy</span
                          >Copy message text</a
                        >
                        <a class="dropdown-item" href="#">
                          <span
                            class="material-symbols-outlined align-middle font-size-20 me-1"
                            >edit</span
                          >Edit</a
                        >
                      </div>
                    </div>
                    <div class="iq-chating-content d-flex align-items-center">
                      <p class="mr-2 mb-0">Thanks, from ThemeForest.</p>
                    </div>
                  </div>
                </div>
              </div>
              <div class="iq-message-body iq-other-user">
                <div class="chat-profile text-center">
                  <img
                    src="../../../assets/images/chat/avatar/05.png"
                    alt="chat-user"
                    class="avatar-40 rounded-pill"
                    loading="lazy"
                  />
                  <small class="iq-chating p-0 mb-0 d-block">6:54</small>
                </div>
                <div class="iq-chat-text">
                  <div
                    class="d-flex align-items-center justify-content-start gap-md-2"
                  >
                    <div class="iq-chating-content d-flex align-items-center">
                      <p class="mr-2 mb-0">I will purchase it for sure. 👍</p>
                    </div>
                    <a
                      href="#"
                      class="material-symbols-outlined font-size-20 text-dark reply"
                      v-b-tooltip.hover
                      title="Reply"
                      >reply</a
                    >
                    <div
                      class="dropdown cursor-pointer more"
                      v-b-tooltip.hover
                      title="More"
                    >
                      <div
                        class="lh-1"
                        id="post-option"
                        data-bs-toggle="dropdown"
                      >
                        <span class="material-symbols-outlined text-dark">
                          more_vert
                        </span>
                      </div>
                      <div
                        class="dropdown-menu dropdown-menu-right"
                        aria-labelledby="post-option"
                        style=""
                      >
                        <a class="dropdown-item" href="#"
                          ><span
                            class="material-symbols-outlined align-middle font-size-20 me-1"
                            >content_copy</span
                          >Copy message text</a
                        >
                        <a class="dropdown-item" href="#">
                          <span
                            class="material-symbols-outlined align-middle font-size-20 me-1"
                            >edit</span
                          >Edit</a
                        >
                      </div>
                    </div>
                  </div>
                </div>
              </div>
              <div class="iq-message-body iq-current-user">
                <div class="chat-profile text-center">
                  <img
                    src="../../../assets/images/chat/avatar/10.png"
                    alt="chat-user"
                    class="avatar-40 rounded-pill"
                    loading="lazy"
                  />
                  <small class="iq-chating p-0 mb-0 d-block">6:55</small>
                </div>
                <div class="iq-chat-text">
                  <div
                    class="d-flex align-items-center justify-content-end gap-1 gap-md-2"
                  >
                    <div
                      class="dropdown cursor-pointer more"
                      v-b-tooltip.hover
                      title="More"
                    >
                      <div
                        class="lh-1"
                        id="post-option"
                        data-bs-toggle="dropdown"
                      >
                        <span class="material-symbols-outlined text-dark">
                          more_vert
                        </span>
                      </div>
                      <div
                        class="dropdown-menu dropdown-menu-right"
                        aria-labelledby="post-option"
                        style=""
                      >
                        <a class="dropdown-item" href="#"
                          ><span
                            class="material-symbols-outlined align-middle font-size-20 me-1"
                            >content_copy</span
                          >Copy message text</a
                        >
                        <a class="dropdown-item" href="#">
                          <span
                            class="material-symbols-outlined align-middle font-size-20 me-1"
                            >edit</span
                          >Edit</a
                        >
                      </div>
                    </div>
                    <div class="iq-chating-content d-flex align-items-center">
                      <p class="mr-2 mb-0">Okay Thanks..</p>
                    </div>
                  </div>
                </div>
              </div>
              <div class="iq-message-body iq-other-user">
                <div class="chat-profile text-center">
                  <img
                    src="../../../assets/images/chat/avatar/05.png"
                    alt="chat-user"
                    class="avatar-40 rounded-pill"
                    loading="lazy"
                  />
                  <small class="iq-chating p-0 mb-0 d-block">7:54</small>
                </div>
                <div class="iq-chat-text">
                  <div
                    class="d-flex align-items-center justify-content-start gap-md-2"
                  >
                    <div class="iq-chating-content d-flex align-items-center">
                      <p class="mr-2 mb-0">Looks clean and fresh UI.</p>
                    </div>
                    <a
                      href="#"
                      class="material-symbols-outlined font-size-20 text-dark reply"
                      v-b-tooltip.hover
                      title="Reply"
                      >reply</a
                    >
                    <div
                      class="dropdown cursor-pointer more"
                      v-b-tooltip.hover
                      title="More"
                    >
                      <div
                        class="lh-1"
                        id="post-option"
                        data-bs-toggle="dropdown"
                      >
                        <span class="material-symbols-outlined text-dark">
                          more_vert
                        </span>
                      </div>
                      <div
                        class="dropdown-menu dropdown-menu-right"
                        aria-labelledby="post-option"
                        style=""
                      >
                        <a class="dropdown-item" href="#"
                          ><span
                            class="material-symbols-outlined align-middle font-size-20 me-1"
                            >content_copy</span
                          >Copy message text</a
                        >
                        <a class="dropdown-item" href="#">
                          <span
                            class="material-symbols-outlined align-middle font-size-20 me-1"
                            >edit</span
                          >Edit</a
                        >
                      </div>
                    </div>
                  </div>
                </div>
              </div>
              <div class="iq-message-body iq-current-user">
                <div class="chat-profile text-center">
                  <img
                    src="../../../assets/images/chat/avatar/10.png"
                    alt="chat-user"
                    class="avatar-40 rounded-pill"
                    loading="lazy"
                  />
                  <small class="iq-chating p-0 mb-0 d-block">7:55</small>
                </div>
                <div class="iq-chat-text">
                  <div
                    class="d-flex align-items-center justify-content-end gap-1 gap-md-2"
                  >
                    <div
                      class="dropdown cursor-pointer more"
                      v-b-tooltip.hover
                      title="More"
                    >
                      <div
                        class="lh-1"
                        id="post-option"
                        data-bs-toggle="dropdown"
                      >
                        <span class="material-symbols-outlined text-dark">
                          more_vert
                        </span>
                      </div>
                      <div
                        class="dropdown-menu dropdown-menu-right"
                        aria-labelledby="post-option"
                        style=""
                      >
                        <a class="dropdown-item" href="#"
                          ><span
                            class="material-symbols-outlined align-middle font-size-20 me-1"
                            >content_copy</span
                          >Copy message text</a
                        >
                        <a class="dropdown-item" href="#">
                          <span
                            class="material-symbols-outlined align-middle font-size-20 me-1"
                            >edit</span
                          >Edit</a
                        >
                      </div>
                    </div>
                    <div class="iq-chating-content d-flex align-items-center">
                      <p class="mr-2 mb-0">Thanks, from ThemeForest.</p>
                    </div>
                  </div>
                </div>
              </div>
            </div>
            <div class="card-footer px-3 py-3 border-top rounded-0">
              <form class="d-flex align-items-center" action="#">
                <div class="chat-attagement d-flex">
                  <a href="#" class="d-flex align-items-center pe-3">
                    <svg
                      class="icon-24"
                      width="24"
                      viewBox="0 0 24 25"
                      fill="none"
                      xmlns="http://www.w3.org/2000/svg"
                    >
                      <g clip-path="url(#clip0_156_599)">
                        <path
                          d="M20.4853 4.01473C18.2188 1.74823 15.2053 0.5 12 0.5C8.79469 0.5 5.78119 1.74823 3.51473 4.01473C1.24819 6.28119 0 9.29469 0 12.5C0 15.7053 1.24819 18.7188 3.51473 20.9853C5.78119 23.2518 8.79469 24.5 12 24.5C15.2053 24.5 18.2188 23.2518 20.4853 20.9853C22.7518 18.7188 24 15.7053 24 12.5C24 9.29469 22.7518 6.28119 20.4853 4.01473ZM12 23.0714C6.17091 23.0714 1.42856 18.3291 1.42856 12.5C1.42856 6.67091 6.17091 1.92856 12 1.92856C17.8291 1.92856 22.5714 6.67091 22.5714 12.5C22.5714 18.3291 17.8291 23.0714 12 23.0714Z"
                          fill="currentcolor"
                        ></path>
                        <path
                          d="M9.40398 9.3309C8.23431 8.16114 6.33104 8.16123 5.16136 9.3309C4.88241 9.60981 4.88241 10.0621 5.16136 10.3411C5.44036 10.62 5.89266 10.62 6.17157 10.3411C6.78432 9.72836 7.78126 9.7284 8.39392 10.3411C8.53342 10.4806 8.71618 10.5503 8.89895 10.5503C9.08171 10.5503 9.26457 10.4806 9.40398 10.3411C9.68293 10.0621 9.68293 9.60986 9.40398 9.3309Z"
                          fill="currentcolor"
                        ></path>
                        <path
                          d="M18.8384 9.3309C17.6688 8.16123 15.7655 8.16114 14.5958 9.3309C14.3169 9.60981 14.3169 10.0621 14.5958 10.3411C14.8748 10.62 15.3271 10.62 15.606 10.3411C16.2187 9.72836 17.2156 9.72831 17.8284 10.3411C17.9679 10.4806 18.1506 10.5503 18.3334 10.5503C18.5162 10.5503 18.699 10.4806 18.8384 10.3411C19.1174 10.0621 19.1174 9.60986 18.8384 9.3309Z"
                          fill="currentcolor"
                        ></path>
                        <path
                          d="M18.3335 13.024H5.6668C5.2723 13.024 4.95251 13.3438 4.95251 13.7383C4.95251 17.6243 8.11409 20.7859 12.0001 20.7859C15.8862 20.7859 19.0477 17.6243 19.0477 13.7383C19.0477 13.3438 18.728 13.024 18.3335 13.024ZM12.0001 19.3573C9.14366 19.3573 6.77816 17.215 6.42626 14.4525H17.574C17.2221 17.215 14.8566 19.3573 12.0001 19.3573Z"
                          fill="currentcolor"
                        ></path>
                      </g>
                      <defs>
                        <clipPath>
                          <rect
                            width="24"
                            height="24"
                            fill="white"
                            transform="translate(0 0.5)"
                          ></rect>
                        </clipPath>
                      </defs>
                    </svg>
                  </a>
                  <a href="#" class="d-flex align-items-center pe-3">
                    <svg
                      class="icon-24"
                      width="18"
                      height="23"
                      viewBox="0 0 18 23"
                      fill="none"
                      xmlns="http://www.w3.org/2000/svg"
                    >
                      <path
                        d="M9.00021 21.5V18.3391"
                        stroke="currentcolor"
                        stroke-width="2"
                        stroke-linecap="round"
                        stroke-linejoin="round"
                      ></path>
                      <path
                        fill-rule="evenodd"
                        clip-rule="evenodd"
                        d="M9.00021 14.3481V14.3481C6.75611 14.3481 4.9384 12.5218 4.9384 10.2682V5.58095C4.9384 3.32732 6.75611 1.5 9.00021 1.5C11.2433 1.5 13.061 3.32732 13.061 5.58095V10.2682C13.061 12.5218 11.2433 14.3481 9.00021 14.3481Z"
                        stroke="currentcolor"
                        stroke-width="2"
                        stroke-linecap="round"
                        stroke-linejoin="round"
                      ></path>
                      <path
                        d="M17 10.3006C17 14.7394 13.418 18.3383 9 18.3383C4.58093 18.3383 1 14.7394 1 10.3006"
                        stroke="currentcolor"
                        stroke-width="2"
                        stroke-linecap="round"
                        stroke-linejoin="round"
                      ></path>
                      <path
                        d="M11.0689 6.25579H13.0585"
                        stroke="currentcolor"
                        stroke-width="2"
                        stroke-linecap="round"
                        stroke-linejoin="round"
                      ></path>
                      <path
                        d="M10.0704 9.59344H13.0605"
                        stroke="currentcolor"
                        stroke-width="2"
                        stroke-linecap="round"
                        stroke-linejoin="round"
                      ></path>
                    </svg>
                  </a>
                </div>
                <input
                  type="text"
                  class="form-control me-3"
                  placeholder="Type your message"
                />
                <button
                  type="submit"
                  class="btn btn-primary d-flex align-items-center"
                >
                  <svg
                    class="icon-20"
                    width="18"
                    viewBox="0 0 20 21"
                    fill="none"
                    xmlns="http://www.w3.org/2000/svg"
                  >
                    <path
                      d="M13.8325 6.67463L8.10904 12.4592L1.59944 8.38767C0.66675 7.80414 0.860765 6.38744 1.91572 6.07893L17.3712 1.55277C18.3373 1.26963 19.2326 2.17283 18.9456 3.142L14.3731 18.5868C14.0598 19.6432 12.6512 19.832 12.0732 18.8953L8.10601 12.4602"
                      stroke="currentcolor"
                      stroke-width="2"
                      stroke-linecap="round"
                      stroke-linejoin="round"
                    ></path></svg
                  ><span class="d-none d-lg-block ms-1">Send</span>
                </button>
              </form>
            </div>
          </div>
          <div
            class="card tab-pane mb-0 fade"
            id="user-content-106"
            role="tabpanel"
          >
            <div class="chat-head">
              <header
                class="d-flex justify-content-between align-items-center pt-3 ps-3 pe-3 pb-3"
              >
                <div class="d-flex align-items-center gap-3">
                  <div class="d-block d-xl-none">
                    <button
                      class="btn btn-sm btn-primary rounded btn-icon"
                      data-toggle="sidebar"
                      data-active="true"
                    >
                      <span class="btn-inner">
                        <svg class="icon-rtl" width="20px" viewBox="0 0 24 24">
                          <path
                            fill="currentColor"
                            d="M4,11V13H16L10.5,18.5L11.92,19.92L19.84,12L11.92,4.08L10.5,5.5L16,11H4Z"
                          ></path>
                        </svg>
                      </span>
                    </button>
                  </div>
                  <div class="avatar chat-user-profile m-0">
                    <img
                      src="../../../assets/images/chat/avatar/06.png"
                      alt="avatar"
                      class="avatar-50 rounded-pill"
                      loading="lazy"
                    />
                    <div class="iq-profile-badge bg-success"></div>
                  </div>
                  <div>
                    <h5 class="mb-0">Monty Carlo</h5>
                    <small class="text-capitalize fw-500">Online</small>
                  </div>
                </div>

                <div class="chat-header-icons d-inline-flex ms-auto">
                  <a
                    href="#"
                    class="chat-icon-phone bg-primary-subtle d-flex align-items-center justify-content-center"
                  >
                    <i class="material-symbols-outlined md-18">phone</i>
                  </a>
                  <a
                    href="#"
                    class="chat-icon-video bg-primary-subtle d-flex align-items-center justify-content-center"
                  >
                    <i class="material-symbols-outlined md-18">videocam</i>
                  </a>
                  <a
                    href="#"
                    class="chat-icon-delete bg-primary-subtle d-flex align-items-center justify-content-center"
                  >
                    <i class="material-symbols-outlined md-18">delete</i>
                  </a>
                  <span
                    class="dropdown bg-primary-subtle d-flex align-items-center justify-content-center"
                  >
                    <svg
                      class="icon-20 nav-hide-arrow cursor-pointer pe-0"
                      id="dropdownMenuButton-13"
                      data-bs-toggle="dropdown"
                      aria-haspopup="true"
                      aria-expanded="false"
                      role="menu"
                      width="24"
                      xmlns="http://www.w3.org/2000/svg"
                      viewBox="0 0 24 24"
                      fill="none"
                    >
                      <path
                        fill-rule="evenodd"
                        clip-rule="evenodd"
                        d="M10 20.6788C10 21.9595 11.0378 23 12.3113 23C13.5868 23 14.6265 21.9595 14.6265 20.6788C14.6265 19.3981 13.5868 18.3576 12.3113 18.3576C11.0378 18.3576 10 19.3981 10 20.6788ZM10 12.0005C10 13.2812 11.0378 14.3217 12.3113 14.3217C13.5868 14.3217 14.6265 13.2812 14.6265 12.0005C14.6265 10.7198 13.5868 9.67929 12.3113 9.67929C11.0378 9.67929 10 10.7198 10 12.0005ZM12.3113 5.64239C11.0378 5.64239 10 4.60192 10 3.3212C10 2.04047 11.0378 1 12.3113 1C13.5868 1 14.6265 2.04047 14.6265 3.3212C14.6265 4.60192 13.5868 5.64239 12.3113 5.64239Z"
                        fill="currentColor"
                      ></path>
                    </svg>
                    <span
                      class="dropdown-menu dropdown-menu-right"
                      aria-labelledby="dropdownMenuButton-13"
                    >
                      <a
                        class="dropdown-item d-flex align-items-center"
                        href="#"
                        ><i class="material-symbols-outlined md-18 me-1"
                          >push_pin</i
                        >Pin to top</a
                      >
                      <a
                        class="dropdown-item d-flex align-items-center"
                        href="#"
                        ><i class="material-symbols-outlined md-18 me-1"
                          >delete_outline</i
                        >Delete chat</a
                      >
                      <a
                        class="dropdown-item d-flex align-items-center"
                        href="#"
                        ><i class="material-symbols-outlined md-18 me-1"
                          >watch_later</i
                        >Block</a
                      >
                    </span>
                  </span>
                </div>
              </header>
            </div>
            <div class="card-body chat-body bg-body">
              <div class="chat-day-title">
                <span class="main-title">Feb 1,2021</span>
              </div>
              <div class="iq-message-body iq-current-user">
                <div class="chat-profile text-center">
                  <img
                    src="../../../assets/images/chat/avatar/10.png"
                    alt="chat-user"
                    class="avatar-40 rounded-pill"
                    loading="lazy"
                  />
                  <small class="iq-chating p-0 mb-0 d-block">16:34</small>
                </div>
                <div class="iq-chat-text">
                  <div
                    class="d-flex align-items-center justify-content-end gap-1 gap-md-2"
                  >
                    <div
                      class="dropdown cursor-pointer more"
                      v-b-tooltip.hover
                      title="More"
                    >
                      <div
                        class="lh-1"
                        id="post-option"
                        data-bs-toggle="dropdown"
                      >
                        <span class="material-symbols-outlined text-dark">
                          more_vert
                        </span>
                      </div>
                      <div
                        class="dropdown-menu dropdown-menu-right"
                        aria-labelledby="post-option"
                        style=""
                      >
                        <a class="dropdown-item" href="#"
                          ><span
                            class="material-symbols-outlined align-middle font-size-20 me-1"
                            >content_copy</span
                          >Copy message text</a
                        >
                        <a class="dropdown-item" href="#">
                          <span
                            class="material-symbols-outlined align-middle font-size-20 me-1"
                            >edit</span
                          >Edit</a
                        >
                      </div>
                    </div>
                    <div class="iq-chating-content d-flex align-items-center">
                      <p class="mr-2 mb-0">Hey, how are you??</p>
                    </div>
                  </div>
                </div>
              </div>
              <div class="iq-message-body iq-other-user">
                <div class="chat-profile text-center">
                  <img
                    src="../../../assets/images/chat/avatar/06.png"
                    alt="chat-user"
                    class="avatar-40 rounded-pill"
                    loading="lazy"
                  />
                  <small class="iq-chating p-0 mb-0 d-block">16:40</small>
                </div>
                <div class="iq-chat-text">
                  <div
                    class="d-flex align-items-center justify-content-start gap-md-2"
                  >
                    <div class="iq-chating-content d-flex align-items-center">
                      <p class="mr-2 mb-0">
                        Hey, I am looking for the best dashboard template. Could
                        you please help me to find it out?
                      </p>
                    </div>
                    <a
                      href="#"
                      class="material-symbols-outlined font-size-20 text-dark reply"
                      v-b-tooltip.hover
                      title="Reply"
                      >reply</a
                    >
                    <div
                      class="dropdown cursor-pointer more"
                      v-b-tooltip.hover
                      title="More"
                    >
                      <div
                        class="lh-1"
                        id="post-option"
                        data-bs-toggle="dropdown"
                      >
                        <span class="material-symbols-outlined text-dark">
                          more_vert
                        </span>
                      </div>
                      <div
                        class="dropdown-menu dropdown-menu-right"
                        aria-labelledby="post-option"
                        style=""
                      >
                        <a class="dropdown-item" href="#"
                          ><span
                            class="material-symbols-outlined align-middle font-size-20 me-1"
                            >content_copy</span
                          >Copy message text</a
                        >
                        <a class="dropdown-item" href="#">
                          <span
                            class="material-symbols-outlined align-middle font-size-20 me-1"
                            >edit</span
                          >Edit</a
                        >
                      </div>
                    </div>
                  </div>
                </div>
              </div>
              <div class="iq-message-body iq-current-user">
                <div class="chat-profile text-center">
                  <img
                    src="../../../assets/images/chat/avatar/10.png"
                    alt="chat-user"
                    class="avatar-40 rounded-pill"
                    loading="lazy"
                  />
                  <small class="iq-chating p-0 mb-0 d-block">16:42</small>
                </div>
                <div class="iq-chat-text">
                  <div
                    class="d-flex align-items-center justify-content-end gap-1 gap-md-2"
                  >
                    <div
                      class="dropdown cursor-pointer more"
                      v-b-tooltip.hover
                      title="More"
                    >
                      <div
                        class="lh-1"
                        id="post-option"
                        data-bs-toggle="dropdown"
                      >
                        <span class="material-symbols-outlined text-dark">
                          more_vert
                        </span>
                      </div>
                      <div
                        class="dropdown-menu dropdown-menu-right"
                        aria-labelledby="post-option"
                        style=""
                      >
                        <a class="dropdown-item" href="#"
                          ><span
                            class="material-symbols-outlined align-middle font-size-20 me-1"
                            >content_copy</span
                          >Copy message text</a
                        >
                        <a class="dropdown-item" href="#">
                          <span
                            class="material-symbols-outlined align-middle font-size-20 me-1"
                            >edit</span
                          >Edit</a
                        >
                      </div>
                    </div>
                    <div class="iq-chating-content d-flex align-items-center">
                      <p class="mr-2 mb-0">
                        Absolutely Yes! SocialV is the Responsive Bootstrap 5
                        Admin Dashboard Template.
                      </p>
                    </div>
                  </div>
                </div>
              </div>
              <div class="iq-message-body iq-other-user">
                <div class="chat-profile text-center">
                  <img
                    src="../../../assets/images/chat/avatar/06.png"
                    alt="chat-user"
                    class="avatar-40 rounded-pill"
                    loading="lazy"
                  />
                  <small class="iq-chating p-0 mb-0 d-block">17:00</small>
                </div>
                <div class="iq-chat-text">
                  <div
                    class="d-flex align-items-center justify-content-start gap-md-2"
                  >
                    <div class="iq-chating-content d-flex align-items-center">
                      <p class="mr-2 mb-0">Looks clean and fresh UI.</p>
                    </div>
                    <a
                      href="#"
                      class="material-symbols-outlined font-size-20 text-dark reply"
                      v-b-tooltip.hover
                      title="Reply"
                      >reply</a
                    >
                    <div
                      class="dropdown cursor-pointer more"
                      v-b-tooltip.hover
                      title="More"
                    >
                      <div
                        class="lh-1"
                        id="post-option"
                        data-bs-toggle="dropdown"
                      >
                        <span class="material-symbols-outlined text-dark">
                          more_vert
                        </span>
                      </div>
                      <div
                        class="dropdown-menu dropdown-menu-right"
                        aria-labelledby="post-option"
                        style=""
                      >
                        <a class="dropdown-item" href="#"
                          ><span
                            class="material-symbols-outlined align-middle font-size-20 me-1"
                            >content_copy</span
                          >Copy message text</a
                        >
                        <a class="dropdown-item" href="#">
                          <span
                            class="material-symbols-outlined align-middle font-size-20 me-1"
                            >edit</span
                          >Edit</a
                        >
                      </div>
                    </div>
                  </div>
                </div>
              </div>
              <div class="iq-message-body iq-other-user">
                <div class="chat-profile text-center">
                  <img
                    src="../../../assets/images/chat/avatar/06.png"
                    alt="chat-user"
                    class="avatar-40 rounded-pill"
                    loading="lazy"
                  />
                  <small class="iq-chating p-0 mb-0 d-block">17:01</small>
                </div>
                <div class="iq-chat-text">
                  <div
                    class="d-flex align-items-center justify-content-start gap-md-2"
                  >
                    <div class="iq-chating-content d-flex align-items-center">
                      <p class="mr-2 mb-0">I will purchase it for sure.</p>
                    </div>
                    <a
                      href="#"
                      class="material-symbols-outlined font-size-20 text-dark reply"
                      v-b-tooltip.hover
                      title="Reply"
                      >reply</a
                    >
                    <div
                      class="dropdown cursor-pointer more"
                      v-b-tooltip.hover
                      title="More"
                    >
                      <div
                        class="lh-1"
                        id="post-option"
                        data-bs-toggle="dropdown"
                      >
                        <span class="material-symbols-outlined text-dark">
                          more_vert
                        </span>
                      </div>
                      <div
                        class="dropdown-menu dropdown-menu-right"
                        aria-labelledby="post-option"
                        style=""
                      >
                        <a class="dropdown-item" href="#"
                          ><span
                            class="material-symbols-outlined align-middle font-size-20 me-1"
                            >content_copy</span
                          >Copy message text</a
                        >
                        <a class="dropdown-item" href="#">
                          <span
                            class="material-symbols-outlined align-middle font-size-20 me-1"
                            >edit</span
                          >Edit</a
                        >
                      </div>
                    </div>
                  </div>
                </div>
              </div>
              <div class="iq-message-body iq-current-user">
                <div class="chat-profile text-center">
                  <img
                    src="../../../assets/images/chat/avatar/10.png"
                    alt="chat-user"
                    class="avatar-40 rounded-pill"
                    loading="lazy"
                  />
                  <small class="iq-chating p-0 mb-0 d-block">17:10</small>
                </div>
                <div class="iq-chat-text">
                  <div
                    class="d-flex align-items-center justify-content-end gap-1 gap-md-2"
                  >
                    <div
                      class="dropdown cursor-pointer more"
                      v-b-tooltip.hover
                      title="More"
                    >
                      <div
                        class="lh-1"
                        id="post-option"
                        data-bs-toggle="dropdown"
                      >
                        <span class="material-symbols-outlined text-dark">
                          more_vert
                        </span>
                      </div>
                      <div
                        class="dropdown-menu dropdown-menu-right"
                        aria-labelledby="post-option"
                        style=""
                      >
                        <a class="dropdown-item" href="#"
                          ><span
                            class="material-symbols-outlined align-middle font-size-20 me-1"
                            >content_copy</span
                          >Copy message text</a
                        >
                        <a class="dropdown-item" href="#">
                          <span
                            class="material-symbols-outlined align-middle font-size-20 me-1"
                            >edit</span
                          >Edit</a
                        >
                      </div>
                    </div>
                    <div class="iq-chating-content d-flex align-items-center">
                      <p class="mr-2 mb-0">Ok Cool!!</p>
                    </div>
                  </div>
                </div>
              </div>
              <div class="chat-day-title">
                <span class="main-title">Today</span>
              </div>
              <div class="iq-message-body iq-current-user">
                <div class="chat-profile text-center">
                  <img
                    src="../../../assets/images/chat/avatar/06.png"
                    alt="chat-user"
                    class="avatar-40 rounded-pill"
                    loading="lazy"
                  />
                  <small class="iq-chating p-0 mb-0 d-block">12:00</small>
                </div>
                <div class="iq-chat-text">
                  <div
                    class="d-flex align-items-center justify-content-end gap-1 gap-md-2"
                  >
                    <div
                      class="dropdown cursor-pointer more"
                      v-b-tooltip.hover
                      title="More"
                    >
                      <div
                        class="lh-1"
                        id="post-option"
                        data-bs-toggle="dropdown"
                      >
                        <span class="material-symbols-outlined text-dark">
                          more_vert
                        </span>
                      </div>
                      <div
                        class="dropdown-menu dropdown-menu-right"
                        aria-labelledby="post-option"
                        style=""
                      >
                        <a class="dropdown-item" href="#"
                          ><span
                            class="material-symbols-outlined align-middle font-size-20 me-1"
                            >content_copy</span
                          >Copy message text</a
                        >
                        <a class="dropdown-item" href="#">
                          <span
                            class="material-symbols-outlined align-middle font-size-20 me-1"
                            >edit</span
                          >Edit</a
                        >
                      </div>
                    </div>
                    <div class="iq-chating-content d-flex align-items-center">
                      <p class="mr-2 mb-0">This are nice pics.</p>
                    </div>
                  </div>
                </div>
              </div>
              <div class="iq-message-body iq-other-user">
                <div class="chat-profile text-center">
                  <img
                    src="../../../assets/images/chat/avatar/06.png"
                    alt="chat-user"
                    class="avatar-40 rounded-pill"
                    loading="lazy"
                  />
                  <small class="iq-chating p-0 mb-0 d-block">12:05</small>
                </div>
                <div class="iq-chat-text">
                  <div
                    class="d-flex align-items-center justify-content-start gap-md-2"
                  >
                    <div class="iq-chating-content d-flex align-items-center">
                      <p class="mr-2 mb-0">
                        I want some teachnic for fast code can you help?
                      </p>
                    </div>
                    <a
                      href="#"
                      class="material-symbols-outlined font-size-20 text-dark reply"
                      v-b-tooltip.hover
                      title="Reply"
                      >reply</a
                    >
                    <div
                      class="dropdown cursor-pointer more"
                      v-b-tooltip.hover
                      title="More"
                    >
                      <div
                        class="lh-1"
                        id="post-option"
                        data-bs-toggle="dropdown"
                      >
                        <span class="material-symbols-outlined text-dark">
                          more_vert
                        </span>
                      </div>
                      <div
                        class="dropdown-menu dropdown-menu-right"
                        aria-labelledby="post-option"
                        style=""
                      >
                        <a class="dropdown-item" href="#"
                          ><span
                            class="material-symbols-outlined align-middle font-size-20 me-1"
                            >content_copy</span
                          >Copy message text</a
                        >
                        <a class="dropdown-item" href="#">
                          <span
                            class="material-symbols-outlined align-middle font-size-20 me-1"
                            >edit</span
                          >Edit</a
                        >
                      </div>
                    </div>
                  </div>
                </div>
              </div>
              <div class="iq-message-body iq-current-user">
                <div class="chat-profile text-center">
                  <img
                    src="../../../assets/images/chat/avatar/10.png"
                    alt="chat-user"
                    class="avatar-40 rounded-pill"
                    loading="lazy"
                  />
                  <small class="iq-chating p-0 mb-0 d-block">13:10</small>
                </div>
                <div class="iq-chat-text">
                  <div
                    class="d-flex align-items-center justify-content-end gap-1 gap-md-2"
                  >
                    <div
                      class="dropdown cursor-pointer more"
                      v-b-tooltip.hover
                      title="More"
                    >
                      <div
                        class="lh-1"
                        id="post-option"
                        data-bs-toggle="dropdown"
                      >
                        <span class="material-symbols-outlined text-dark">
                          more_vert
                        </span>
                      </div>
                      <div
                        class="dropdown-menu dropdown-menu-right"
                        aria-labelledby="post-option"
                        style=""
                      >
                        <a class="dropdown-item" href="#"
                          ><span
                            class="material-symbols-outlined align-middle font-size-20 me-1"
                            >content_copy</span
                          >Copy message text</a
                        >
                        <a class="dropdown-item" href="#">
                          <span
                            class="material-symbols-outlined align-middle font-size-20 me-1"
                            >edit</span
                          >Edit</a
                        >
                      </div>
                    </div>
                    <div class="iq-chating-content d-flex align-items-center">
                      <p class="mr-2 mb-0">Yes Look clean and fresh UI.</p>
                    </div>
                  </div>
                </div>
              </div>
              <div></div>
              <div class="iq-message-body iq-other-user">
                <div class="chat-profile text-center">
                  <img
                    src="../../../assets/images/chat/avatar/06.png"
                    alt="chat-user"
                    class="avatar-40 rounded-pill"
                    loading="lazy"
                  />
                  <small class="iq-chating p-0 mb-0 d-block">15:00</small>
                </div>
                <div class="iq-chat-text">
                  <div
                    class="d-flex align-items-center justify-content-start gap-md-2"
                  >
                    <div
                      class="iq-chating-content d-flex align-items-center iq-image"
                    >
                      <p class="mr-2 mb-0"></p>
                      <div class="">
                        <a
                          data-fslightbox="gallery"
                          href="../../../assets/images/chat/avatar/05.png"
                        >
                          <img
                            src="../../../assets/images/chat/avatar/05.png"
                            class="rounded iq-single-image"
                            alt="chat-user"
                            loading="lazy"
                          />
                        </a>
                      </div>
                    </div>
                    <a
                      href="#"
                      class="material-symbols-outlined font-size-20 text-dark reply"
                      v-b-tooltip.hover
                      title="Reply"
                      >reply</a
                    >
                    <div
                      class="dropdown cursor-pointer more"
                      v-b-tooltip.hover
                      title="More"
                    >
                      <div
                        class="lh-1"
                        id="post-option"
                        data-bs-toggle="dropdown"
                      >
                        <span class="material-symbols-outlined text-dark">
                          more_vert
                        </span>
                      </div>
                      <div
                        class="dropdown-menu dropdown-menu-right"
                        aria-labelledby="post-option"
                        style=""
                      >
                        <a class="dropdown-item" href="#"
                          ><span
                            class="material-symbols-outlined align-middle font-size-20 me-1"
                            >content_copy</span
                          >Copy message text</a
                        >
                        <a class="dropdown-item" href="#">
                          <span
                            class="material-symbols-outlined align-middle font-size-20 me-1"
                            >edit</span
                          >Edit</a
                        >
                      </div>
                    </div>
                  </div>
                </div>
              </div>
            </div>
            <div class="card-footer px-3 py-3 border-top rounded-0">
              <form class="d-flex align-items-center" action="#">
                <div class="chat-attagement d-flex">
                  <a href="#" class="d-flex align-items-center pe-3">
                    <svg
                      class="icon-24"
                      width="24"
                      viewBox="0 0 24 25"
                      fill="none"
                      xmlns="http://www.w3.org/2000/svg"
                    >
                      <g clip-path="url(#clip0_156_599)">
                        <path
                          d="M20.4853 4.01473C18.2188 1.74823 15.2053 0.5 12 0.5C8.79469 0.5 5.78119 1.74823 3.51473 4.01473C1.24819 6.28119 0 9.29469 0 12.5C0 15.7053 1.24819 18.7188 3.51473 20.9853C5.78119 23.2518 8.79469 24.5 12 24.5C15.2053 24.5 18.2188 23.2518 20.4853 20.9853C22.7518 18.7188 24 15.7053 24 12.5C24 9.29469 22.7518 6.28119 20.4853 4.01473ZM12 23.0714C6.17091 23.0714 1.42856 18.3291 1.42856 12.5C1.42856 6.67091 6.17091 1.92856 12 1.92856C17.8291 1.92856 22.5714 6.67091 22.5714 12.5C22.5714 18.3291 17.8291 23.0714 12 23.0714Z"
                          fill="currentcolor"
                        ></path>
                        <path
                          d="M9.40398 9.3309C8.23431 8.16114 6.33104 8.16123 5.16136 9.3309C4.88241 9.60981 4.88241 10.0621 5.16136 10.3411C5.44036 10.62 5.89266 10.62 6.17157 10.3411C6.78432 9.72836 7.78126 9.7284 8.39392 10.3411C8.53342 10.4806 8.71618 10.5503 8.89895 10.5503C9.08171 10.5503 9.26457 10.4806 9.40398 10.3411C9.68293 10.0621 9.68293 9.60986 9.40398 9.3309Z"
                          fill="currentcolor"
                        ></path>
                        <path
                          d="M18.8384 9.3309C17.6688 8.16123 15.7655 8.16114 14.5958 9.3309C14.3169 9.60981 14.3169 10.0621 14.5958 10.3411C14.8748 10.62 15.3271 10.62 15.606 10.3411C16.2187 9.72836 17.2156 9.72831 17.8284 10.3411C17.9679 10.4806 18.1506 10.5503 18.3334 10.5503C18.5162 10.5503 18.699 10.4806 18.8384 10.3411C19.1174 10.0621 19.1174 9.60986 18.8384 9.3309Z"
                          fill="currentcolor"
                        ></path>
                        <path
                          d="M18.3335 13.024H5.6668C5.2723 13.024 4.95251 13.3438 4.95251 13.7383C4.95251 17.6243 8.11409 20.7859 12.0001 20.7859C15.8862 20.7859 19.0477 17.6243 19.0477 13.7383C19.0477 13.3438 18.728 13.024 18.3335 13.024ZM12.0001 19.3573C9.14366 19.3573 6.77816 17.215 6.42626 14.4525H17.574C17.2221 17.215 14.8566 19.3573 12.0001 19.3573Z"
                          fill="currentcolor"
                        ></path>
                      </g>
                      <defs>
                        <clipPath>
                          <rect
                            width="24"
                            height="24"
                            fill="white"
                            transform="translate(0 0.5)"
                          ></rect>
                        </clipPath>
                      </defs>
                    </svg>
                  </a>
                  <a href="#" class="d-flex align-items-center pe-3">
                    <svg
                      class="icon-24"
                      width="18"
                      height="23"
                      viewBox="0 0 18 23"
                      fill="none"
                      xmlns="http://www.w3.org/2000/svg"
                    >
                      <path
                        d="M9.00021 21.5V18.3391"
                        stroke="currentcolor"
                        stroke-width="2"
                        stroke-linecap="round"
                        stroke-linejoin="round"
                      ></path>
                      <path
                        fill-rule="evenodd"
                        clip-rule="evenodd"
                        d="M9.00021 14.3481V14.3481C6.75611 14.3481 4.9384 12.5218 4.9384 10.2682V5.58095C4.9384 3.32732 6.75611 1.5 9.00021 1.5C11.2433 1.5 13.061 3.32732 13.061 5.58095V10.2682C13.061 12.5218 11.2433 14.3481 9.00021 14.3481Z"
                        stroke="currentcolor"
                        stroke-width="2"
                        stroke-linecap="round"
                        stroke-linejoin="round"
                      ></path>
                      <path
                        d="M17 10.3006C17 14.7394 13.418 18.3383 9 18.3383C4.58093 18.3383 1 14.7394 1 10.3006"
                        stroke="currentcolor"
                        stroke-width="2"
                        stroke-linecap="round"
                        stroke-linejoin="round"
                      ></path>
                      <path
                        d="M11.0689 6.25579H13.0585"
                        stroke="currentcolor"
                        stroke-width="2"
                        stroke-linecap="round"
                        stroke-linejoin="round"
                      ></path>
                      <path
                        d="M10.0704 9.59344H13.0605"
                        stroke="currentcolor"
                        stroke-width="2"
                        stroke-linecap="round"
                        stroke-linejoin="round"
                      ></path>
                    </svg>
                  </a>
                </div>
                <input
                  type="text"
                  class="form-control me-3"
                  placeholder="Type your message"
                />
                <button
                  type="submit"
                  class="btn btn-primary d-flex align-items-center"
                >
                  <svg
                    class="icon-20"
                    width="18"
                    viewBox="0 0 20 21"
                    fill="none"
                    xmlns="http://www.w3.org/2000/svg"
                  >
                    <path
                      d="M13.8325 6.67463L8.10904 12.4592L1.59944 8.38767C0.66675 7.80414 0.860765 6.38744 1.91572 6.07893L17.3712 1.55277C18.3373 1.26963 19.2326 2.17283 18.9456 3.142L14.3731 18.5868C14.0598 19.6432 12.6512 19.832 12.0732 18.8953L8.10601 12.4602"
                      stroke="currentcolor"
                      stroke-width="2"
                      stroke-linecap="round"
                      stroke-linejoin="round"
                    ></path></svg
                  ><span class="d-none d-lg-block ms-1">Send</span>
                </button>
              </form>
            </div>
          </div>
          <div
            class="card tab-pane mb-0 fade"
            id="user-content-107"
            role="tabpanel"
          >
            <div class="chat-head">
              <header
                class="d-flex justify-content-between align-items-center pt-3 ps-3 pe-3 pb-3"
              >
                <div class="d-flex align-items-center gap-3">
                  <div class="d-block d-xl-none">
                    <button
                      class="btn btn-sm btn-primary rounded btn-icon"
                      data-toggle="sidebar"
                      data-active="true"
                    >
                      <span class="btn-inner">
                        <svg class="icon-rtl" width="20px" viewBox="0 0 24 24">
                          <path
                            fill="currentColor"
                            d="M4,11V13H16L10.5,18.5L11.92,19.92L19.84,12L11.92,4.08L10.5,5.5L16,11H4Z"
                          ></path>
                        </svg>
                      </span>
                    </button>
                  </div>
                  <div class="avatar chat-user-profile m-0">
                    <img
                      src="../../../assets/images/chat/avatar/07.png"
                      alt="avatar"
                      class="avatar-50 rounded-pill"
                      loading="lazy"
                    />
                    <div class="iq-profile-badge bg-success"></div>
                  </div>
                  <div>
                    <h5 class="mb-0">Paige Turner</h5>
                    <small class="text-capitalize fw-500">Online</small>
                  </div>
                </div>

                <div class="chat-header-icons d-inline-flex ms-auto">
                  <a
                    href="#"
                    class="chat-icon-phone bg-primary-subtle d-flex align-items-center justify-content-center"
                  >
                    <i class="material-symbols-outlined md-18">phone</i>
                  </a>
                  <a
                    href="#"
                    class="chat-icon-video bg-primary-subtle d-flex align-items-center justify-content-center"
                  >
                    <i class="material-symbols-outlined md-18">videocam</i>
                  </a>
                  <a
                    href="#"
                    class="chat-icon-delete bg-primary-subtle d-flex align-items-center justify-content-center"
                  >
                    <i class="material-symbols-outlined md-18">delete</i>
                  </a>
                  <span
                    class="dropdown bg-primary-subtle d-flex align-items-center justify-content-center"
                  >
                    <svg
                      class="icon-20 nav-hide-arrow cursor-pointer pe-0"
                      id="dropdownMenuButton-14"
                      data-bs-toggle="dropdown"
                      aria-haspopup="true"
                      aria-expanded="false"
                      role="menu"
                      width="24"
                      xmlns="http://www.w3.org/2000/svg"
                      viewBox="0 0 24 24"
                      fill="none"
                    >
                      <path
                        fill-rule="evenodd"
                        clip-rule="evenodd"
                        d="M10 20.6788C10 21.9595 11.0378 23 12.3113 23C13.5868 23 14.6265 21.9595 14.6265 20.6788C14.6265 19.3981 13.5868 18.3576 12.3113 18.3576C11.0378 18.3576 10 19.3981 10 20.6788ZM10 12.0005C10 13.2812 11.0378 14.3217 12.3113 14.3217C13.5868 14.3217 14.6265 13.2812 14.6265 12.0005C14.6265 10.7198 13.5868 9.67929 12.3113 9.67929C11.0378 9.67929 10 10.7198 10 12.0005ZM12.3113 5.64239C11.0378 5.64239 10 4.60192 10 3.3212C10 2.04047 11.0378 1 12.3113 1C13.5868 1 14.6265 2.04047 14.6265 3.3212C14.6265 4.60192 13.5868 5.64239 12.3113 5.64239Z"
                        fill="currentColor"
                      ></path>
                    </svg>
                    <span
                      class="dropdown-menu dropdown-menu-right"
                      aria-labelledby="dropdownMenuButton-14"
                    >
                      <a
                        class="dropdown-item d-flex align-items-center"
                        href="#"
                        ><i class="material-symbols-outlined md-18 me-1"
                          >push_pin</i
                        >Pin to top</a
                      >
                      <a
                        class="dropdown-item d-flex align-items-center"
                        href="#"
                        ><i class="material-symbols-outlined md-18 me-1"
                          >delete_outline</i
                        >Delete chat</a
                      >
                      <a
                        class="dropdown-item d-flex align-items-center"
                        href="#"
                        ><i class="material-symbols-outlined md-18 me-1"
                          >watch_later</i
                        >Block</a
                      >
                    </span>
                  </span>
                </div>
              </header>
            </div>
            <div class="card-body chat-body bg-body">
              <div class="chat-day-title">
                <span class="main-title">Feb 1,2021</span>
              </div>
              <div class="iq-message-body iq-current-user">
                <div class="chat-profile text-center">
                  <img
                    src="../../../assets/images/chat/avatar/10.png"
                    alt="chat-user"
                    class="avatar-40 rounded-pill"
                    loading="lazy"
                  />
                  <small class="iq-chating p-0 mb-0 d-block">16:34</small>
                </div>
                <div class="iq-chat-text">
                  <div
                    class="d-flex align-items-center justify-content-end gap-1 gap-md-2"
                  >
                    <div
                      class="dropdown cursor-pointer more"
                      v-b-tooltip.hover
                      title="More"
                    >
                      <div
                        class="lh-1"
                        id="post-option"
                        data-bs-toggle="dropdown"
                      >
                        <span class="material-symbols-outlined text-dark">
                          more_vert
                        </span>
                      </div>
                      <div
                        class="dropdown-menu dropdown-menu-right"
                        aria-labelledby="post-option"
                        style=""
                      >
                        <a class="dropdown-item" href="#"
                          ><span
                            class="material-symbols-outlined align-middle font-size-20 me-1"
                            >content_copy</span
                          >Copy message text</a
                        >
                        <a class="dropdown-item" href="#">
                          <span
                            class="material-symbols-outlined align-middle font-size-20 me-1"
                            >edit</span
                          >Edit</a
                        >
                      </div>
                    </div>
                    <div class="iq-chating-content d-flex align-items-center">
                      <p class="mr-2 mb-0">
                        How can we help? We're here for you! 😄
                      </p>
                    </div>
                  </div>
                </div>
              </div>
              <div class="iq-message-body iq-other-user">
                <div class="chat-profile text-center">
                  <img
                    src="../../../assets/images/chat/avatar/07.png"
                    alt="chat-user"
                    class="avatar-40 rounded-pill"
                    loading="lazy"
                  />
                  <small class="iq-chating p-0 mb-0 d-block">16:40</small>
                </div>
                <div class="iq-chat-text">
                  <div
                    class="d-flex align-items-center justify-content-start gap-md-2"
                  >
                    <div class="iq-chating-content d-flex align-items-center">
                      <p class="mr-2 mb-0">
                        Hey John, I am looking for the best admin template.
                        Could you please help me to find it out? 🤔
                      </p>
                    </div>
                    <a
                      href="#"
                      class="material-symbols-outlined font-size-20 text-dark reply"
                      v-b-tooltip.hover
                      title="Reply"
                      >reply</a
                    >
                    <div
                      class="dropdown cursor-pointer more"
                      v-b-tooltip.hover
                      title="More"
                    >
                      <div
                        class="lh-1"
                        id="post-option"
                        data-bs-toggle="dropdown"
                      >
                        <span class="material-symbols-outlined text-dark">
                          more_vert
                        </span>
                      </div>
                      <div
                        class="dropdown-menu dropdown-menu-right"
                        aria-labelledby="post-option"
                        style=""
                      >
                        <a class="dropdown-item" href="#"
                          ><span
                            class="material-symbols-outlined align-middle font-size-20 me-1"
                            >content_copy</span
                          >Copy message text</a
                        >
                        <a class="dropdown-item" href="#">
                          <span
                            class="material-symbols-outlined align-middle font-size-20 me-1"
                            >edit</span
                          >Edit</a
                        >
                      </div>
                    </div>
                  </div>
                </div>
              </div>
              <div class="iq-message-body iq-current-user">
                <div class="chat-profile text-center">
                  <img
                    src="../../../assets/images/chat/avatar/10.png"
                    alt="chat-user"
                    class="avatar-40 rounded-pill"
                    loading="lazy"
                  />
                  <small class="iq-chating p-0 mb-0 d-block">6:49</small>
                </div>
                <div class="iq-chat-text">
                  <div
                    class="d-flex align-items-center justify-content-end gap-1 gap-md-2"
                  >
                    <div
                      class="dropdown cursor-pointer more"
                      v-b-tooltip.hover
                      title="More"
                    >
                      <div
                        class="lh-1"
                        id="post-option"
                        data-bs-toggle="dropdown"
                      >
                        <span class="material-symbols-outlined text-dark">
                          more_vert
                        </span>
                      </div>
                      <div
                        class="dropdown-menu dropdown-menu-right"
                        aria-labelledby="post-option"
                        style=""
                      >
                        <a class="dropdown-item" href="#"
                          ><span
                            class="material-symbols-outlined align-middle font-size-20 me-1"
                            >content_copy</span
                          >Copy message text</a
                        >
                        <a class="dropdown-item" href="#">
                          <span
                            class="material-symbols-outlined align-middle font-size-20 me-1"
                            >edit</span
                          >Edit</a
                        >
                      </div>
                    </div>
                    <div class="iq-chating-content d-flex align-items-center">
                      <p class="mr-2 mb-0">
                        Absolutely Yes! SocialV is the Responsive Bootstrap 5
                        Admin Dashboard Template.
                      </p>
                    </div>
                  </div>
                </div>
              </div>
              <div class="iq-message-body iq-other-user">
                <div class="chat-profile text-center">
                  <img
                    src="../../../assets/images/chat/avatar/07.png"
                    alt="chat-user"
                    class="avatar-40 rounded-pill"
                    loading="lazy"
                  />
                  <small class="iq-chating p-0 mb-0 d-block">6:52</small>
                </div>
                <div class="iq-chat-text">
                  <div
                    class="d-flex align-items-center justify-content-start gap-md-2"
                  >
                    <div class="iq-chating-content d-flex align-items-center">
                      <p class="mr-2 mb-0">Looks clean and fresh UI.</p>
                    </div>
                    <a
                      href="#"
                      class="material-symbols-outlined font-size-20 text-dark reply"
                      v-b-tooltip.hover
                      title="Reply"
                      >reply</a
                    >
                    <div
                      class="dropdown cursor-pointer more"
                      v-b-tooltip.hover
                      title="More"
                    >
                      <div
                        class="lh-1"
                        id="post-option"
                        data-bs-toggle="dropdown"
                      >
                        <span class="material-symbols-outlined text-dark">
                          more_vert
                        </span>
                      </div>
                      <div
                        class="dropdown-menu dropdown-menu-right"
                        aria-labelledby="post-option"
                        style=""
                      >
                        <a class="dropdown-item" href="#"
                          ><span
                            class="material-symbols-outlined align-middle font-size-20 me-1"
                            >content_copy</span
                          >Copy message text</a
                        >
                        <a class="dropdown-item" href="#">
                          <span
                            class="material-symbols-outlined align-middle font-size-20 me-1"
                            >edit</span
                          >Edit</a
                        >
                      </div>
                    </div>
                  </div>
                </div>
              </div>
              <div class="iq-message-body iq-current-user">
                <div class="chat-profile text-center">
                  <img
                    src="../../../assets/images/chat/avatar/10.png"
                    alt="chat-user"
                    class="avatar-40 rounded-pill"
                    loading="lazy"
                  />
                  <small class="iq-chating p-0 mb-0 d-block">6:53</small>
                </div>
                <div class="iq-chat-text">
                  <div
                    class="d-flex align-items-center justify-content-end gap-1 gap-md-2"
                  >
                    <div
                      class="dropdown cursor-pointer more"
                      v-b-tooltip.hover
                      title="More"
                    >
                      <div
                        class="lh-1"
                        id="post-option"
                        data-bs-toggle="dropdown"
                      >
                        <span class="material-symbols-outlined text-dark">
                          more_vert
                        </span>
                      </div>
                      <div
                        class="dropdown-menu dropdown-menu-right"
                        aria-labelledby="post-option"
                        style=""
                      >
                        <a class="dropdown-item" href="#"
                          ><span
                            class="material-symbols-outlined align-middle font-size-20 me-1"
                            >content_copy</span
                          >Copy message text</a
                        >
                        <a class="dropdown-item" href="#">
                          <span
                            class="material-symbols-outlined align-middle font-size-20 me-1"
                            >edit</span
                          >Edit</a
                        >
                      </div>
                    </div>
                    <div class="iq-chating-content d-flex align-items-center">
                      <p class="mr-2 mb-0">Thanks, from ThemeForest.</p>
                    </div>
                  </div>
                </div>
              </div>
              <div class="iq-message-body iq-other-user">
                <div class="chat-profile text-center">
                  <img
                    src="../../../assets/images/chat/avatar/07.png"
                    alt="chat-user"
                    class="avatar-40 rounded-pill"
                    loading="lazy"
                  />
                  <small class="iq-chating p-0 mb-0 d-block">6:54</small>
                </div>
                <div class="iq-chat-text">
                  <div
                    class="d-flex align-items-center justify-content-start gap-md-2"
                  >
                    <div class="iq-chating-content d-flex align-items-center">
                      <p class="mr-2 mb-0">I will purchase it for sure. 👍</p>
                    </div>
                    <a
                      href="#"
                      class="material-symbols-outlined font-size-20 text-dark reply"
                      v-b-tooltip.hover
                      title="Reply"
                      >reply</a
                    >
                    <div
                      class="dropdown cursor-pointer more"
                      v-b-tooltip.hover
                      title="More"
                    >
                      <div
                        class="lh-1"
                        id="post-option"
                        data-bs-toggle="dropdown"
                      >
                        <span class="material-symbols-outlined text-dark">
                          more_vert
                        </span>
                      </div>
                      <div
                        class="dropdown-menu dropdown-menu-right"
                        aria-labelledby="post-option"
                        style=""
                      >
                        <a class="dropdown-item" href="#"
                          ><span
                            class="material-symbols-outlined align-middle font-size-20 me-1"
                            >content_copy</span
                          >Copy message text</a
                        >
                        <a class="dropdown-item" href="#">
                          <span
                            class="material-symbols-outlined align-middle font-size-20 me-1"
                            >edit</span
                          >Edit</a
                        >
                      </div>
                    </div>
                  </div>
                </div>
              </div>
              <div class="iq-message-body iq-current-user">
                <div class="chat-profile text-center">
                  <img
                    src="../../../assets/images/chat/avatar/10.png"
                    alt="chat-user"
                    class="avatar-40 rounded-pill"
                    loading="lazy"
                  />
                  <small class="iq-chating p-0 mb-0 d-block">6:55</small>
                </div>
                <div class="iq-chat-text">
                  <div
                    class="d-flex align-items-center justify-content-end gap-1 gap-md-2"
                  >
                    <div
                      class="dropdown cursor-pointer more"
                      v-b-tooltip.hover
                      title="More"
                    >
                      <div
                        class="lh-1"
                        id="post-option"
                        data-bs-toggle="dropdown"
                      >
                        <span class="material-symbols-outlined text-dark">
                          more_vert
                        </span>
                      </div>
                      <div
                        class="dropdown-menu dropdown-menu-right"
                        aria-labelledby="post-option"
                        style=""
                      >
                        <a class="dropdown-item" href="#"
                          ><span
                            class="material-symbols-outlined align-middle font-size-20 me-1"
                            >content_copy</span
                          >Copy message text</a
                        >
                        <a class="dropdown-item" href="#">
                          <span
                            class="material-symbols-outlined align-middle font-size-20 me-1"
                            >edit</span
                          >Edit</a
                        >
                      </div>
                    </div>
                    <div class="iq-chating-content d-flex align-items-center">
                      <p class="mr-2 mb-0">Okay Thanks..</p>
                    </div>
                  </div>
                </div>
              </div>
              <div class="iq-message-body iq-other-user">
                <div class="chat-profile text-center">
                  <img
                    src="../../../assets/images/chat/avatar/07.png"
                    alt="chat-user"
                    class="avatar-40 rounded-pill"
                    loading="lazy"
                  />
                  <small class="iq-chating p-0 mb-0 d-block">7:54</small>
                </div>
                <div class="iq-chat-text">
                  <div
                    class="d-flex align-items-center justify-content-start gap-md-2"
                  >
                    <div class="iq-chating-content d-flex align-items-center">
                      <p class="mr-2 mb-0">Looks clean and fresh UI.</p>
                    </div>
                    <a
                      href="#"
                      class="material-symbols-outlined font-size-20 text-dark reply"
                      v-b-tooltip.hover
                      title="Reply"
                      >reply</a
                    >
                    <div
                      class="dropdown cursor-pointer more"
                      v-b-tooltip.hover
                      title="More"
                    >
                      <div
                        class="lh-1"
                        id="post-option"
                        data-bs-toggle="dropdown"
                      >
                        <span class="material-symbols-outlined text-dark">
                          more_vert
                        </span>
                      </div>
                      <div
                        class="dropdown-menu dropdown-menu-right"
                        aria-labelledby="post-option"
                        style=""
                      >
                        <a class="dropdown-item" href="#"
                          ><span
                            class="material-symbols-outlined align-middle font-size-20 me-1"
                            >content_copy</span
                          >Copy message text</a
                        >
                        <a class="dropdown-item" href="#">
                          <span
                            class="material-symbols-outlined align-middle font-size-20 me-1"
                            >edit</span
                          >Edit</a
                        >
                      </div>
                    </div>
                  </div>
                </div>
              </div>
              <div class="iq-message-body iq-current-user">
                <div class="chat-profile text-center">
                  <img
                    src="../../../assets/images/chat/avatar/10.png"
                    alt="chat-user"
                    class="avatar-40 rounded-pill"
                    loading="lazy"
                  />
                  <small class="iq-chating p-0 mb-0 d-block">7:55</small>
                </div>
                <div class="iq-chat-text">
                  <div
                    class="d-flex align-items-center justify-content-end gap-1 gap-md-2"
                  >
                    <div
                      class="dropdown cursor-pointer more"
                      v-b-tooltip.hover
                      title="More"
                    >
                      <div
                        class="lh-1"
                        id="post-option"
                        data-bs-toggle="dropdown"
                      >
                        <span class="material-symbols-outlined text-dark">
                          more_vert
                        </span>
                      </div>
                      <div
                        class="dropdown-menu dropdown-menu-right"
                        aria-labelledby="post-option"
                        style=""
                      >
                        <a class="dropdown-item" href="#"
                          ><span
                            class="material-symbols-outlined align-middle font-size-20 me-1"
                            >content_copy</span
                          >Copy message text</a
                        >
                        <a class="dropdown-item" href="#">
                          <span
                            class="material-symbols-outlined align-middle font-size-20 me-1"
                            >edit</span
                          >Edit</a
                        >
                      </div>
                    </div>
                    <div class="iq-chating-content d-flex align-items-center">
                      <p class="mr-2 mb-0">Thanks, from ThemeForest.</p>
                    </div>
                  </div>
                </div>
              </div>
            </div>
            <div class="card-footer px-3 py-3 border-top rounded-0">
              <form class="d-flex align-items-center" action="#">
                <div class="chat-attagement d-flex">
                  <a href="#" class="d-flex align-items-center pe-3">
                    <svg
                      class="icon-24"
                      width="24"
                      viewBox="0 0 24 25"
                      fill="none"
                      xmlns="http://www.w3.org/2000/svg"
                    >
                      <g clip-path="url(#clip0_156_599)">
                        <path
                          d="M20.4853 4.01473C18.2188 1.74823 15.2053 0.5 12 0.5C8.79469 0.5 5.78119 1.74823 3.51473 4.01473C1.24819 6.28119 0 9.29469 0 12.5C0 15.7053 1.24819 18.7188 3.51473 20.9853C5.78119 23.2518 8.79469 24.5 12 24.5C15.2053 24.5 18.2188 23.2518 20.4853 20.9853C22.7518 18.7188 24 15.7053 24 12.5C24 9.29469 22.7518 6.28119 20.4853 4.01473ZM12 23.0714C6.17091 23.0714 1.42856 18.3291 1.42856 12.5C1.42856 6.67091 6.17091 1.92856 12 1.92856C17.8291 1.92856 22.5714 6.67091 22.5714 12.5C22.5714 18.3291 17.8291 23.0714 12 23.0714Z"
                          fill="currentcolor"
                        ></path>
                        <path
                          d="M9.40398 9.3309C8.23431 8.16114 6.33104 8.16123 5.16136 9.3309C4.88241 9.60981 4.88241 10.0621 5.16136 10.3411C5.44036 10.62 5.89266 10.62 6.17157 10.3411C6.78432 9.72836 7.78126 9.7284 8.39392 10.3411C8.53342 10.4806 8.71618 10.5503 8.89895 10.5503C9.08171 10.5503 9.26457 10.4806 9.40398 10.3411C9.68293 10.0621 9.68293 9.60986 9.40398 9.3309Z"
                          fill="currentcolor"
                        ></path>
                        <path
                          d="M18.8384 9.3309C17.6688 8.16123 15.7655 8.16114 14.5958 9.3309C14.3169 9.60981 14.3169 10.0621 14.5958 10.3411C14.8748 10.62 15.3271 10.62 15.606 10.3411C16.2187 9.72836 17.2156 9.72831 17.8284 10.3411C17.9679 10.4806 18.1506 10.5503 18.3334 10.5503C18.5162 10.5503 18.699 10.4806 18.8384 10.3411C19.1174 10.0621 19.1174 9.60986 18.8384 9.3309Z"
                          fill="currentcolor"
                        ></path>
                        <path
                          d="M18.3335 13.024H5.6668C5.2723 13.024 4.95251 13.3438 4.95251 13.7383C4.95251 17.6243 8.11409 20.7859 12.0001 20.7859C15.8862 20.7859 19.0477 17.6243 19.0477 13.7383C19.0477 13.3438 18.728 13.024 18.3335 13.024ZM12.0001 19.3573C9.14366 19.3573 6.77816 17.215 6.42626 14.4525H17.574C17.2221 17.215 14.8566 19.3573 12.0001 19.3573Z"
                          fill="currentcolor"
                        ></path>
                      </g>
                      <defs>
                        <clipPath>
                          <rect
                            width="24"
                            height="24"
                            fill="white"
                            transform="translate(0 0.5)"
                          ></rect>
                        </clipPath>
                      </defs>
                    </svg>
                  </a>
                  <a href="#" class="d-flex align-items-center pe-3">
                    <svg
                      class="icon-24"
                      width="18"
                      height="23"
                      viewBox="0 0 18 23"
                      fill="none"
                      xmlns="http://www.w3.org/2000/svg"
                    >
                      <path
                        d="M9.00021 21.5V18.3391"
                        stroke="currentcolor"
                        stroke-width="2"
                        stroke-linecap="round"
                        stroke-linejoin="round"
                      ></path>
                      <path
                        fill-rule="evenodd"
                        clip-rule="evenodd"
                        d="M9.00021 14.3481V14.3481C6.75611 14.3481 4.9384 12.5218 4.9384 10.2682V5.58095C4.9384 3.32732 6.75611 1.5 9.00021 1.5C11.2433 1.5 13.061 3.32732 13.061 5.58095V10.2682C13.061 12.5218 11.2433 14.3481 9.00021 14.3481Z"
                        stroke="currentcolor"
                        stroke-width="2"
                        stroke-linecap="round"
                        stroke-linejoin="round"
                      ></path>
                      <path
                        d="M17 10.3006C17 14.7394 13.418 18.3383 9 18.3383C4.58093 18.3383 1 14.7394 1 10.3006"
                        stroke="currentcolor"
                        stroke-width="2"
                        stroke-linecap="round"
                        stroke-linejoin="round"
                      ></path>
                      <path
                        d="M11.0689 6.25579H13.0585"
                        stroke="currentcolor"
                        stroke-width="2"
                        stroke-linecap="round"
                        stroke-linejoin="round"
                      ></path>
                      <path
                        d="M10.0704 9.59344H13.0605"
                        stroke="currentcolor"
                        stroke-width="2"
                        stroke-linecap="round"
                        stroke-linejoin="round"
                      ></path>
                    </svg>
                  </a>
                </div>
                <input
                  type="text"
                  class="form-control me-3"
                  placeholder="Type your message"
                />
                <button
                  type="submit"
                  class="btn btn-primary d-flex align-items-center"
                >
                  <svg
                    class="icon-20"
                    width="18"
                    viewBox="0 0 20 21"
                    fill="none"
                    xmlns="http://www.w3.org/2000/svg"
                  >
                    <path
                      d="M13.8325 6.67463L8.10904 12.4592L1.59944 8.38767C0.66675 7.80414 0.860765 6.38744 1.91572 6.07893L17.3712 1.55277C18.3373 1.26963 19.2326 2.17283 18.9456 3.142L14.3731 18.5868C14.0598 19.6432 12.6512 19.832 12.0732 18.8953L8.10601 12.4602"
                      stroke="currentcolor"
                      stroke-width="2"
                      stroke-linecap="round"
                      stroke-linejoin="round"
                    ></path></svg
                  ><span class="d-none d-lg-block ms-1">Send</span>
                </button>
              </form>
            </div>
          </div>
          <div
            class="card tab-pane mb-0 fade"
            id="user-content-108"
            role="tabpanel"
          >
            <div class="chat-head">
              <header
                class="d-flex justify-content-between align-items-center pt-3 ps-3 pe-3 pb-3"
              >
                <div class="d-flex align-items-center gap-3">
                  <div class="d-block d-xl-none">
                    <button
                      class="btn btn-sm btn-primary rounded btn-icon"
                      data-toggle="sidebar"
                      data-active="true"
                    >
                      <span class="btn-inner">
                        <svg class="icon-rtl" width="20px" viewBox="0 0 24 24">
                          <path
                            fill="currentColor"
                            d="M4,11V13H16L10.5,18.5L11.92,19.92L19.84,12L11.92,4.08L10.5,5.5L16,11H4Z"
                          ></path>
                        </svg>
                      </span>
                    </button>
                  </div>
                  <div class="avatar chat-user-profile m-0">
                    <img
                      src="../../../assets/images/chat/avatar/08.png"
                      alt="avatar"
                      class="avatar-50 rounded-pill"
                      loading="lazy"
                    />
                    <div class="iq-profile-badge bg-danger"></div>
                  </div>
                  <div>
                    <h5 class="mb-0">Arnold Schwarzenegger</h5>
                    <small class="text-capitalize fw-500">offline</small>
                  </div>
                </div>

                <div class="chat-header-icons d-inline-flex ms-auto">
                  <a
                    href="#"
                    class="chat-icon-phone bg-primary-subtle d-flex align-items-center justify-content-center"
                  >
                    <i class="material-symbols-outlined md-18">phone</i>
                  </a>
                  <a
                    href="#"
                    class="chat-icon-video bg-primary-subtle d-flex align-items-center justify-content-center"
                  >
                    <i class="material-symbols-outlined md-18">videocam</i>
                  </a>
                  <a
                    href="#"
                    class="chat-icon-delete bg-primary-subtle d-flex align-items-center justify-content-center"
                  >
                    <i class="material-symbols-outlined md-18">delete</i>
                  </a>
                  <span
                    class="dropdown bg-primary-subtle d-flex align-items-center justify-content-center"
                  >
                    <svg
                      class="icon-20 nav-hide-arrow cursor-pointer pe-0"
                      id="dropdownMenuButton-15"
                      data-bs-toggle="dropdown"
                      aria-haspopup="true"
                      aria-expanded="false"
                      role="menu"
                      width="24"
                      xmlns="http://www.w3.org/2000/svg"
                      viewBox="0 0 24 24"
                      fill="none"
                    >
                      <path
                        fill-rule="evenodd"
                        clip-rule="evenodd"
                        d="M10 20.6788C10 21.9595 11.0378 23 12.3113 23C13.5868 23 14.6265 21.9595 14.6265 20.6788C14.6265 19.3981 13.5868 18.3576 12.3113 18.3576C11.0378 18.3576 10 19.3981 10 20.6788ZM10 12.0005C10 13.2812 11.0378 14.3217 12.3113 14.3217C13.5868 14.3217 14.6265 13.2812 14.6265 12.0005C14.6265 10.7198 13.5868 9.67929 12.3113 9.67929C11.0378 9.67929 10 10.7198 10 12.0005ZM12.3113 5.64239C11.0378 5.64239 10 4.60192 10 3.3212C10 2.04047 11.0378 1 12.3113 1C13.5868 1 14.6265 2.04047 14.6265 3.3212C14.6265 4.60192 13.5868 5.64239 12.3113 5.64239Z"
                        fill="currentColor"
                      ></path>
                    </svg>
                    <span
                      class="dropdown-menu dropdown-menu-right"
                      aria-labelledby="dropdownMenuButton-15"
                    >
                      <a
                        class="dropdown-item d-flex align-items-center"
                        href="#"
                        ><i class="material-symbols-outlined md-18 me-1"
                          >push_pin</i
                        >Pin to top</a
                      >
                      <a
                        class="dropdown-item d-flex align-items-center"
                        href="#"
                        ><i class="material-symbols-outlined md-18 me-1"
                          >delete_outline</i
                        >Delete chat</a
                      >
                      <a
                        class="dropdown-item d-flex align-items-center"
                        href="#"
                        ><i class="material-symbols-outlined md-18 me-1"
                          >watch_later</i
                        >Block</a
                      >
                    </span>
                  </span>
                </div>
              </header>
            </div>
            <div class="card-body chat-body bg-body">
              <div class="chat-day-title">
                <span class="main-title">Feb 1,2021</span>
              </div>
              <div class="iq-message-body iq-current-user">
                <div class="chat-profile text-center">
                  <img
                    src="../../../assets/images/chat/avatar/10.png"
                    alt="chat-user"
                    class="avatar-40 rounded-pill"
                    loading="lazy"
                  />
                  <small class="iq-chating p-0 mb-0 d-block">16:34</small>
                </div>
                <div class="iq-chat-text">
                  <div
                    class="d-flex align-items-center justify-content-end gap-1 gap-md-2"
                  >
                    <div
                      class="dropdown cursor-pointer more"
                      v-b-tooltip.hover
                      title="More"
                    >
                      <div
                        class="lh-1"
                        id="post-option"
                        data-bs-toggle="dropdown"
                      >
                        <span class="material-symbols-outlined text-dark">
                          more_vert
                        </span>
                      </div>
                      <div
                        class="dropdown-menu dropdown-menu-right"
                        aria-labelledby="post-option"
                        style=""
                      >
                        <a class="dropdown-item" href="#"
                          ><span
                            class="material-symbols-outlined align-middle font-size-20 me-1"
                            >content_copy</span
                          >Copy message text</a
                        >
                        <a class="dropdown-item" href="#">
                          <span
                            class="material-symbols-outlined align-middle font-size-20 me-1"
                            >edit</span
                          >Edit</a
                        >
                      </div>
                    </div>
                    <div class="iq-chating-content d-flex align-items-center">
                      <p class="mr-2 mb-0">
                        How can we help? We're here for you! 😄
                      </p>
                    </div>
                  </div>
                </div>
              </div>
              <div class="iq-message-body iq-other-user">
                <div class="chat-profile text-center">
                  <img
                    src="../../../assets/images/chat/avatar/08.png"
                    alt="chat-user"
                    class="avatar-40 rounded-pill"
                    loading="lazy"
                  />
                  <small class="iq-chating p-0 mb-0 d-block">16:40</small>
                </div>
                <div class="iq-chat-text">
                  <div
                    class="d-flex align-items-center justify-content-start gap-md-2"
                  >
                    <div class="iq-chating-content d-flex align-items-center">
                      <p class="mr-2 mb-0">
                        Hey John, I am looking for the best admin template.
                        Could you please help me to find it out? 🤔
                      </p>
                    </div>
                    <a
                      href="#"
                      class="material-symbols-outlined font-size-20 text-dark reply"
                      v-b-tooltip.hover
                      title="Reply"
                      >reply</a
                    >
                    <div
                      class="dropdown cursor-pointer more"
                      v-b-tooltip.hover
                      title="More"
                    >
                      <div
                        class="lh-1"
                        id="post-option"
                        data-bs-toggle="dropdown"
                      >
                        <span class="material-symbols-outlined text-dark">
                          more_vert
                        </span>
                      </div>
                      <div
                        class="dropdown-menu dropdown-menu-right"
                        aria-labelledby="post-option"
                        style=""
                      >
                        <a class="dropdown-item" href="#"
                          ><span
                            class="material-symbols-outlined align-middle font-size-20 me-1"
                            >content_copy</span
                          >Copy message text</a
                        >
                        <a class="dropdown-item" href="#">
                          <span
                            class="material-symbols-outlined align-middle font-size-20 me-1"
                            >edit</span
                          >Edit</a
                        >
                      </div>
                    </div>
                  </div>
                </div>
              </div>
              <div class="iq-message-body iq-current-user">
                <div class="chat-profile text-center">
                  <img
                    src="../../../assets/images/chat/avatar/10.png"
                    alt="chat-user"
                    class="avatar-40 rounded-pill"
                    loading="lazy"
                  />
                  <small class="iq-chating p-0 mb-0 d-block">6:49</small>
                </div>
                <div class="iq-chat-text">
                  <div
                    class="d-flex align-items-center justify-content-end gap-1 gap-md-2"
                  >
                    <div
                      class="dropdown cursor-pointer more"
                      v-b-tooltip.hover
                      title="More"
                    >
                      <div
                        class="lh-1"
                        id="post-option"
                        data-bs-toggle="dropdown"
                      >
                        <span class="material-symbols-outlined text-dark">
                          more_vert
                        </span>
                      </div>
                      <div
                        class="dropdown-menu dropdown-menu-right"
                        aria-labelledby="post-option"
                        style=""
                      >
                        <a class="dropdown-item" href="#"
                          ><span
                            class="material-symbols-outlined align-middle font-size-20 me-1"
                            >content_copy</span
                          >Copy message text</a
                        >
                        <a class="dropdown-item" href="#">
                          <span
                            class="material-symbols-outlined align-middle font-size-20 me-1"
                            >edit</span
                          >Edit</a
                        >
                      </div>
                    </div>
                    <div class="iq-chating-content d-flex align-items-center">
                      <p class="mr-2 mb-0">
                        Absolutely Yes! SocialV is the Responsive Bootstrap 5
                        Admin Dashboard Template.
                      </p>
                    </div>
                  </div>
                </div>
              </div>
              <div class="iq-message-body iq-other-user">
                <div class="chat-profile text-center">
                  <img
                    src="../../../assets/images/chat/avatar/08.png"
                    alt="chat-user"
                    class="avatar-40 rounded-pill"
                    loading="lazy"
                  />
                  <small class="iq-chating p-0 mb-0 d-block">6:52</small>
                </div>
                <div class="iq-chat-text">
                  <div
                    class="d-flex align-items-center justify-content-start gap-md-2"
                  >
                    <div class="iq-chating-content d-flex align-items-center">
                      <p class="mr-2 mb-0">Looks clean and fresh UI.</p>
                    </div>
                    <a
                      href="#"
                      class="material-symbols-outlined font-size-20 text-dark reply"
                      v-b-tooltip.hover
                      title="Reply"
                      >reply</a
                    >
                    <div
                      class="dropdown cursor-pointer more"
                      v-b-tooltip.hover
                      title="More"
                    >
                      <div
                        class="lh-1"
                        id="post-option"
                        data-bs-toggle="dropdown"
                      >
                        <span class="material-symbols-outlined text-dark">
                          more_vert
                        </span>
                      </div>
                      <div
                        class="dropdown-menu dropdown-menu-right"
                        aria-labelledby="post-option"
                        style=""
                      >
                        <a class="dropdown-item" href="#"
                          ><span
                            class="material-symbols-outlined align-middle font-size-20 me-1"
                            >content_copy</span
                          >Copy message text</a
                        >
                        <a class="dropdown-item" href="#">
                          <span
                            class="material-symbols-outlined align-middle font-size-20 me-1"
                            >edit</span
                          >Edit</a
                        >
                      </div>
                    </div>
                  </div>
                </div>
              </div>
              <div class="iq-message-body iq-current-user">
                <div class="chat-profile text-center">
                  <img
                    src="../../../assets/images/chat/avatar/10.png"
                    alt="chat-user"
                    class="avatar-40 rounded-pill"
                    loading="lazy"
                  />
                  <small class="iq-chating p-0 mb-0 d-block">6:53</small>
                </div>
                <div class="iq-chat-text">
                  <div
                    class="d-flex align-items-center justify-content-end gap-1 gap-md-2"
                  >
                    <div
                      class="dropdown cursor-pointer more"
                      v-b-tooltip.hover
                      title="More"
                    >
                      <div
                        class="lh-1"
                        id="post-option"
                        data-bs-toggle="dropdown"
                      >
                        <span class="material-symbols-outlined text-dark">
                          more_vert
                        </span>
                      </div>
                      <div
                        class="dropdown-menu dropdown-menu-right"
                        aria-labelledby="post-option"
                        style=""
                      >
                        <a class="dropdown-item" href="#"
                          ><span
                            class="material-symbols-outlined align-middle font-size-20 me-1"
                            >content_copy</span
                          >Copy message text</a
                        >
                        <a class="dropdown-item" href="#">
                          <span
                            class="material-symbols-outlined align-middle font-size-20 me-1"
                            >edit</span
                          >Edit</a
                        >
                      </div>
                    </div>
                    <div class="iq-chating-content d-flex align-items-center">
                      <p class="mr-2 mb-0">Thanks, from ThemeForest.</p>
                    </div>
                  </div>
                </div>
              </div>
              <div class="iq-message-body iq-other-user">
                <div class="chat-profile text-center">
                  <img
                    src="../../../assets/images/chat/avatar/08.png"
                    alt="chat-user"
                    class="avatar-40 rounded-pill"
                    loading="lazy"
                  />
                  <small class="iq-chating p-0 mb-0 d-block">6:54</small>
                </div>
                <div class="iq-chat-text">
                  <div
                    class="d-flex align-items-center justify-content-start gap-md-2"
                  >
                    <div class="iq-chating-content d-flex align-items-center">
                      <p class="mr-2 mb-0">I will purchase it for sure. 👍</p>
                    </div>
                    <a
                      href="#"
                      class="material-symbols-outlined font-size-20 text-dark reply"
                      v-b-tooltip.hover
                      title="Reply"
                      >reply</a
                    >
                    <div
                      class="dropdown cursor-pointer more"
                      v-b-tooltip.hover
                      title="More"
                    >
                      <div
                        class="lh-1"
                        id="post-option"
                        data-bs-toggle="dropdown"
                      >
                        <span class="material-symbols-outlined text-dark">
                          more_vert
                        </span>
                      </div>
                      <div
                        class="dropdown-menu dropdown-menu-right"
                        aria-labelledby="post-option"
                        style=""
                      >
                        <a class="dropdown-item" href="#"
                          ><span
                            class="material-symbols-outlined align-middle font-size-20 me-1"
                            >content_copy</span
                          >Copy message text</a
                        >
                        <a class="dropdown-item" href="#">
                          <span
                            class="material-symbols-outlined align-middle font-size-20 me-1"
                            >edit</span
                          >Edit</a
                        >
                      </div>
                    </div>
                  </div>
                </div>
              </div>
              <div class="iq-message-body iq-current-user">
                <div class="chat-profile text-center">
                  <img
                    src="../../../assets/images/chat/avatar/10.png"
                    alt="chat-user"
                    class="avatar-40 rounded-pill"
                    loading="lazy"
                  />
                  <small class="iq-chating p-0 mb-0 d-block">6:55</small>
                </div>
                <div class="iq-chat-text">
                  <div
                    class="d-flex align-items-center justify-content-end gap-1 gap-md-2"
                  >
                    <div
                      class="dropdown cursor-pointer more"
                      v-b-tooltip.hover
                      title="More"
                    >
                      <div
                        class="lh-1"
                        id="post-option"
                        data-bs-toggle="dropdown"
                      >
                        <span class="material-symbols-outlined text-dark">
                          more_vert
                        </span>
                      </div>
                      <div
                        class="dropdown-menu dropdown-menu-right"
                        aria-labelledby="post-option"
                        style=""
                      >
                        <a class="dropdown-item" href="#"
                          ><span
                            class="material-symbols-outlined align-middle font-size-20 me-1"
                            >content_copy</span
                          >Copy message text</a
                        >
                        <a class="dropdown-item" href="#">
                          <span
                            class="material-symbols-outlined align-middle font-size-20 me-1"
                            >edit</span
                          >Edit</a
                        >
                      </div>
                    </div>
                    <div class="iq-chating-content d-flex align-items-center">
                      <p class="mr-2 mb-0">Okay Thanks..</p>
                    </div>
                  </div>
                </div>
              </div>
              <div class="iq-message-body iq-other-user">
                <div class="chat-profile text-center">
                  <img
                    src="../../../assets/images/chat/avatar/08.png"
                    alt="chat-user"
                    class="avatar-40 rounded-pill"
                    loading="lazy"
                  />
                  <small class="iq-chating p-0 mb-0 d-block">7:54</small>
                </div>
                <div class="iq-chat-text">
                  <div
                    class="d-flex align-items-center justify-content-start gap-md-2"
                  >
                    <div class="iq-chating-content d-flex align-items-center">
                      <p class="mr-2 mb-0">Looks clean and fresh UI.</p>
                    </div>
                    <a
                      href="#"
                      class="material-symbols-outlined font-size-20 text-dark reply"
                      v-b-tooltip.hover
                      title="Reply"
                      >reply</a
                    >
                    <div
                      class="dropdown cursor-pointer more"
                      v-b-tooltip.hover
                      title="More"
                    >
                      <div
                        class="lh-1"
                        id="post-option"
                        data-bs-toggle="dropdown"
                      >
                        <span class="material-symbols-outlined text-dark">
                          more_vert
                        </span>
                      </div>
                      <div
                        class="dropdown-menu dropdown-menu-right"
                        aria-labelledby="post-option"
                        style=""
                      >
                        <a class="dropdown-item" href="#"
                          ><span
                            class="material-symbols-outlined align-middle font-size-20 me-1"
                            >content_copy</span
                          >Copy message text</a
                        >
                        <a class="dropdown-item" href="#">
                          <span
                            class="material-symbols-outlined align-middle font-size-20 me-1"
                            >edit</span
                          >Edit</a
                        >
                      </div>
                    </div>
                  </div>
                </div>
              </div>
              <div class="iq-message-body iq-current-user">
                <div class="chat-profile text-center">
                  <img
                    src="../../../assets/images/chat/avatar/10.png"
                    alt="chat-user"
                    class="avatar-40 rounded-pill"
                    loading="lazy"
                  />
                  <small class="iq-chating p-0 mb-0 d-block">7:55</small>
                </div>
                <div class="iq-chat-text">
                  <div
                    class="d-flex align-items-center justify-content-end gap-1 gap-md-2"
                  >
                    <div
                      class="dropdown cursor-pointer more"
                      v-b-tooltip.hover
                      title="More"
                    >
                      <div
                        class="lh-1"
                        id="post-option"
                        data-bs-toggle="dropdown"
                      >
                        <span class="material-symbols-outlined text-dark">
                          more_vert
                        </span>
                      </div>
                      <div
                        class="dropdown-menu dropdown-menu-right"
                        aria-labelledby="post-option"
                        style=""
                      >
                        <a class="dropdown-item" href="#"
                          ><span
                            class="material-symbols-outlined align-middle font-size-20 me-1"
                            >content_copy</span
                          >Copy message text</a
                        >
                        <a class="dropdown-item" href="#">
                          <span
                            class="material-symbols-outlined align-middle font-size-20 me-1"
                            >edit</span
                          >Edit</a
                        >
                      </div>
                    </div>
                    <div class="iq-chating-content d-flex align-items-center">
                      <p class="mr-2 mb-0">Thanks, from ThemeForest.</p>
                    </div>
                  </div>
                </div>
              </div>
            </div>
            <div class="card-footer px-3 py-3 border-top rounded-0">
              <form class="d-flex align-items-center" action="#">
                <div class="chat-attagement d-flex">
                  <a href="#" class="d-flex align-items-center pe-3">
                    <svg
                      class="icon-24"
                      width="24"
                      viewBox="0 0 24 25"
                      fill="none"
                      xmlns="http://www.w3.org/2000/svg"
                    >
                      <g clip-path="url(#clip0_156_599)">
                        <path
                          d="M20.4853 4.01473C18.2188 1.74823 15.2053 0.5 12 0.5C8.79469 0.5 5.78119 1.74823 3.51473 4.01473C1.24819 6.28119 0 9.29469 0 12.5C0 15.7053 1.24819 18.7188 3.51473 20.9853C5.78119 23.2518 8.79469 24.5 12 24.5C15.2053 24.5 18.2188 23.2518 20.4853 20.9853C22.7518 18.7188 24 15.7053 24 12.5C24 9.29469 22.7518 6.28119 20.4853 4.01473ZM12 23.0714C6.17091 23.0714 1.42856 18.3291 1.42856 12.5C1.42856 6.67091 6.17091 1.92856 12 1.92856C17.8291 1.92856 22.5714 6.67091 22.5714 12.5C22.5714 18.3291 17.8291 23.0714 12 23.0714Z"
                          fill="currentcolor"
                        ></path>
                        <path
                          d="M9.40398 9.3309C8.23431 8.16114 6.33104 8.16123 5.16136 9.3309C4.88241 9.60981 4.88241 10.0621 5.16136 10.3411C5.44036 10.62 5.89266 10.62 6.17157 10.3411C6.78432 9.72836 7.78126 9.7284 8.39392 10.3411C8.53342 10.4806 8.71618 10.5503 8.89895 10.5503C9.08171 10.5503 9.26457 10.4806 9.40398 10.3411C9.68293 10.0621 9.68293 9.60986 9.40398 9.3309Z"
                          fill="currentcolor"
                        ></path>
                        <path
                          d="M18.8384 9.3309C17.6688 8.16123 15.7655 8.16114 14.5958 9.3309C14.3169 9.60981 14.3169 10.0621 14.5958 10.3411C14.8748 10.62 15.3271 10.62 15.606 10.3411C16.2187 9.72836 17.2156 9.72831 17.8284 10.3411C17.9679 10.4806 18.1506 10.5503 18.3334 10.5503C18.5162 10.5503 18.699 10.4806 18.8384 10.3411C19.1174 10.0621 19.1174 9.60986 18.8384 9.3309Z"
                          fill="currentcolor"
                        ></path>
                        <path
                          d="M18.3335 13.024H5.6668C5.2723 13.024 4.95251 13.3438 4.95251 13.7383C4.95251 17.6243 8.11409 20.7859 12.0001 20.7859C15.8862 20.7859 19.0477 17.6243 19.0477 13.7383C19.0477 13.3438 18.728 13.024 18.3335 13.024ZM12.0001 19.3573C9.14366 19.3573 6.77816 17.215 6.42626 14.4525H17.574C17.2221 17.215 14.8566 19.3573 12.0001 19.3573Z"
                          fill="currentcolor"
                        ></path>
                      </g>
                      <defs>
                        <clipPath>
                          <rect
                            width="24"
                            height="24"
                            fill="white"
                            transform="translate(0 0.5)"
                          ></rect>
                        </clipPath>
                      </defs>
                    </svg>
                  </a>
                  <a href="#" class="d-flex align-items-center pe-3">
                    <svg
                      class="icon-24"
                      width="18"
                      height="23"
                      viewBox="0 0 18 23"
                      fill="none"
                      xmlns="http://www.w3.org/2000/svg"
                    >
                      <path
                        d="M9.00021 21.5V18.3391"
                        stroke="currentcolor"
                        stroke-width="2"
                        stroke-linecap="round"
                        stroke-linejoin="round"
                      ></path>
                      <path
                        fill-rule="evenodd"
                        clip-rule="evenodd"
                        d="M9.00021 14.3481V14.3481C6.75611 14.3481 4.9384 12.5218 4.9384 10.2682V5.58095C4.9384 3.32732 6.75611 1.5 9.00021 1.5C11.2433 1.5 13.061 3.32732 13.061 5.58095V10.2682C13.061 12.5218 11.2433 14.3481 9.00021 14.3481Z"
                        stroke="currentcolor"
                        stroke-width="2"
                        stroke-linecap="round"
                        stroke-linejoin="round"
                      ></path>
                      <path
                        d="M17 10.3006C17 14.7394 13.418 18.3383 9 18.3383C4.58093 18.3383 1 14.7394 1 10.3006"
                        stroke="currentcolor"
                        stroke-width="2"
                        stroke-linecap="round"
                        stroke-linejoin="round"
                      ></path>
                      <path
                        d="M11.0689 6.25579H13.0585"
                        stroke="currentcolor"
                        stroke-width="2"
                        stroke-linecap="round"
                        stroke-linejoin="round"
                      ></path>
                      <path
                        d="M10.0704 9.59344H13.0605"
                        stroke="currentcolor"
                        stroke-width="2"
                        stroke-linecap="round"
                        stroke-linejoin="round"
                      ></path>
                    </svg>
                  </a>
                </div>
                <input
                  type="text"
                  class="form-control me-3"
                  placeholder="Type your message"
                />
                <button
                  type="submit"
                  class="btn btn-primary d-flex align-items-center"
                >
                  <svg
                    class="icon-20"
                    width="18"
                    viewBox="0 0 20 21"
                    fill="none"
                    xmlns="http://www.w3.org/2000/svg"
                  >
                    <path
                      d="M13.8325 6.67463L8.10904 12.4592L1.59944 8.38767C0.66675 7.80414 0.860765 6.38744 1.91572 6.07893L17.3712 1.55277C18.3373 1.26963 19.2326 2.17283 18.9456 3.142L14.3731 18.5868C14.0598 19.6432 12.6512 19.832 12.0732 18.8953L8.10601 12.4602"
                      stroke="currentcolor"
                      stroke-width="2"
                      stroke-linecap="round"
                      stroke-linejoin="round"
                    ></path></svg
                  ><span class="d-none d-lg-block ms-1">Send</span>
                </button>
              </form>
            </div>
          </div>
          <div
            class="card tab-pane mb-0 fade"
            id="user-content-109"
            role="tabpanel"
          >
            <div class="chat-head">
              <header
                class="d-flex justify-content-between align-items-center pt-3 ps-3 pe-3 pb-3"
              >
                <div class="d-flex align-items-center gap-3">
                  <div class="d-block d-xl-none">
                    <button
                      class="btn btn-sm btn-primary rounded btn-icon"
                      data-toggle="sidebar"
                      data-active="true"
                    >
                      <span class="btn-inner">
                        <svg class="icon-rtl" width="20px" viewBox="0 0 24 24">
                          <path
                            fill="currentColor"
                            d="M4,11V13H16L10.5,18.5L11.92,19.92L19.84,12L11.92,4.08L10.5,5.5L16,11H4Z"
                          ></path>
                        </svg>
                      </span>
                    </button>
                  </div>
                  <div class="avatar chat-user-profile m-0">
                    <img
                      src="../../../assets/images/chat/avatar/09.png"
                      alt="avatar"
                      class="avatar-50 rounded-pill"
                      loading="lazy"
                    />
                    <div class="iq-profile-badge bg-success"></div>
                  </div>
                  <div>
                    <h5 class="mb-0">Leonardo DiCaprio</h5>
                    <small class="text-capitalize fw-500">Online</small>
                  </div>
                </div>

                <div class="chat-header-icons d-inline-flex ms-auto">
                  <a
                    href="#"
                    class="chat-icon-phone bg-primary-subtle d-flex align-items-center justify-content-center"
                  >
                    <i class="material-symbols-outlined md-18">phone</i>
                  </a>
                  <a
                    href="#"
                    class="chat-icon-video bg-primary-subtle d-flex align-items-center justify-content-center"
                  >
                    <i class="material-symbols-outlined md-18">videocam</i>
                  </a>
                  <a
                    href="#"
                    class="chat-icon-delete bg-primary-subtle d-flex align-items-center justify-content-center"
                  >
                    <i class="material-symbols-outlined md-18">delete</i>
                  </a>
                  <span
                    class="dropdown bg-primary-subtle d-flex align-items-center justify-content-center"
                  >
                    <svg
                      class="icon-20 nav-hide-arrow cursor-pointer pe-0"
                      id="dropdownMenuButton-16"
                      data-bs-toggle="dropdown"
                      aria-haspopup="true"
                      aria-expanded="false"
                      role="menu"
                      width="24"
                      xmlns="http://www.w3.org/2000/svg"
                      viewBox="0 0 24 24"
                      fill="none"
                    >
                      <path
                        fill-rule="evenodd"
                        clip-rule="evenodd"
                        d="M10 20.6788C10 21.9595 11.0378 23 12.3113 23C13.5868 23 14.6265 21.9595 14.6265 20.6788C14.6265 19.3981 13.5868 18.3576 12.3113 18.3576C11.0378 18.3576 10 19.3981 10 20.6788ZM10 12.0005C10 13.2812 11.0378 14.3217 12.3113 14.3217C13.5868 14.3217 14.6265 13.2812 14.6265 12.0005C14.6265 10.7198 13.5868 9.67929 12.3113 9.67929C11.0378 9.67929 10 10.7198 10 12.0005ZM12.3113 5.64239C11.0378 5.64239 10 4.60192 10 3.3212C10 2.04047 11.0378 1 12.3113 1C13.5868 1 14.6265 2.04047 14.6265 3.3212C14.6265 4.60192 13.5868 5.64239 12.3113 5.64239Z"
                        fill="currentColor"
                      ></path>
                    </svg>
                    <span
                      class="dropdown-menu dropdown-menu-right"
                      aria-labelledby="dropdownMenuButton-16"
                    >
                      <a
                        class="dropdown-item d-flex align-items-center"
                        href="#"
                        ><i class="material-symbols-outlined md-18 me-1"
                          >push_pin</i
                        >Pin to top</a
                      >
                      <a
                        class="dropdown-item d-flex align-items-center"
                        href="#"
                        ><i class="material-symbols-outlined md-18 me-1"
                          >delete_outline</i
                        >Delete chat</a
                      >
                      <a
                        class="dropdown-item d-flex align-items-center"
                        href="#"
                        ><i class="material-symbols-outlined md-18 me-1"
                          >watch_later</i
                        >Block</a
                      >
                    </span>
                  </span>
                </div>
              </header>
            </div>
            <div class="card-body chat-body bg-body">
              <div class="chat-day-title">
                <span class="main-title">Feb 1,2021</span>
              </div>
              <div class="iq-message-body iq-current-user">
                <div class="chat-profile text-center">
                  <img
                    src="../../../assets/images/chat/avatar/10.png"
                    alt="chat-user"
                    class="avatar-40 rounded-pill"
                    loading="lazy"
                  />
                  <small class="iq-chating p-0 mb-0 d-block">16:34</small>
                </div>
                <div class="iq-chat-text">
                  <div
                    class="d-flex align-items-center justify-content-end gap-1 gap-md-2"
                  >
                    <div
                      class="dropdown cursor-pointer more"
                      v-b-tooltip.hover
                      title="More"
                    >
                      <div
                        class="lh-1"
                        id="post-option"
                        data-bs-toggle="dropdown"
                      >
                        <span class="material-symbols-outlined text-dark">
                          more_vert
                        </span>
                      </div>
                      <div
                        class="dropdown-menu dropdown-menu-right"
                        aria-labelledby="post-option"
                        style=""
                      >
                        <a class="dropdown-item" href="#"
                          ><span
                            class="material-symbols-outlined align-middle font-size-20 me-1"
                            >content_copy</span
                          >Copy message text</a
                        >
                        <a class="dropdown-item" href="#">
                          <span
                            class="material-symbols-outlined align-middle font-size-20 me-1"
                            >edit</span
                          >Edit</a
                        >
                      </div>
                    </div>
                    <div class="iq-chating-content d-flex align-items-center">
                      <p class="mr-2 mb-0">
                        How can we help? We're here for you! 😄
                      </p>
                    </div>
                  </div>
                </div>
              </div>
              <div class="iq-message-body iq-other-user">
                <div class="chat-profile text-center">
                  <img
                    src="../../../assets/images/chat/avatar/09.png"
                    alt="chat-user"
                    class="avatar-40 rounded-pill"
                    loading="lazy"
                  />
                  <small class="iq-chating p-0 mb-0 d-block">16:40</small>
                </div>
                <div class="iq-chat-text">
                  <div
                    class="d-flex align-items-center justify-content-start gap-md-2"
                  >
                    <div class="iq-chating-content d-flex align-items-center">
                      <p class="mr-2 mb-0">
                        Hey John, I am looking for the best admin template.Could
                        you please help me to find it out? 🤔
                      </p>
                    </div>
                    <a
                      href="#"
                      class="material-symbols-outlined font-size-20 text-dark reply"
                      v-b-tooltip.hover
                      title="Reply"
                      >reply</a
                    >
                    <div
                      class="dropdown cursor-pointer more"
                      v-b-tooltip.hover
                      title="More"
                    >
                      <div
                        class="lh-1"
                        id="post-option"
                        data-bs-toggle="dropdown"
                      >
                        <span class="material-symbols-outlined text-dark">
                          more_vert
                        </span>
                      </div>
                      <div
                        class="dropdown-menu dropdown-menu-right"
                        aria-labelledby="post-option"
                        style=""
                      >
                        <a class="dropdown-item" href="#"
                          ><span
                            class="material-symbols-outlined align-middle font-size-20 me-1"
                            >content_copy</span
                          >Copy message text</a
                        >
                        <a class="dropdown-item" href="#">
                          <span
                            class="material-symbols-outlined align-middle font-size-20 me-1"
                            >edit</span
                          >Edit</a
                        >
                      </div>
                    </div>
                  </div>
                </div>
              </div>
              <div class="iq-message-body iq-current-user">
                <div class="chat-profile text-center">
                  <img
                    src="../../../assets/images/chat/avatar/10.png"
                    alt="chat-user"
                    class="avatar-40 rounded-pill"
                    loading="lazy"
                  />
                  <small class="iq-chating p-0 mb-0 d-block">6:49</small>
                </div>
                <div class="iq-chat-text">
                  <div
                    class="d-flex align-items-center justify-content-end gap-1 gap-md-2"
                  >
                    <div
                      class="dropdown cursor-pointer more"
                      v-b-tooltip.hover
                      title="More"
                    >
                      <div
                        class="lh-1"
                        id="post-option"
                        data-bs-toggle="dropdown"
                      >
                        <span class="material-symbols-outlined text-dark">
                          more_vert
                        </span>
                      </div>
                      <div
                        class="dropdown-menu dropdown-menu-right"
                        aria-labelledby="post-option"
                        style=""
                      >
                        <a class="dropdown-item" href="#"
                          ><span
                            class="material-symbols-outlined align-middle font-size-20 me-1"
                            >content_copy</span
                          >Copy message text</a
                        >
                        <a class="dropdown-item" href="#">
                          <span
                            class="material-symbols-outlined align-middle font-size-20 me-1"
                            >edit</span
                          >Edit</a
                        >
                      </div>
                    </div>
                    <div class="iq-chating-content d-flex align-items-center">
                      <p class="mr-2 mb-0">
                        Absolutely Yes! SocialV is the Responsive Bootstrap 5
                        Admin Dashboard Template.
                      </p>
                    </div>
                  </div>
                </div>
              </div>
              <div class="iq-message-body iq-other-user">
                <div class="chat-profile text-center">
                  <img
                    src="../../../assets/images/chat/avatar/09.png"
                    alt="chat-user"
                    class="avatar-40 rounded-pill"
                    loading="lazy"
                  />
                  <small class="iq-chating p-0 mb-0 d-block">6:52</small>
                </div>
                <div class="iq-chat-text">
                  <div
                    class="d-flex align-items-center justify-content-start gap-md-2"
                  >
                    <div class="iq-chating-content d-flex align-items-center">
                      <p class="mr-2 mb-0">Looks clean and fresh UI.</p>
                    </div>
                    <a
                      href="#"
                      class="material-symbols-outlined font-size-20 text-dark reply"
                      v-b-tooltip.hover
                      title="Reply"
                      >reply</a
                    >
                    <div
                      class="dropdown cursor-pointer more"
                      v-b-tooltip.hover
                      title="More"
                    >
                      <div
                        class="lh-1"
                        id="post-option"
                        data-bs-toggle="dropdown"
                      >
                        <span class="material-symbols-outlined text-dark">
                          more_vert
                        </span>
                      </div>
                      <div
                        class="dropdown-menu dropdown-menu-right"
                        aria-labelledby="post-option"
                        style=""
                      >
                        <a class="dropdown-item" href="#"
                          ><span
                            class="material-symbols-outlined align-middle font-size-20 me-1"
                            >content_copy</span
                          >Copy message text</a
                        >
                        <a class="dropdown-item" href="#">
                          <span
                            class="material-symbols-outlined align-middle font-size-20 me-1"
                            >edit</span
                          >Edit</a
                        >
                      </div>
                    </div>
                  </div>
                </div>
              </div>
              <div class="iq-message-body iq-current-user">
                <div class="chat-profile text-center">
                  <img
                    src="../../../assets/images/chat/avatar/10.png"
                    alt="chat-user"
                    class="avatar-40 rounded-pill"
                    loading="lazy"
                  />
                  <small class="iq-chating p-0 mb-0 d-block">6:53</small>
                </div>
                <div class="iq-chat-text">
                  <div
                    class="d-flex align-items-center justify-content-end gap-1 gap-md-2"
                  >
                    <div
                      class="dropdown cursor-pointer more"
                      v-b-tooltip.hover
                      title="More"
                    >
                      <div
                        class="lh-1"
                        id="post-option"
                        data-bs-toggle="dropdown"
                      >
                        <span class="material-symbols-outlined text-dark">
                          more_vert
                        </span>
                      </div>
                      <div
                        class="dropdown-menu dropdown-menu-right"
                        aria-labelledby="post-option"
                        style=""
                      >
                        <a class="dropdown-item" href="#"
                          ><span
                            class="material-symbols-outlined align-middle font-size-20 me-1"
                            >content_copy</span
                          >Copy message text</a
                        >
                        <a class="dropdown-item" href="#">
                          <span
                            class="material-symbols-outlined align-middle font-size-20 me-1"
                            >edit</span
                          >Edit</a
                        >
                      </div>
                    </div>
                    <div class="iq-chating-content d-flex align-items-center">
                      <p class="mr-2 mb-0">Thanks, from ThemeForest.</p>
                    </div>
                  </div>
                </div>
              </div>
              <div class="iq-message-body iq-other-user">
                <div class="chat-profile text-center">
                  <img
                    src="../../../assets/images/chat/avatar/09.png"
                    alt="chat-user"
                    class="avatar-40 rounded-pill"
                    loading="lazy"
                  />
                  <small class="iq-chating p-0 mb-0 d-block">6:54</small>
                </div>
                <div class="iq-chat-text">
                  <div
                    class="d-flex align-items-center justify-content-start gap-md-2"
                  >
                    <div class="iq-chating-content d-flex align-items-center">
                      <p class="mr-2 mb-0">I will purchase it for sure. 👍</p>
                    </div>
                    <a
                      href="#"
                      class="material-symbols-outlined font-size-20 text-dark reply"
                      v-b-tooltip.hover
                      title="Reply"
                      >reply</a
                    >
                    <div
                      class="dropdown cursor-pointer more"
                      v-b-tooltip.hover
                      title="More"
                    >
                      <div
                        class="lh-1"
                        id="post-option"
                        data-bs-toggle="dropdown"
                      >
                        <span class="material-symbols-outlined text-dark">
                          more_vert
                        </span>
                      </div>
                      <div
                        class="dropdown-menu dropdown-menu-right"
                        aria-labelledby="post-option"
                        style=""
                      >
                        <a class="dropdown-item" href="#"
                          ><span
                            class="material-symbols-outlined align-middle font-size-20 me-1"
                            >content_copy</span
                          >Copy message text</a
                        >
                        <a class="dropdown-item" href="#">
                          <span
                            class="material-symbols-outlined align-middle font-size-20 me-1"
                            >edit</span
                          >Edit</a
                        >
                      </div>
                    </div>
                  </div>
                </div>
              </div>
              <div class="iq-message-body iq-current-user">
                <div class="chat-profile text-center">
                  <img
                    src="../../../assets/images/chat/avatar/10.png"
                    alt="chat-user"
                    class="avatar-40 rounded-pill"
                    loading="lazy"
                  />
                  <small class="iq-chating p-0 mb-0 d-block">6:55</small>
                </div>
                <div class="iq-chat-text">
                  <div
                    class="d-flex align-items-center justify-content-end gap-1 gap-md-2"
                  >
                    <div
                      class="dropdown cursor-pointer more"
                      v-b-tooltip.hover
                      title="More"
                    >
                      <div
                        class="lh-1"
                        id="post-option"
                        data-bs-toggle="dropdown"
                      >
                        <span class="material-symbols-outlined text-dark">
                          more_vert
                        </span>
                      </div>
                      <div
                        class="dropdown-menu dropdown-menu-right"
                        aria-labelledby="post-option"
                        style=""
                      >
                        <a class="dropdown-item" href="#"
                          ><span
                            class="material-symbols-outlined align-middle font-size-20 me-1"
                            >content_copy</span
                          >Copy message text</a
                        >
                        <a class="dropdown-item" href="#">
                          <span
                            class="material-symbols-outlined align-middle font-size-20 me-1"
                            >edit</span
                          >Edit</a
                        >
                      </div>
                    </div>
                    <div class="iq-chating-content d-flex align-items-center">
                      <p class="mr-2 mb-0">Okay Thanks..</p>
                    </div>
                  </div>
                </div>
              </div>
              <div class="iq-message-body iq-other-user">
                <div class="chat-profile text-center">
                  <img
                    src="../../../assets/images/chat/avatar/09.png"
                    alt="chat-user"
                    class="avatar-40 rounded-pill"
                    loading="lazy"
                  />
                  <small class="iq-chating p-0 mb-0 d-block">7:54</small>
                </div>
                <div class="iq-chat-text">
                  <div
                    class="d-flex align-items-center justify-content-start gap-md-2"
                  >
                    <div class="iq-chating-content d-flex align-items-center">
                      <p class="mr-2 mb-0">Looks clean and fresh UI.</p>
                    </div>
                    <a
                      href="#"
                      class="material-symbols-outlined font-size-20 text-dark reply"
                      v-b-tooltip.hover
                      title="Reply"
                      >reply</a
                    >
                    <div
                      class="dropdown cursor-pointer more"
                      v-b-tooltip.hover
                      title="More"
                    >
                      <div
                        class="lh-1"
                        id="post-option"
                        data-bs-toggle="dropdown"
                      >
                        <span class="material-symbols-outlined text-dark">
                          more_vert
                        </span>
                      </div>
                      <div
                        class="dropdown-menu dropdown-menu-right"
                        aria-labelledby="post-option"
                        style=""
                      >
                        <a class="dropdown-item" href="#"
                          ><span
                            class="material-symbols-outlined align-middle font-size-20 me-1"
                            >content_copy</span
                          >Copy message text</a
                        >
                        <a class="dropdown-item" href="#">
                          <span
                            class="material-symbols-outlined align-middle font-size-20 me-1"
                            >edit</span
                          >Edit</a
                        >
                      </div>
                    </div>
                  </div>
                </div>
              </div>
              <div class="iq-message-body iq-current-user">
                <div class="chat-profile text-center">
                  <img
                    src="../../../assets/images/chat/avatar/10.png"
                    alt="chat-user"
                    class="avatar-40 rounded-pill"
                    loading="lazy"
                  />
                  <small class="iq-chating p-0 mb-0 d-block">7:55</small>
                </div>
                <div class="iq-chat-text">
                  <div
                    class="d-flex align-items-center justify-content-end gap-1 gap-md-2"
                  >
                    <div
                      class="dropdown cursor-pointer more"
                      v-b-tooltip.hover
                      title="More"
                    >
                      <div
                        class="lh-1"
                        id="post-option"
                        data-bs-toggle="dropdown"
                      >
                        <span class="material-symbols-outlined text-dark">
                          more_vert
                        </span>
                      </div>
                      <div
                        class="dropdown-menu dropdown-menu-right"
                        aria-labelledby="post-option"
                        style=""
                      >
                        <a class="dropdown-item" href="#"
                          ><span
                            class="material-symbols-outlined align-middle font-size-20 me-1"
                            >content_copy</span
                          >Copy message text</a
                        >
                        <a class="dropdown-item" href="#">
                          <span
                            class="material-symbols-outlined align-middle font-size-20 me-1"
                            >edit</span
                          >Edit</a
                        >
                      </div>
                    </div>
                    <div class="iq-chating-content d-flex align-items-center">
                      <p class="mr-2 mb-0">Thanks, from ThemeForest.</p>
                    </div>
                  </div>
                </div>
              </div>
            </div>
            <div class="card-footer px-3 py-3 border-top rounded-0">
              <form class="d-flex align-items-center" action="#">
                <div class="chat-attagement d-flex">
                  <a href="#" class="d-flex align-items-center pe-3">
                    <svg
                      class="icon-24"
                      width="24"
                      viewBox="0 0 24 25"
                      fill="none"
                      xmlns="http://www.w3.org/2000/svg"
                    >
                      <g clip-path="url(#clip0_156_599)">
                        <path
                          d="M20.4853 4.01473C18.2188 1.74823 15.2053 0.5 12 0.5C8.79469 0.5 5.78119 1.74823 3.51473 4.01473C1.24819 6.28119 0 9.29469 0 12.5C0 15.7053 1.24819 18.7188 3.51473 20.9853C5.78119 23.2518 8.79469 24.5 12 24.5C15.2053 24.5 18.2188 23.2518 20.4853 20.9853C22.7518 18.7188 24 15.7053 24 12.5C24 9.29469 22.7518 6.28119 20.4853 4.01473ZM12 23.0714C6.17091 23.0714 1.42856 18.3291 1.42856 12.5C1.42856 6.67091 6.17091 1.92856 12 1.92856C17.8291 1.92856 22.5714 6.67091 22.5714 12.5C22.5714 18.3291 17.8291 23.0714 12 23.0714Z"
                          fill="currentcolor"
                        ></path>
                        <path
                          d="M9.40398 9.3309C8.23431 8.16114 6.33104 8.16123 5.16136 9.3309C4.88241 9.60981 4.88241 10.0621 5.16136 10.3411C5.44036 10.62 5.89266 10.62 6.17157 10.3411C6.78432 9.72836 7.78126 9.7284 8.39392 10.3411C8.53342 10.4806 8.71618 10.5503 8.89895 10.5503C9.08171 10.5503 9.26457 10.4806 9.40398 10.3411C9.68293 10.0621 9.68293 9.60986 9.40398 9.3309Z"
                          fill="currentcolor"
                        ></path>
                        <path
                          d="M18.8384 9.3309C17.6688 8.16123 15.7655 8.16114 14.5958 9.3309C14.3169 9.60981 14.3169 10.0621 14.5958 10.3411C14.8748 10.62 15.3271 10.62 15.606 10.3411C16.2187 9.72836 17.2156 9.72831 17.8284 10.3411C17.9679 10.4806 18.1506 10.5503 18.3334 10.5503C18.5162 10.5503 18.699 10.4806 18.8384 10.3411C19.1174 10.0621 19.1174 9.60986 18.8384 9.3309Z"
                          fill="currentcolor"
                        ></path>
                        <path
                          d="M18.3335 13.024H5.6668C5.2723 13.024 4.95251 13.3438 4.95251 13.7383C4.95251 17.6243 8.11409 20.7859 12.0001 20.7859C15.8862 20.7859 19.0477 17.6243 19.0477 13.7383C19.0477 13.3438 18.728 13.024 18.3335 13.024ZM12.0001 19.3573C9.14366 19.3573 6.77816 17.215 6.42626 14.4525H17.574C17.2221 17.215 14.8566 19.3573 12.0001 19.3573Z"
                          fill="currentcolor"
                        ></path>
                      </g>
                      <defs>
                        <clipPath>
                          <rect
                            width="24"
                            height="24"
                            fill="white"
                            transform="translate(0 0.5)"
                          ></rect>
                        </clipPath>
                      </defs>
                    </svg>
                  </a>
                  <a href="#" class="d-flex align-items-center pe-3">
                    <svg
                      class="icon-24"
                      width="18"
                      height="23"
                      viewBox="0 0 18 23"
                      fill="none"
                      xmlns="http://www.w3.org/2000/svg"
                    >
                      <path
                        d="M9.00021 21.5V18.3391"
                        stroke="currentcolor"
                        stroke-width="2"
                        stroke-linecap="round"
                        stroke-linejoin="round"
                      ></path>
                      <path
                        fill-rule="evenodd"
                        clip-rule="evenodd"
                        d="M9.00021 14.3481V14.3481C6.75611 14.3481 4.9384 12.5218 4.9384 10.2682V5.58095C4.9384 3.32732 6.75611 1.5 9.00021 1.5C11.2433 1.5 13.061 3.32732 13.061 5.58095V10.2682C13.061 12.5218 11.2433 14.3481 9.00021 14.3481Z"
                        stroke="currentcolor"
                        stroke-width="2"
                        stroke-linecap="round"
                        stroke-linejoin="round"
                      ></path>
                      <path
                        d="M17 10.3006C17 14.7394 13.418 18.3383 9 18.3383C4.58093 18.3383 1 14.7394 1 10.3006"
                        stroke="currentcolor"
                        stroke-width="2"
                        stroke-linecap="round"
                        stroke-linejoin="round"
                      ></path>
                      <path
                        d="M11.0689 6.25579H13.0585"
                        stroke="currentcolor"
                        stroke-width="2"
                        stroke-linecap="round"
                        stroke-linejoin="round"
                      ></path>
                      <path
                        d="M10.0704 9.59344H13.0605"
                        stroke="currentcolor"
                        stroke-width="2"
                        stroke-linecap="round"
                        stroke-linejoin="round"
                      ></path>
                    </svg>
                  </a>
                </div>
                <input
                  type="text"
                  class="form-control me-3"
                  placeholder="Type your message"
                />
                <button
                  type="submit"
                  class="btn btn-primary d-flex align-items-center"
                >
                  <svg
                    class="icon-20"
                    width="18"
                    viewBox="0 0 20 21"
                    fill="none"
                    xmlns="http://www.w3.org/2000/svg"
                  >
                    <path
                      d="M13.8325 6.67463L8.10904 12.4592L1.59944 8.38767C0.66675 7.80414 0.860765 6.38744 1.91572 6.07893L17.3712 1.55277C18.3373 1.26963 19.2326 2.17283 18.9456 3.142L14.3731 18.5868C14.0598 19.6432 12.6512 19.832 12.0732 18.8953L8.10601 12.4602"
                      stroke="currentcolor"
                      stroke-width="2"
                      stroke-linecap="round"
                      stroke-linejoin="round"
                    ></path></svg
                  ><span class="d-none d-lg-block ms-1">Send</span>
                </button>
              </form>
            </div>
          </div>
        </div>
      </div>
    </main>
  </div>
</template>

<script setup>
import { socialvue } from "../../../config/pluginInit";
import ToggleButton from "../../../components/custom/Chat/ToggleButton";
import ToggleContent from "../../../components/custom/Chat/ToggleContent";
import { Users, MessagesUser1 } from "../../../FackApi/api/chat";
import User from "../../../Model/User";
import chat from "../../../layouts/Chat.vue";
import { ref, computed, onMounted } from "vue";
import Scrollbar from "smooth-scrollbar";

const toggleSidebar = () => {
  document.getElementsByTagName("ASIDE")[0].classList.toggle("sidebar-mini");
};

const search = ref("");
const user = new User({
  id: 1,
  name: "Bni Jordan",
  role: "Web Designer",
  image: require("../../../assets/images/user/1.jpg"),
  isActive: true,
});
const usersList = Users;
//const chat = MessagesUser1;
const active = ref(true);

const filteredList = computed(() => {
  return usersList.filter((item) => {
    return item.name.toLowerCase().includes(search.value.toLowerCase());
  });
});

const publicList = computed(() => {
  return usersList.filter((item) => {
    return !item.isPrivate
      ? item.name.toLowerCase().includes(search.value.toLowerCase())
      : false;
  });
});

const privateList = computed(() => {
  return usersList.filter((item) => {
    return item.isPrivate
      ? item.name.toLowerCase().includes(search.value.toLowerCase())
      : false;
  });
});

const checkUser = (item, type) => {
  const user = usersList.find((user) => user.id === item);
  let final;
  if (user !== undefined) {
    switch (type) {
      case "name":
        final = user.name;
        break;
      case "image":
        final = user.image;
        break;
      case "role":
        final = user.role;
        break;
    }
    return final;
  }
  return require("../../../assets/images/user/user-5.jpg");
};

onMounted(() => {
  Scrollbar.init(document.querySelector(".data-scrollbar"), {
    continuousScrolling: false,
  });
});
</script>

<style>
.remove-toggle::after {
  content: unset;
}
</style>
